...
export const CURRENT_RESEARCH_MILESTONES = [
  ...
  {
    id: 'known-truth-mechanism-cameras', phase: 'Phase 1151-1201', tone: '#f59e0b',
    title: { zh: '已知真值机制相机与外部效度审计', en: 'Known-Truth Mechanism Cameras and External Validity' },
    summary: { zh: '用机制真值已知的网络校准双路径、冗余、协同、形成事件、响应商与救援相机，再让自由网络接受外部效度检验。', en: 'Calibrated cameras for dual paths, redundancy, synergy, formation events, response quotients, and rescue on known-truth networks before external-validity tests.' },
    objective: { zh: '确认分析算法在什么条件下能识别真实内部结构，而不是只产生看起来合理的投影。', en: 'Determine when analysis algorithms recover real internal structure rather than plausible projections.' },
    result: { zh: '相机在部分已知真值条件下通过，但学习后机制形态、组合泛化、长期身份和粗联盟救援反复出现边界。', en: 'Cameras passed in some known-truth settings, while learned morphology, compositional generalization, long-term identity, and coarse rescue exposed limits.' },
    lesson: { zh: '可迁移研究对象更接近条件化未来响应和功能等价类，而不是固定层、方向或单点组件。', en: 'Transferable objects are closer to conditioned future responses and functional equivalence classes than fixed layers, directions, or single components.' },
    impact: { zh: '为后续类型化证据编译器、救援协议和功能商合同建立校准基础。', en: 'Established calibration foundations for typed evidence compilation, rescue protocols, and functional quotient contracts.' },
  },
  {
    id: 'typed-functional-objects', phase: 'Phase 1202-1228', tone: '#22d3ee',
    title: { zh: '类型化功能对象与原子操作', en: 'Typed Functional Objects and Atomic Operations' },
    summary: { zh: '对象—属性绑定、操作行为、生成时间和内部响应被拆成分别授权的对象；五个行为原子获得独立确认。', en: 'Object-attribute binding, operation behavior, generation time, and internal response were separated into independently authorized objects; five behavior atoms were confirmed.' },
    objective: { zh: '找到既能通过行为资格、又能与竞争程序区分，并可安全进入内部机制研究的最小功能对象。', en: 'Find minimal functional objects that pass behavior qualification, distinguish competing programs, and safely authorize internal study.' },
    result: { zh: '获得局部 Qwen3 纵深和双向因果转移拼图，但单点答案边界、固定数值域和教师强制空间都不能承载完整机制。', en: 'Obtained local Qwen3 depth and bidirectional causal-transfer pieces, while single answer boundaries, fixed numeric domains, and teacher-forced space remained insufficient.' },
    lesson: { zh: '行为、内部响应、干预和自然生成是不同证据类型，任何一项都不能吞并其他类型。', en: 'Behavior, internal response, intervention, and natural generation are distinct evidence types and cannot substitute for one another.' },
    impact: { zh: '研究从单一总门转向类型化对象、类型化资格和明确权限边界。', en: 'Shifted research from a single aggregate gate toward typed objects, typed qualification, and explicit permissions.' },
  },
  {
    id: 'typed-generation-boundary', phase: 'Phase 1229-1235', tone: '#a78bfa',
    title: { zh: '程序可识别材料与七读出边界', en: 'Program-Identifiable Materials and Seven-Readout Boundary' },
    summary: { zh: '去答案载荷、非双射世界和三重密封材料把内容选择、严格格式、句式生成与自然内容分开测试。', en: 'Answer-free inputs, non-bijective worlds, and triple-sealed materials tested content selection, exact format, sentence generation, and natural content separately.' },
    objective: { zh: '判断同一查询功能在不同输出合同时究竟保持哪些行为，而不预设隐藏内容模块。', en: 'Determine which behaviors remain across output contracts without presupposing a hidden content module.' },
    result: { zh: 'Phase1235 四类门为 (1,0,1,1)：候选、固定句式和自然内容通过，严格短字符串失败；总门为 0。', en: 'Phase1235 gates were (1,0,1,1): candidate, fixed sentence, and natural content passed, while exact short string failed; the total gate was zero.' },
    lesson: { zh: '完整候选排序正确不蕴含所有字符串读出稳定，行为对象必须表示为类型化响应向量。', en: 'Correct full-candidate ranking does not imply stable string readouts; behavior objects must be represented as typed response vectors.' },
    impact: { zh: 'K210 成为行为与 token-rank 的正式边界，同时停止未授权的 hidden 扫描。', en: 'K210 became a formal behavior and token-rank boundary while stopping unauthorized hidden-state scans.' },
  },
  {
    id: 'research-os-wp00', phase: 'Phase 1236 · C001-WP00', tone: '#34d399',
    title: { zh: '全局结构辨识研究操作系统', en: 'Global Structure-Identification Research OS' },
    summary: { zh: '关键证据、假说、拼图、对象、构念、合同和裁决进入机器可校验注册表，Phase 降为执行批次。', en: 'Key evidence, hypotheses, puzzles, objects, constructs, contracts, and decisions entered machine-validatable registries; phases became execution batches.' },
    objective: { zh: '阻止局部正结果越级冒充全局机制，并让每次模型运行都只能由冻结合同明确授权。', en: 'Prevent local positives from masquerading as global mechanisms and require frozen contracts to authorize every model run.' },
    result: { zh: 'WP00 校验通过；WP01 合同已预注册，但 run_ready=false，只允许无模型材料和反泄漏预审计。', en: 'WP00 validation passed; the WP01 contract is preregistered but run_ready=false, allowing only model-free material and leakage pre-audits.' },
    lesson: { zh: '工程闭合不是科学正证据；“机器可拒绝”比继续增加理论名词更能保护研究有效性。', en: 'Engineering closure is not positive scientific evidence; machine-enforced refusal protects validity better than adding theory labels.' },
    impact: { zh: '下一主线改为响应张量、候选机制竞赛、主动实验和封存批量裁决。', en: 'The next main line centers on response tensors, mechanism competition, active experiments, and sealed batch adjudication.' },
  },
];
