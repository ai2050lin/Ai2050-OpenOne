...
MODEL="${1:?usage: $0 MODEL [phase294 args...]}"
shift || true

export OPENONE_CONSERVATIVE_ENV="${OPENONE_CONSERVATIVE_ENV:-openone-cuda121}"
export OUTPUT_DIR="${OUTPUT_DIR:-results/gpt5_phase294_dynamic_recompute_pilot}"
export MAX_SECONDS="${MAX_SECONDS:-5400}"
if [[ "$MODEL" == "glm4" ]]; then
...
RUN_ID="$(date +%Y%m%d_%H%M%S)_phase294_${MODEL}"
...
echo "=== Phase294 conservative logged run ==="
date '+%Y-%m-%d %H:%M:%S %Z'
echo "run_id=${RUN_ID}"
echo "model=${MODEL}"
...
try:
    ...
    print("flash_attn_unavailable", repr(exc))
PY
    echo "--- relevant processes ---"
    ps -eo pid,ppid,etime,stat,pcpu,pmem,wchan:32,cmd | grep -E 'phase294_dynamic_recompute_pilot|python tests/gpt5|nvidia-smi|cuda|ComfyUI|python main.py' | grep -v grep || true
...
(
  while true; do
    ...
    ps -eo pid,etime,stat,pcpu,pmem,wchan:28,cmd | grep -E 'phase294_dynamic_recompute_pilot|python tests/gpt5|nvidia-smi|cuda|python main.py' | grep -v grep || true
    sleep 10
  done
...
PYTHONUNBUFFERED=1 timeout --foreground --kill-after=30s "${MAX_SECONDS}s" \
  python tests/gpt5/phase294_dynamic_recompute_pilot.py "$MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --hard-exit-after-model \
    "$@"
RC="$?"
set -e
echo "phase294_exit_code=${RC}"
