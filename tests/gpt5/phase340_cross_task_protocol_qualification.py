#!/usr/bin/env python3
"""Run baseline-only Phase340 protocol qualification on one CUDA model."""
...
def base(case: dict[str, Any], model: str) -> dict[str, Any]:
    return {
        ...
        "condition": "baseline", "internal_intervention": False,
    }


def run_model(model: str, round_name: str = ROUND_DEFAULT, batch_size: int = 6) -> dict[str, Any]:
    root = OUT / round_name
    ...
    try:
        ...
        for start in range(0, len(cases), batch_size):
            ...
            scores = score_cases(loaded, batch, None, None)
            generated = generate_batch(loaded, batch, None, None, 16)
            for case, score, rollout in zip(batch, scores, generated, strict=True):
        ...
        write_jsonl(model_root / "phase340_phrase_rows.jsonl", phrase_rows)
        write_jsonl(model_root / "phase340_rollout_rows.jsonl", rollout_rows)
        complete = {
            "schema_version": SCHEMA_VERSION, "phase_id": PHASE, "created_at": now(),
            ...
            "valid": len(cases) == 324 and len(phrase_rows) == 324 and len(rollout_rows) == 324,
        }
...
def main() -> None:
    ...
    print(json.dumps(run_model(args.model, args.round, args.batch_size), ensure_ascii=False, indent=2))
