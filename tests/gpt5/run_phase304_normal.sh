...
MODEL="${1:?usage: $0 MODEL [phase304 args...]}"
shift || true

export OPENONE_NORMAL_ENV="${OPENONE_NORMAL_ENV:-openone-cu130-py312}"
export OUTPUT_DIR="${OUTPUT_DIR:-results/gpt5_phase304_role_query_template_calibration}"
...
RUN_ID="$(date +%Y%m%d_%H%M%S)_phase304normal_${MODEL}"
...
echo "=== Phase304 normal logged run ==="
date '+%Y-%m-%d %H:%M:%S %Z'
echo "run_id=${RUN_ID}"
echo "model=${MODEL}"
...
if [[ "$ENABLE_GPU_MONITOR" == "1" ]]; then
  ...
  (
    while true; do
      ...
      ps -eo pid,etime,stat,pcpu,pmem,wchan:28,cmd | grep -E 'phase304_role_query_template_calibration|python tests/gpt5|nvidia-smi|cuda|python main.py' | grep -v grep || true
      sleep 10
    done
...
echo "=== Run Phase304 normal ==="
set +e
PYTHONUNBUFFERED=1 timeout --foreground --kill-after=30s "${MAX_SECONDS}s" \
  python tests/gpt5/phase304_role_query_template_calibration.py "$MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --hard-exit-after-model \
    "$@"
RC="$?"
set -e
echo "phase304_exit_code=${RC}"
