...
def add_table(lines: list[str], title: str, entries: list[dict[str, Any]], limit: int = 140) -> None:
    lines += [
        f"### {title}",
        "",
...
def main() -> None:
    ...
    lines = ["# Phase82 Natural Frame Component Ablation Summary", ""]
    for model in MODELS:
        path = input_dir / f"{model}_phase82_natural_frame_component_ablation.json"
        lines += [f"## {model}", ""]
        if not path.exists():
            lines += ["missing", ""]
            continue
        ...
        sm = data.get("summary", {})
        by_condition = sorted_entries(sm.get("by_condition", {}), 180)
        by_condition_path = sorted_entries(sm.get("by_condition_path", {}), 240)
        ...
        lines += [
            ...
            f"module={data.get('module')}, contrast_rank={data.get('contrast_rank')}, component_rank={data.get('component_rank')}, relations={data.get('relations')}",
            "",
        ]
        add_table(lines, "By condition", by_condition, 180)
        add_table(lines, "Top condition paths", by_condition_path, 180)
        add_table(lines, "Top condition relations", by_condition_relation, 240)
    out_json = output_dir / "phase82_natural_frame_component_ablation_summary.json"
    out_md = output_dir / "PHASE82_NATURAL_FRAME_COMPONENT_ABLATION_SUMMARY.md"
