#!/usr/bin/env python3
"""Run Phase558 fixed-identity color behavior on one CUDA model at a time."""
...
def verify_protocol() -> dict[str, Any]:
    ...
    if not audit["valid"] or audit["status"] != "static_pass_no_model_run":
        raise RuntimeError("Phase558 static protocol did not pass")
    if protocol["open_cases_sha256"] != sha256_file(CASES_PATH):
        raise RuntimeError("Phase558 open case bank drift")
    if protocol["evidence_policy"]["sealed_split_read"]:
        raise RuntimeError("Phase558 behavior must not read sealed rows")
    return protocol
...
def rows_path(model: str) -> Path:
    return OUT_DIR / f"phase558_{model}_behavior_rows.jsonl"


def contract_path(model: str) -> Path:
    return OUT_DIR / f"phase558_{model}_behavior_run_contract.json"


def summary_path(model: str) -> Path:
    return OUT_DIR / f"phase558_{model}_behavior_execution_summary.json"


def prepare(model: str, restart: bool) -> tuple[set[str], Path]:
    output = rows_path(model)
    contract = {
        "schema_version": "phase558_behavior_run_contract.v1",
        "created_at": now(),
        "model": model,
    ...
    if contract_path(model).exists():
        ...
        for key in (
            ...
            if existing[key] != contract[key]:
                raise RuntimeError(f"Phase558 resume contract drift: {model}/{key}")
    else:
...
def run(model_name: str, batch_size: int, max_new_tokens: int, restart: bool) -> Path:
    ...
    if len(all_rows) != EXPECTED_MODEL_ROWS or any(row["sealed"] for row in all_rows):
        raise RuntimeError(f"Unexpected Phase558 open denominator for {model_name}")
    ...
    try:
        if not torch.cuda.is_available():
            raise RuntimeError("Phase558 behavior requires CUDA")
        ...
        if run_dtype != "torch.bfloat16" or quantized_8bit:
            raise RuntimeError(
                f"Phase558 precision drift: dtype={run_dtype}, 8bit={quantized_8bit}"
            )
        ...
        for start in range(0, len(pending), batch_size):
            ...
            if start == 0 or done == len(all_rows) or (start // batch_size) % 25 == 24:
                print(
                    f"[{time.strftime('%H:%M:%S')}] {model_name} Phase558 "
                    f"{done}/{len(all_rows)}",
                    flush=True,
                )
        final_rows = read_jsonl(output)
        if len(final_rows) != len(all_rows) or len({row["case_id"] for row in final_rows}) != len(all_rows):
            raise RuntimeError(f"Incomplete Phase558 behavior for {model_name}")
        payload = {
            "schema_version": "phase558_behavior_execution_summary.v1",
            "phase_id": "Phase558",
            "created_at": now(),
            "status": "complete",
            "model": model_name,
