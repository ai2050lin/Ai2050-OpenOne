#!/usr/bin/env python3
"""
Phase 133: true-last value contribution head-effect ranking.

Rank heads by the causal effect of removing their all-pre-answer value
contribution, then test cumulative top-k causal head aggregates.
"""
...
def summarize_from_delta(score_delta: np.ndarray, proj_delta: np.ndarray, target_idx: int, categories: list[str]) -> dict[str, Any]:
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        for ci, cat in enumerate(test_categories, 1):
            ...
            all_heads_patched = run_source_condition(
                model, tokenizer, device, layers, prompts, cat_local_ids, categories,
                args.batch_size, args.max_length, last_layer, monitor_basis, last_layer,
            ...
            aggregates.append({
                ...
                **summarize_condition(all_heads_patched, baseline, target_idx, categories),
            })
            cat_out = {
...
def write_markdown(result: dict[str, Any], path: Path) -> None:
    lines = [f"# Phase 133 Value Head Effect Ranking: {result['model']}", ""]
    ...
    for cat, item in result["category_results"].items():
        ...
        lines.append(
            ...
            f"{_fmt(by_mode.get('top_causal_1'))} | {_fmt(by_mode.get('top_causal_2'))} | "
            f"{_fmt(by_mode.get('top_causal_4'))} | {_fmt(by_mode.get('top_causal_8'))} | "
            f"{_fmt(by_mode.get('all_heads'))} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    json_path = out_dir / f"phase133_{args.model}_value_head_effect_ranking.json"
    md_path = out_dir / f"phase133_{args.model}_value_head_effect_ranking.md"
