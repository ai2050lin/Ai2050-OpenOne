...
def load_one(path: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "value_position": data.get("value_position"),
        "choice_layer": data.get("choice_layer"),
        "choice_heads": data.get("choice_heads"),
        "choice_position": data.get("choice_position"),
        "by_condition": by_condition,
        "by_slot": group(rows, ["slot"]),
        "strongest_value_conditions": [{"key": k, **v} for k, v in sorted(by_condition.items(), key=lambda kv: kv[1]["value_delta"])],
        "strongest_letter_conditions": [{"key": k, **v} for k, v in sorted(by_condition.items(), key=lambda kv: kv[1]["letter_delta"])],
    }
...
def main() -> None:
    ...
    paths = sorted(out_dir.glob("*_phase102_value_factor_bridge_decomposition.json"))
    summary = {"phase": 102, "output_dir": str(out_dir), "models": [load_one(path) for path in paths]}
    ...
    out_path = out_dir / "phase102_value_factor_bridge_decomposition_summary.json"
