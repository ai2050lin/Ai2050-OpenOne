#!/usr/bin/env python3
"""Collect Phase405 natural future next-token responses."""
...
SOURCE = OUT / "protocol/private/phase405_all_cases.jsonl"
...
def run(model: str, split: str) -> dict[str, Any]:
    ...
    if not cases:
        payload = {
            "schema_version": "79.1.0",
            "phase_id": "Phase405-NaturalFutureCollection",
            "created_at": now(),
            "model": model,
            "split": split,
            "authorized_families": [],
    ...
    try:
        ...
        if runtime_dtype != FROZEN_DTYPES[model]:
            raise RuntimeError(f"Phase405 dtype mismatch for {model}: {runtime_dtype}")
        ...
        for index, case in enumerate(cases, 1):
            ...
            if int(encoded["attention_mask"].sum()) != int(
                ...
                raise RuntimeError("Phase405 formal batch=1 unexpectedly padded")
            encoded = {
            ...
            if candidate_logits_valid:
                ...
                ) - max(float(candidate_logits[position]) for position in distractors)
            ...
            global_logits_valid = bool(torch.isfinite(logits).all())
            if global_logits_valid:
                global_top_id: int | None = int(torch.argmax(logits).item())
                global_top_text: str | None = tokenizer.decode([global_top_id])
                global_top_is_target = global_top_id == int(
                    case["target_token_id_private"]
                )
                global_top_in_candidate_set = global_top_id in token_ids
            else:
                global_top_id = None
                global_top_text = None
                global_top_is_target = False
                global_top_in_candidate_set = False
            ...
            rows.append(
                {
                    ...
                    "global_logits_valid": global_logits_valid,
                    "global_top_token_id_private": global_top_id,
                    "global_top_token_text_private": global_top_text,
                    "global_top_is_target_token": global_top_is_target,
                    "global_top_in_candidate_set": global_top_in_candidate_set,
            ...
            if index % 128 == 0 or index == len(cases):
                print(
                    f"[{model}/phase405/{split}] {index}/{len(cases)} "
                    f"candidate_correct={sum(row['finite_candidate_correct'] for row in rows)} "
                    f"natural_top={sum(row['global_top_is_target_token'] for row in rows)}",
                    flush=True,
                )
            if index % 256 == 0:
                del output, logits
        ...
        family_candidate_correct = Counter(
            row["family_id"] for row in rows if row["finite_candidate_correct"]
        )
        family_natural_top = Counter(
            row["family_id"] for row in rows if row["global_top_is_target_token"]
        )
        payload = {
            "schema_version": "79.1.0",
            "phase_id": "Phase405-NaturalFutureCollection",
            ...
            "global_top_in_candidate_set_count": sum(
                row["global_top_in_candidate_set"] for row in rows
            ),
            ...
            "nonfinite_global_logit_case_count": sum(
                not row["global_logits_valid"] for row in rows
            ),
            "families": [
                {
                    "family_id": family,
                    "case_count": family_counts[family],
                    "finite_candidate_correct_count": family_candidate_correct[
                        family
                    ],
                    "global_top_is_target_count": family_natural_top[family],
                }
                for family in families
            ],
            "valid": all(row["formal_single_case_unpadded"] for row in rows),
            "claim_boundary": {
                "candidate_panel_is_full_future_distribution": False,
                "natural_top_correct_is_internal_mechanism": False,
            },
        }
        write_jsonl(
