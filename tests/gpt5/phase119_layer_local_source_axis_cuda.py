#!/usr/bin/env python3
"""
Phase 119: layer-local source axis discovery.

Discover local category subspaces/axes separately for each layer and source
site, then causally patch those local axes at the same layer/site.
"""
...
OUT_ROOT = Path("results/gpt5_phase119_layer_local_source_axis")
...
def post_object_positions(obj_span: list[int], answer_pos: int) -> list[int]:
    start = min(max(obj_span) + 1, answer_pos)
    return list(range(start, answer_pos + 1)) or [answer_pos]
...
def item_positions(tokenizer: Any, item: dict[str, Any], answer_pos: int) -> dict[str, list[int]]:
    ...
    post = post_object_positions(span, answer_pos)
    return {
        "object_last": [obj_last],
        "object_span_mean": span,
        "post_object_mean": post,
        "answer_last": [answer_pos],
    }


def make_site_subspace_hook(
    basis: torch.Tensor,
        ...
        for ci, cat in enumerate(test_categories, 1):
            ...
            for layer_id in patch_layers:
                for site in sites:
                    ...
                    seed = 19000 + target_idx * 997 + layer_id * 31 + sites.index(site)
                    ...
                    for axis_type, patch_basis in axes.items():
                        patched = run_patch_condition(
                            model, tokenizer, device, layers, prompts, cat_local_ids, categories,
...
def write_markdown(result: dict[str, Any], path: Path) -> None:
    lines = [f"# Phase 119 Layer-local Source Axis Discovery: {result['model']}", ""]
    lines.append(f"Generated: {result['timestamp']}")
    lines.append(f"Layers: {result['patch_layers']}; sites: {result['sites']}")
    lines.append("")
    lines.append("| category | axis | best local source | best answer_last |")
    lines.append("|---|---|---|---|")
    for cat, item in result["category_results"].items():
        for axis in result["axis_types"]:
            ...
            source = [c for c in conds if c["site"] != "answer_last"]
            answer = [c for c in conds if c["site"] == "answer_last"]
            ...
            lines.append(f"| {cat} | {axis} | {fmt(source)} | {fmt(answer)} |")
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--sites", default="object_last,object_span_mean,post_object_mean,answer_last")
    ...
    json_path = out_dir / f"phase119_{args.model}_layer_local_source_axis.json"
    md_path = out_dir / f"phase119_{args.model}_layer_local_source_axis.md"
