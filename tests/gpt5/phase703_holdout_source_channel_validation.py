...
Phase 702 showed that source-restricted positive channel ensembles can reproduce
the Phase700 source-combo effect, but channel ranking was learned and evaluated
on the same paired cases.

Phase 703 cross-validates the channel ranking:
  - split paired cases into two folds;
  - rank source-restricted channels on one fold;
  - patch only the opposite holdout fold;
  - compare ranked channels with random same-count channels.
"""
...
OUT_ROOT = Path("results/glm5_phase703_holdout_source_channel_validation")
...
def run_model(args) -> dict[str, Any]:
    ...
    try:
        ...
        fold_a = paired_ids[::2]
        fold_b = paired_ids[1::2]
        fold_specs = [("A_to_B", fold_a, fold_b), ("B_to_A", fold_b, fold_a)]

        for fold_idx, (split_name, train_ids, test_ids) in enumerate(fold_specs):
            ...
            for case_id in train_ids:
                train_score_rows.extend(case_score_rows[case_id])
            channel_scores = build_channel_scores(train_score_rows, layer_meta)
            conditions = condition_channel_sets(channel_scores, counts, args.seed + fold_idx)
            fold_summaries.append({
                "split": split_name,
                "train_n": len(train_ids),
                "test_n": len(test_ids),
            ...
            for idx, case_id in enumerate(test_ids, 1):
                ...
                if idx % args.log_every == 0 or idx == len(test_ids):
                    log(f"{args.model}: {split_name} holdout patched {idx}/{len(test_ids)} test cases")
    finally:
        release_model(model)
        del tokenizer
        gc.collect()
    ...
    summary = summarize_model(args.model, rows, fold_summaries, paired_ids)
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    (OUT_ROOT / f"phase703_{args.model}_holdout_rows.jsonl").write_text(
        "\n".join(json.dumps(row, ensure_ascii=False, sort_keys=True) for row in rows) + "\n",
        encoding="utf-8",
    )
    payload = {
        "phase": 703,
        "title": "Holdout Source-Restricted Channel Validation",
    ...
    (OUT_ROOT / f"phase703_{args.model}_holdout_summary.json").write_text(
...
def write_cross_summary() -> dict[str, Any]:
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    models = []
    for path in sorted(OUT_ROOT.glob("phase703_*_holdout_summary.json")):
        models.append(json.loads(path.read_text(encoding="utf-8")))
    payload = {
        "phase": 703,
        "title": "Holdout Source-Restricted Channel Validation Cross-Model Summary",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "models": models,
    }
    (OUT_ROOT / "phase703_cross_model_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    lines = [
    ...
    for section, key in [("Best Restore", "best_restore_conditions"), ("Best Degradation", "best_degradation_conditions")]:
        ...
        for item in models:
            ...
            lines.append("")
    (OUT_ROOT / "phase703_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True), flush=True)
    return payload
