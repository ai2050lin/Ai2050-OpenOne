...
Phase 704 showed that unrelated donors can also restore many target cases,
so cross-case donor patching cannot be treated as pure semantic value transfer.
Phase 705 measures the competition field directly: target value, donor value,
prose route, and continuation route after each donor patch.
"""
...
OUT_ROOT = Path("results/glm5_phase705_target_donor_identity_competition_audit")
...
def run_delta_patched(
    ...
    try:
        ...
        diag = classify(out.logits[0, -1].detach(), routes, expected_ids)
        diag.update(identity_competition_diag(out.logits[0, -1].detach(), routes, expected_ids, donor_expected_ids))
    finally:
        for handle in handles:
            handle.remove()
    final = final_box.get("final")
...
def run_model(args) -> dict[str, Any]:
    ...
    try:
        ...
        for idx, pair in enumerate(donor_pairs, 1):
            ...
            donor_expected_ids = donor["expected_ids"]
    ...
    (OUT_ROOT / f"phase705_{args.model}_identity_competition_rows.jsonl").write_text(
        "\n".join(json.dumps(row, ensure_ascii=False, sort_keys=True) for row in rows) + "\n",
        encoding="utf-8",
    )
    payload = {
        "phase": 705,
        "title": "Target-vs-Donor Value Identity Competition Audit",
    ...
    (OUT_ROOT / f"phase705_{args.model}_identity_competition_summary.json").write_text(
...
def write_cross_summary() -> dict[str, Any]:
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    models = []
    for path in sorted(OUT_ROOT.glob("phase705_*_identity_competition_summary.json")):
        models.append(json.loads(path.read_text(encoding="utf-8")))
    payload = {
        "phase": 705,
        "title": "Target-vs-Donor Value Identity Competition Audit Cross-Model Summary",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "models": models,
    }
    (OUT_ROOT / "phase705_cross_model_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    lines = [
    ...
    for section, key in [("Best Restore", "best_restore_conditions"), ("Best Degradation", "best_degradation_conditions")]:
        ...
        for item in models:
            ...
            lines.append("")
    (OUT_ROOT / "phase705_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True), flush=True)
    return payload
