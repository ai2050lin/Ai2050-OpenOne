#!/usr/bin/env python3
"""
Phase 144: dirty-clean contrast suppressor and container re-audit.

Derive a local suppressor direction from dirty-vs-clean support restores and
re-audit container with wider scales including Phase141-like input_answer tests.
"""
...
OUT_ROOT = Path("results/gpt5_phase144_dirty_clean_contrast_container")
...
def run_condition(
    ...
    for start in range(0, len(prompts), batch_size):
        ...
        def layer_pre_hook(_module: Any, inputs: tuple[Any, ...]):
            ...
            if mode in {"support", "joint", "naive_joint", "empirical_joint", "contrast_joint"}:
        ...
        if mode in {"support", "suppressor", "joint", "naive_joint", "empirical_joint", "contrast_joint"}:
            def apply_site(x: torch.Tensor) -> torch.Tensor:
                ...
                if mode in {"suppressor", "joint", "naive_joint", "empirical_joint", "contrast_joint"} and comp_b is not None:
        ...
        if "answer_vec" not in locals():
            answer_vec = []
        answer_vec.append(ans.detach().cpu().numpy().astype(np.float32))
        del out, batch
        torch.cuda.empty_cache()
    return {
        ...
        "answer_vec": np.concatenate(answer_vec, axis=0),
    }


def contrast_basis_from_outputs(clean_out: dict[str, np.ndarray], dirty_out: dict[str, np.ndarray]) -> np.ndarray:
    diff = dirty_out["answer_vec"].mean(axis=0) - clean_out["answer_vec"].mean(axis=0)
    norm = float(np.linalg.norm(diff))
    if norm < 1e-8:
        return diff.reshape(1, -1).astype(np.float32)
    return (diff / norm).reshape(1, -1).astype(np.float32)
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        result: dict[str, Any] = {
            ...
            "contrast_suppress_scales": parse_float_list(args.contrast_suppress_scales),
        ...
        for offset in layer_offsets:
            ...
            for cat in test_categories:
                ...
                # Keep every answer-side basis available so empirical_joint can
                # suppress the competitor actually released by support-only.
            for c2 in categories:
                if c2 not in cache:
                    pre_basis2, pre_sv2 = svd_basis(build_category_contrast_matrix(pre_centers, categories, c2), args.rank)
                    ans_basis2, ans_sv2 = svd_basis(build_category_contrast_matrix(ans_centers, categories, c2), args.rank)
                    cat_train2 = [r for r in train_records if r["cat"] == c2]
                    x_train2 = project_np(np.stack([r["pre_vec"] for r in cat_train2]), pre_basis2)
                    y_train2 = project_np(np.stack([r["answer_vec"] for r in cat_train2]), ans_basis2)
                    cache[c2] = {
                        "pre_basis": pre_basis2,
                        "ans_basis": ans_basis2,
                        "comp_basis": ans_basis2,
                        "transfer": ridge_map(x_train2, y_train2, args.ridge),
                        "pre_sv": pre_sv2,
                        "ans_sv": ans_sv2,
                    }
            ...
            for ci, cat in enumerate(test_categories, 1):
                ...
                rows = []
                support_outputs: dict[tuple[str, float], dict[str, np.ndarray]] = {}
                support_rows_by_setting: dict[tuple[str, float], dict[str, Any]] = {}
                for site in sites:
                    for scale in scales:
                        patched = run_condition(
                            model, tokenizer, device, layers, prompts, cat_local_ids, categories,
                            args.batch_size, args.max_length, layer_id, "support", site,
                            cc["pre_basis"], cc["ans_basis"], cc["transfer"], cc["comp_basis"],
                        ...
                        row = {
                            ...
                            **summarize_condition(patched, clean, target_idx, categories),
                            ...
                            "suppressor_source": DEFAULT_COMPETITORS.get(cat, "animal"),
                        }
                        ...
                        support_rows_by_setting[(site, scale)] = row
                        support_outputs[(site, scale)] = patched
                        rows.append(row)

                contrast_scales = parse_float_list(args.contrast_suppress_scales)
                for site in sites:
                    site_rows = [r for r in rows if r["mode"] == "support" and r["site"] == site]
                    clean_candidates = [r for r in site_rows if r["is_constrained_clean"]]
                    if clean_candidates:
                        clean_row = max(clean_candidates, key=lambda r: (r["recovery_ratio"], -r["max_other_delta"]))
                    else:
                        clean_row = max(site_rows, key=lambda r: (r["recovery_ratio"] - max(0.0, r["max_other_delta"] - args.release_threshold), -r["max_other_delta"]))
                    dirty_candidates = [
                        r for r in site_rows
                        if r["max_other_delta"] > args.release_threshold and r["recovery_ratio"] >= clean_row["recovery_ratio"]
                    ]
                    if not dirty_candidates:
                        dirty_candidates = [r for r in site_rows if r["max_other_delta"] > args.release_threshold]
                    if not dirty_candidates:
                        continue
                    dirty_row = max(dirty_candidates, key=lambda r: (r["recovery_ratio"], r["max_other_delta"]))
                    basis = contrast_basis_from_outputs(
                        support_outputs[(site, clean_row["scale"])],
                        support_outputs[(site, dirty_row["scale"])],
                    )
                    for suppress_scale in contrast_scales:
                        patched = run_condition(
                            model, tokenizer, device, layers, prompts, cat_local_ids, categories,
                            args.batch_size, args.max_length, layer_id, "contrast_joint", site,
                            cc["pre_basis"], cc["ans_basis"], cc["transfer"], basis,
                            args.remove_scale, dirty_row["scale"], suppress_scale,
                        )
                        row = {
                            "layer_id": layer_id,
                            "layer_offset": offset,
                            "site": site,
                            "scale": dirty_row["scale"],
                            "mode": "contrast_joint",
                            "contrast_clean_scale": clean_row["scale"],
                            "contrast_dirty_scale": dirty_row["scale"],
                            "contrast_suppress_scale": suppress_scale,
                            **summarize_condition(patched, clean, target_idx, categories),
                            "token_audit": token_summary(tokenizer, patched["first_token_ids"]),
                            "suppressor_source": "dirty_clean_answer_diff",
                        }
                        add_metrics(row, remove_summary["target_delta"], args.release_threshold)
                        row["top_competitor"] = max(
                            ((k, v) for k, v in row["mean_delta_by_category"].items() if k != cat),
                ...
                joint_modes = {"joint", "naive_joint", "empirical_joint", "contrast_joint"}
                best_joint = max([r for r in rows if r["mode"] in joint_modes], key=lambda x: (x["recovery_ratio"], -x["max_other_delta"])) if any(m in modes for m in joint_modes) else None
                contrast_rows = [r for r in rows if r["mode"] == "contrast_joint"]
                best_contrast = max(contrast_rows, key=lambda x: (x["recovery_ratio"], -x["max_other_delta"])) if contrast_rows else None
                ...
                result["category_results"][key] = {
                    ...
                    "best_contrast_joint": best_contrast,
                }
        return result
    finally:
...
def write_markdown(result: dict[str, Any], path: Path) -> None:
    lines = [f"# Phase 144 Dirty-Clean Contrast Container: {result['model']}", ""]
    ...
    lines.append("")
    lines.append("| category@layer | transfer | remove | clean count | best clean | best support | best contrast |")
    lines.append("|---|---|---|---|---|---|---|")
    for key, item in result["category_results"].items():
        ...
        lines.append(
            ...
            f"{_fmt(item['best_constrained_clean'])} | {_fmt(item['best_support'])} | "
            f"{_fmt(item.get('best_contrast_joint'))} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--restore-scales", default="0.25,0.5,0.75,1.0,1.25,1.5")
    parser.add_argument("--restore-sites", default="input_answer,attention_output,mlp_input")
    parser.add_argument("--modes", default="support,contrast_joint")
    parser.add_argument("--contrast-suppress-scales", default="0.25,0.5,1.0")
    ...
    json_path = out_dir / f"phase144_{args.model}_dirty_clean_contrast_container.json"
    md_path = out_dir / f"phase144_{args.model}_dirty_clean_contrast_container.md"
