...
RESULT_ROOT = Path("tests/result/phase223_channel_activation_gate_validation")


SPECS = {
    "qwen3": [
        {"spec_id": "qwen3_explain_l29_l31_activation_gate", "pattern_id": "answer_explain", "layers": [29, 31]},
    ],
    "glm4": [
        {"spec_id": "glm4_repeat_l30_activation_gate", "pattern_id": "answer_repeat", "layers": [30]},
    ],
    "deepseek7b": [
        {"spec_id": "deepseek7b_explain_l24_activation_gate", "pattern_id": "answer_explain", "layers": [24]},
    ],
}
...
def activation_stat_rows(
    ...
    for row in channel_rows:
        grouped[(int(row["step"]), int(row["layer_idx"]), str(row["channel_kind"]))].append(row)
    for (step, layer_idx, kind), items in grouped.items():
        for row in sorted(items, key=lambda r: int(r["rank"]))[: int(top_n)]:
            c = int(row["channel_id"])
            s_vec = success_z.get(step, {}).get(layer_idx)
            d_vec = drift_z.get(step, {}).get(layer_idx)
            if s_vec is None or d_vec is None:
                continue
            s = float(s_vec[c].item())
            d = float(d_vec[c].item())
            rows.append(
                {
                    "phase": PHASE,
                    "source_phase": SOURCE_PHASE,
                    "row_kind": "phase223_activation_stat_row",
                    "model": model_name,
                    ...
                    "success_z_mean": s,
                    "drift_z_mean": d,
                    "success_minus_drift": s - d,
                    "signed_channel_score": row.get("signed_channel_score"),
                    "down_col_dot_resid_unit": row.get("down_col_dot_resid_unit"),
                }
            )
    return rows


def install_clamp_hook(
    model,
    condition: str,
    step: int,
    ...
    if down_proj is None:
        return []
    s_vec = success_z.get(int(step), {}).get(int(layer_idx))
    d_vec = drift_z.get(int(step), {}).get(int(layer_idx))
    ...
    def hook(_module: Any, inputs: tuple[Any, ...]):
        ...
        if mode == "zero":
            ...
            if s_vec is not None:
                z[:, -1, idx] = s_vec[idx.cpu()].to(device=z.device, dtype=z.dtype)
        elif mode == "drift":
            if d_vec is not None:
                z[:, -1, idx] = d_vec[idx.cpu()].to(device=z.device, dtype=z.dtype)
        else:
...
def forward_logits_condition(
    ...
    handles = install_clamp_hook(model, condition, int(step), selected, success_z, drift_z)
...
def eval_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        for spec in SPECS[args.model]:
            ...
            for source_group, source_items, target in [
                ...
                for row in source_items:
                    ...
                    filter_rows.append(
                        {
                            "phase": PHASE,
                            "row_kind": "phase223_baseline_filter_row",
                            "model": args.model,
                            "spec_id": spec["spec_id"],
                            "pattern_id": spec["pattern_id"],
            ...
            stat_rows.extend(activation_stat_rows(args.model, spec, spec_channel_rows, success_z, drift_z, int(args.stat_top_n)))
            conditions = ["none"]
            for layer_idx in layers:
                for channel_kind in ["pos", "neg"]:
                    for k in [4, 16, 64]:
                        conditions.append(f"mlpchan_{channel_kind}_zero_L{int(layer_idx)}_K{k}")
                        conditions.append(f"mlpchan_{channel_kind}_success_L{int(layer_idx)}_K{k}")
                        conditions.append(f"mlpchan_{channel_kind}_drift_L{int(layer_idx)}_K{k}")
            ...
            for source_group, eval_rows in [("success_repro", eval_success), ("drift_repro", eval_drift)]:
                for row in eval_rows:
                    for condition in conditions:
                        result = generate_condition(
                            model, tokenizer, device, groups, row, condition, selected, success_z, drift_z, int(args.max_steps)
                        )
                        rollout_rows.append(
                            {
                                "phase": PHASE,
                                "source_phase": SOURCE_PHASE,
                                "row_kind": "phase223_activation_gate_rollout_row",
                                "model": args.model,
                                "spec_id": spec["spec_id"],
                                "pattern_id": spec["pattern_id"],
                                "source_group": source_group,
    ...
    payload = {
        "phase": PHASE,
        "source_phase": SOURCE_PHASE,
        "title": "Channel activation gate validation",
        "status": "complete",
        ...
        "activation_stat_rows": len(stat_rows),
        "effect_rows": effects,
    ...
    write_json(out_dir / f"phase223_{args.model}_summary.json", payload)
    write_jsonl(out_dir / f"phase223_{args.model}_filter_rows.jsonl", filter_rows)
    write_jsonl(out_dir / f"phase223_{args.model}_rollout_rows.jsonl", rollout_rows)
    write_jsonl(out_dir / f"phase223_{args.model}_summary_rows.jsonl", summary_rows)
    write_jsonl(out_dir / f"phase223_{args.model}_effect_rows.jsonl", effects)
    write_jsonl(out_dir / f"phase223_{args.model}_channel_score_rows.jsonl", channel_rows)
    write_jsonl(out_dir / f"phase223_{args.model}_activation_stat_rows.jsonl", stat_rows)
    print(
        json.dumps(
            {
                "phase": PHASE,
...
def summarize_round(round_name: str) -> dict[str, Any]:
    ...
    summaries = [read_json(out_dir / f"phase223_{model}_summary.json") for model in MODELS]
    summaries = [summary for summary in summaries if summary]
    effects = []
    stats = []
    for model in MODELS:
        effects.extend(p214.iter_jsonl(out_dir / f"phase223_{model}_effect_rows.jsonl") or [])
        stats.extend(p214.iter_jsonl(out_dir / f"phase223_{model}_activation_stat_rows.jsonl") or [])
    payload = {
        "phase": PHASE,
        "source_phase": SOURCE_PHASE,
        "title": "Cross-model channel activation gate validation",
        "status": "complete",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        ...
        "activation_stat_rows": sum(int(summary.get("activation_stat_rows") or 0) for summary in summaries),
        "total_damage_match_loss": sum(int(row.get("damage_match_loss") or 0) for row in effects),
        "total_repair_match_gain": sum(int(row.get("repair_match_gain") or 0) for row in effects),
        ...
        "top_activation_stat_rows": sorted(
            stats,
            key=lambda row: abs(float(row.get("signed_channel_score") or 0.0)),
            reverse=True,
        )[:80],
    }
    write_json(out_dir / "phase223_cross_model_summary.json", payload)
    lines = ["# Phase 223 channel activation gate validation", ""]
    for key in [
        "spec_count",
        "filter_rows",
    ...
    for row in payload["top_effect_rows"][:50]:
        lines.append(
    ...
    lines.extend(["", "## Top activation stats", ""])
    for row in payload["top_activation_stat_rows"][:40]:
        lines.append(
            f"- {row.get('spec_id')} L{row.get('layer_idx')} step={row.get('step')} {row.get('channel_kind')} "
            f"channel={row.get('channel_id')} rank={row.get('rank')} "
            f"success_z={row.get('success_z_mean')} drift_z={row.get('drift_z_mean')} "
            f"delta={row.get('success_minus_drift')} signed={row.get('signed_channel_score')}"
        )
    (out_dir / "phase223_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(
        json.dumps(
            {
                "phase": PHASE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase223 channel activation gate validation")
    parser.add_argument("--model", choices=MODELS)
    parser.add_argument("--summarize", action="store_true")
    parser.add_argument("--round-name", default="channel_activation_gate_validation")
    ...
    parser.add_argument("--stat-top-n", type=int, default=12)
