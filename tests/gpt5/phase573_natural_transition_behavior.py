#!/usr/bin/env python3
"""Run staged Phase573 natural-counterfactual behavior gates."""
...
def rows_path(model: str) -> Path:
    return OUT_DIR / f"phase573_{model}_behavior_rows.jsonl.gz"


def summary_path(model: str) -> Path:
    return OUT_DIR / f"phase573_{model}_behavior_summary.json"


def registry_path(model: str) -> Path:
    return OUT_DIR / f"phase573_{model}_behavior_registry.json"


def contract_path(model: str) -> Path:
    return OUT_DIR / f"phase573_{model}_behavior_contract.json"


def generate_batch(
    loaded: Any, model: str, rows: list[dict[str, Any]], repeat: str,
    max_new_tokens: int,
) -> list[dict[str, Any]]:
...
def stable_expected(
    ...
    return bool(
        ...
        and first["semantic_event"] == second["semantic_event"] == "target"
        and first["normalized_generated"] == second["normalized_generated"]
    )


def balanced_worlds(
    base_rows: dict[str, dict[str, Any]], ids: list[str], cap: int
) -> list[str]:
    banks: dict[tuple[str, str, str], deque[str]] = defaultdict(deque)
    for base_id in sorted(ids):
        row = base_rows[base_id]
        key = (
            row["factorial_cell"], row["target"],
            row["other_relation_target"],
        )
        banks[key].append(base_id)
    keys = sorted(banks)
    selected = []
    while keys and len(selected) < cap:
        ...
        for key in keys:
            if len(selected) >= cap:
                break
            if banks[key]:
                selected.append(banks[key].popleft())
            if banks[key]:
...
def run_rows(
    loaded: Any, model: str, cases: list[dict[str, Any]], repeats: tuple[str, ...],
    max_new_tokens: int, output_rows: list[dict[str, Any]], stage: str,
) -> None:
    ...
    for repeat in repeats:
        for start in range(0, len(cases), batch_size):
            ...
            if start == 0 or done == len(cases) or start // batch_size % 32 == 31:
                print(
                    f"[{time.strftime('%H:%M:%S')}] {model} Phase573 {stage}/{repeat} "
                    f"{done}/{len(cases)}",
                    flush=True,
                )
...
def run(model: str, max_new_tokens: int, restart: bool) -> Path:
    ...
    if not audit["valid"] or audit["open_cases_sha256"] != sha256_file(protocol.OPEN_CASES_PATH):
        raise RuntimeError("Phase573 frozen protocol/case drift")
    if restart:
        for path in (
            rows_path(model), summary_path(model), registry_path(model), contract_path(model)
        ):
            path.unlink(missing_ok=True)
    contract = {
        "schema_version": "phase573_behavior_contract.v1",
    ...
    if contract_path(model).exists():
        ...
        for key in (
            ...
            if existing[key] != contract[key]:
                raise RuntimeError(f"Phase573 behavior contract drift: {model}/{key}")
    else:
    ...
    base_rows = {
        row["base_case_id"]: row for row in case_bank.values() if row["variant"] == "base"
    }
    loaded = None
    output_rows: list[dict[str, Any]] = []
    selected_for_controls: dict[str, list[str]] = {}
    final_selected: dict[str, list[str]] = {}
    relation_qualified_counts: dict[str, int] = {}
    all_axis_qualified_counts: dict[str, int] = {}
    ...
    try:
        ...
        if run_dtype != "torch.bfloat16":
            raise RuntimeError(f"Phase573 requires BF16, got {run_dtype}")
        ...
        for split in ("structure_discovery", "structure_confirmation"):
            ...
            run_rows(
                loaded, model, relation_cases, ("noop1", "noop2"),
                max_new_tokens, output_rows, f"{split}_relation",
            )
            by_case_repeat = {
        ...
        if structure_pass:
            for split in ("structure_discovery", "structure_confirmation"):
                ...
                run_rows(
                    loaded, model, controls, ("noop1", "noop2"), max_new_tokens,
                    output_rows, f"{split}_controls",
                )
                by_case_repeat = {
                ...
                all_axis_qualified_counts[split] = len(eligible)
                final_selected[split] = balanced_worlds(
                    base_rows, eligible, protocol.FINAL_WORLDS_PER_AXIS_SPLIT_MODEL
                )
                structure_pass = structure_pass and (
                    len(final_selected[split])
                    >= frozen["behavior_gate"][
        ...
        if structure_pass:
            ...
            heldout_pass = (
                len(final_selected[split])
                >= frozen["behavior_gate"]["minimum_all_axis_qualified_worlds_heldout"]
            )
        ...
        for values in repeats.values():
            if len(values) == 2:
                exact_mismatch += int(
                    values[0]["normalized_generated"] != values[1]["normalized_generated"]
                )
                semantic_mismatch += int(
                    values[0]["semantic_event"] != values[1]["semantic_event"]
                )
        trace_authorized = bool(structure_pass and heldout_pass)
        registry = {
            "schema_version": "phase573_behavior_registry.v1",
            "phase_id": protocol.PHASE,
            "created_at": now(),
            "model": model,
            "relation_qualified_counts_by_split": relation_qualified_counts,
            "all_axis_qualified_counts_by_split": all_axis_qualified_counts,
            ...
            "authorized_for_natural_trace": trace_authorized,
            "selection_uses_internal_trace": False,
            "sealed_split_read": False,
        }
        write_json(registry_path(model), registry)
        summary = {
            "schema_version": "phase573_behavior_summary.v1",
            "phase_id": protocol.PHASE,
            "created_at": now(),
            "status": "complete",
            "model": model,
