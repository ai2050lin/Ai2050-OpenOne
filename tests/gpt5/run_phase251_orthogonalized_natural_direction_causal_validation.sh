...
ROUND_NAME="${ROUND_NAME:-orthogonalized_natural_direction_causal_validation}"
MAX_CANDIDATES_PER_MODEL="${MAX_CANDIDATES_PER_MODEL:-10}"
MAX_ROLLOUT_TOKENS="${MAX_ROLLOUT_TOKENS:-8}"
PERTURB_SCALE="${PERTURB_SCALE:-0.35}"
ATTN_IMPLEMENTATIONS="${ATTN_IMPLEMENTATIONS:-flash_attention_2,sdpa,eager}"

for MODEL in qwen3 glm4 deepseek7b; do
  python tests/gpt5/phase251_orthogonalized_natural_direction_causal_validation.py \
    --model "${MODEL}" \
