...
def make_head_set_pre_hook(
    ...
    def pre_hook(_module: Any, inputs: Any):
        ...
        if condition == "zero_all":
            ...
            if not dpos:
                return inputs
            donor_block = donor[:, dpos, :]
            patched[:, tpos, :] = donor_block if len(dpos) == len(tpos) else donor_block.mean(dim=1, keepdim=True).expand(-1, len(tpos), -1)
        elif condition.startswith("zero_heads"):
            for start, end in spans:
                patched[:, tpos, start:end] = 0
        elif condition.startswith("keep_heads"):
            keep = torch.zeros(patched.shape[-1], dtype=torch.bool, device=patched.device)
            for start, end in spans:
                keep[start:end] = True
            patched[:, tpos, :] = patched[:, tpos, :].masked_fill(~keep.view(1, 1, -1), 0)
            ...
            if not dpos:
                return inputs
            for start, end in spans:
                donor_block = donor[:, dpos, start:end]
                patched[:, tpos, start:end] = (
                    donor_block if len(dpos) == len(tpos) else donor_block.mean(dim=1, keepdim=True).expand(-1, len(tpos), -1)
                )
        else:
        ...
        if condition != "clean":
            handles.append(
                layers[layer_idx].self_attn.o_proj.register_forward_pre_hook(
                    make_head_set_pre_hook(condition, head_indices, head_dim, target_positions, donor_input, donor_positions)
                )
            )
        with torch.no_grad():
...
def joint_score(
    ...
    value_prompt = item["prompt"]
    donor_value_prompt = donor_item["prompt"] if donor_item else None
    ...
    for value in candidates:
        value_scores[value] = fullseq_logprob_headset(
            model, tokenizer, device, layers, value_prompt, donor_value_prompt, value, max_length,
            layer_idx, condition, head_indices, head_dim, position_kind,
        )
    letter_scores = {}
    for letter in letters:
        letter_scores[letter] = fullseq_logprob_headset(
            model, tokenizer, device, layers, letter_prompt, donor_letter_prompt, letter, max_length,
            layer_idx, condition, head_indices, head_dim, position_kind,
        )
...
def build_conditions(head_sets: dict[str, list[int]]) -> list[tuple[str, str, list[int], bool]]:
    ...
    for name, heads in head_sets.items():
        ...
        out.append((f"keep_heads:{name}", name, heads, False))
...
def summarize(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for row in rows:
        groups["by_layer"][f"L{row['layer']}"].append(row)
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    model, tokenizer, device = load_model(args.model, os.environ.get("PHASE99_ATTN_IMPLEMENTATIONS", args.attn_implementations))
    ...
    for name, heads in head_sets.items():
        ...
        if bad:
            raise ValueError(f"{args.model} head set {name} has invalid heads {bad}; n_heads={n_heads}")
    ...
    final_path = out_dir / f"{args.model}_phase99_head_set_route_closure.json"
    partial_path = out_dir / f"{args.model}_phase99_head_set_route_closure.partial.json"
    results: dict[str, Any] = {
        "phase": 99,
        "model": args.model,
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"),
        "task": "head_set_route_closure",
        "layer": layer_idx,
        "n_heads": n_heads,
        "head_dim": head_dim,
        "head_sets": head_sets,
    ...
    if args.resume:
        ...
        if resume_path.exists():
            ...
            if loaded.get("phase") == 99 and loaded.get("model") == args.model:
                results = loaded
                results.setdefault("rows", [])
                results["summary"] = {}
                log(f"resume loaded {resume_path} rows={len(results['rows'])}")
    completed = {
        (int(r["item_idx"]), r["position_kind"], r["condition"], r["head_set_name"])
        for r in results["rows"]
    }
    clean_cache: dict[int, dict[str, Any]] = {}
    t0 = time.time()
    log(f"Phase99 model={args.model} layer=L{layer_idx} items={len(items)} positions={positions} head_sets={head_sets}")
