#!/usr/bin/env python3
"""
Phase 143: time interface validation and empirical suppressor redefinition.

Validate the DS7B time last-1 MLP interface, restore the missing container
input_answer site, and compare fixed versus empirically selected suppressors.
"""
...
OUT_ROOT = Path("results/gpt5_phase143_time_interface_empirical_suppressor")
...
def run_condition(
    ...
    for start in range(0, len(prompts), batch_size):
        ...
        def layer_pre_hook(_module: Any, inputs: tuple[Any, ...]):
            ...
            if mode in {"support", "joint", "naive_joint", "empirical_joint"}:
        ...
        if mode in {"support", "suppressor", "joint", "naive_joint", "empirical_joint"}:
            def apply_site(x: torch.Tensor) -> torch.Tensor:
                ...
                if mode in {"suppressor", "joint", "naive_joint", "empirical_joint"} and comp_b is not None:
        ...
        for offset in layer_offsets:
            ...
            for ci, cat in enumerate(test_categories, 1):
                ...
                for site in sites:
                    for scale in scales:
                        for mode in modes:
                            if mode == "empirical_joint":
                                support_row = support_rows_by_setting.get((site, scale))
                                empirical_comp = None
                                empirical_comp_basis = cc["comp_basis"]
                                if support_row is not None:
                                    empirical_comp = support_row["top_competitor"][0]
                                    if empirical_comp in cache:
                                        empirical_comp_basis = cache[empirical_comp]["ans_basis"]
                            else:
                                empirical_comp = DEFAULT_COMPETITORS.get(cat, "animal")
                                empirical_comp_basis = cc["comp_basis"]
                            patched = run_condition(
                                model, tokenizer, device, layers, prompts, cat_local_ids, categories,
                                args.batch_size, args.max_length, layer_id, mode, site,
                                cc["pre_basis"], cc["ans_basis"], cc["transfer"], empirical_comp_basis,
                            ...
                            row = {
                                ...
                                **summarize_condition(patched, clean, target_idx, categories),
                                ...
                                "suppressor_source": empirical_comp,
                            }
                ...
                joint_modes = {"joint", "naive_joint", "empirical_joint"}
                ...
                empirical_rows = [r for r in rows if r["mode"] == "empirical_joint"]
                best_empirical = max(empirical_rows, key=lambda x: (x["recovery_ratio"], -x["max_other_delta"])) if empirical_rows else None
                ...
                result["category_results"][key] = {
                    ...
                    "best_empirical_joint": best_empirical,
                }
        return result
    finally:
...
def write_markdown(result: dict[str, Any], path: Path) -> None:
    lines = [f"# Phase 143 Time Interface Empirical Suppressor: {result['model']}", ""]
    ...
    lines.append("")
    lines.append("| category@layer | transfer | remove | clean count | best clean | best support | best joint | best empirical |")
    lines.append("|---|---|---|---|---|---|---|---|")
    for key, item in result["category_results"].items():
        ...
        lines.append(
            ...
            f"{_fmt(item.get('best_empirical_joint'))} |"
        )
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    parser.add_argument("--restore-scales", default="0.2,0.25,0.3,0.35,0.4,0.45,0.5")
    parser.add_argument("--restore-sites", default="input_answer,attention_output,mlp_input")
    parser.add_argument("--modes", default="support,naive_joint,empirical_joint")
    ...
    json_path = out_dir / f"phase143_{args.model}_time_interface_empirical_suppressor.json"
    md_path = out_dir / f"phase143_{args.model}_time_interface_empirical_suppressor.md"
