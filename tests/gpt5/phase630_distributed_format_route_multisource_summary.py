#!/usr/bin/env python3
"""
Phase 630 summary: Distributed Format Route Multi-Source Sweep
"""
...
def load_results() -> list[dict]:
    return [
        ...
        for path in sorted(OUT_ROOT.glob("phase630_*_distributed_format_route_multisource_confirm.json"))
    ]
...
def main() -> None:
    ...
    baselines = ["base", "repair_prompt", "semantic_cumulative_only", "semantic_cumulative_random"]
    for data in results:
        modes = data.get("summary", {}).get("by_mode", {})
        best_exact = data.get("summary", {}).get("best_exact", [])[:24]
        best_tok0 = data.get("summary", {}).get("best_tok0", [])[:24]
        lines.extend([
            f"## {data['model']}",
            "",
        ...
        for item in best_exact:
            lines.append(row_line(item["mode"], item))
    ...
    out_path = OUT_ROOT / "phase630_cross_model_summary.md"
