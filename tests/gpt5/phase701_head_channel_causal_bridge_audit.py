...
Phase 699-700 showed that source contributions inside sparse answer_last heads
can recover most DS7B value readout. This phase lowers the resolution from
head-slot to head-channel:
  - score each channel inside Phase698 top heads by direct o_proj contribution;
  - patch only selected channels at answer_last o_proj input;
  - compare top channels with random same-count channels.

This is a bridge audit. A channel is not claimed to be a biological neuron; it
is a neuron-like unit defined as activation scalar times output vector.
"""
...
OUT_ROOT = Path("results/glm5_phase701_head_channel_causal_bridge_audit")
...
def capture_oproj_and_final(
    ...
    for li in scan_layers:
        o_proj, _n_heads, _head_dim = head_meta(model, li)
...
def score_channels_for_case(
    ...
    short_heads: dict[int, torch.Tensor],
    terse_heads: dict[int, torch.Tensor],
    direction: torch.Tensor,
) -> list[dict[str, Any]]:
    rows = []
    for li, heads in head_sets.items():
        if li not in short_heads or li not in terse_heads:
            continue
        o_proj, _n_heads, head_dim = layer_meta[li]
        delta = (terse_heads[li] - short_heads[li]).float()
        # torch.nn.Linear weight shape is [out_features, in_features].
        # Project every input channel's output vector onto the readout direction
        # in one matrix multiply for speed.
...
def make_channel_patch_sets(
    donor_heads: dict[int, torch.Tensor],
    ...
    return [
        {"layer": li, "indices": sorted(set(indices)), "new_full": donor_heads[li]}
        for li, indices in by_layer.items()
        if indices and li in donor_heads
    ]


def install_channel_patch_hooks(model, patches: list[dict[str, Any]], layer_meta: dict[int, tuple[Any, int, int]]):
    handles = []
    for patch in patches:
        li = int(patch["layer"])
        indices = sorted(set(int(i) for i in patch["indices"]))
        donor_full = patch["new_full"]
        o_proj, _n_heads, _head_dim = layer_meta[li]

        def pre_hook(_module, inputs, indices=indices, donor_full=donor_full):
            x = inputs[0]
            y = x.clone()
            donor = donor_full.to(device=y.device, dtype=y.dtype)
            for idx in indices:
                if 0 <= idx < y.shape[-1]:
                    y[0, -1, idx] = donor[idx]
...
def run_channel_patched(
    ...
    handles = install_channel_patch_hooks(model, patches, layer_meta)
    final_box: dict[str, torch.Tensor] = {}
    final_norm = get_final_norm(model)
    if final_norm is not None:
        def final_pre(_module, inputs):
...
def condition_channel_sets(
    ...
    for count in counts:
        top_rows = positive[: min(count, len(positive))]
        conditions[f"top_channel_{count}"] = top_rows
        random_pool = universe[:]
        rng.shuffle(random_pool)
        conditions[f"random_channel_{count}"] = random_pool[: min(count, len(random_pool))]
    conditions["all_positive_channels"] = positive
    return conditions
...
def run_model(args) -> dict[str, Any]:
    ...
    try:
        ...
        for idx, case_id in enumerate(paired_ids, 1):
            ...
            short_diag, short_heads, _short_final = capture_oproj_and_final(
                model, tokenizer, device, short_prompt, scan_layers, direction, routes, expected_ids
            )
            terse_diag, terse_heads, _terse_final = capture_oproj_and_final(
                model, tokenizer, device, terse_prompt, scan_layers, direction, routes, expected_ids
            )
            case_cache[case_id] = {
            ...
            for row in score_channels_for_case(layer_meta, head_sets, short_heads, terse_heads, direction):
        ...
        for (li, head, ch), vals in channel_acc.items():
            channel_scores.append({
                ...
                "mean_delta_value": sum(v["delta_value"] for v in vals) / len(vals),
        ...
        for idx, case_id in enumerate(paired_ids, 1):
            ...
            for cond_name, selected in conditions.items():
                for phase_kind, prompt, donor_key in [
                    ...
                    patches = make_channel_patch_sets(cur[donor_key], selected)
                    patched = run_channel_patched(
                        model,
                        tokenizer,
                        device,
                        prompt,
            ...
            if idx % args.log_every == 0 or idx == len(paired_ids):
                log(f"{args.model}: channel patched {idx}/{len(paired_ids)} paired cases")
    finally:
        release_model(model)
        del tokenizer
        gc.collect()
    ...
    (OUT_ROOT / f"phase701_{args.model}_channel_rows.jsonl").write_text(
        "\n".join(json.dumps(row, ensure_ascii=False, sort_keys=True) for row in rows) + "\n",
        encoding="utf-8",
    )
    payload = {
        "phase": 701,
        "title": "Head-Channel Causal Bridge Audit",
    ...
    (OUT_ROOT / f"phase701_{args.model}_channel_summary.json").write_text(
...
def write_cross_summary() -> dict[str, Any]:
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    models = []
    for path in sorted(OUT_ROOT.glob("phase701_*_channel_summary.json")):
        models.append(json.loads(path.read_text(encoding="utf-8")))
    payload = {
        "phase": 701,
        "title": "Head-Channel Causal Bridge Audit Cross-Model Summary",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "models": models,
    }
    (OUT_ROOT / "phase701_cross_model_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    lines = [
    ...
    for section, key in [("Best Restore", "best_restore_conditions"), ("Best Degradation", "best_degradation_conditions")]:
        ...
        for item in models:
            ...
            lines.append("")
    (OUT_ROOT / "phase701_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True), flush=True)
    return payload
