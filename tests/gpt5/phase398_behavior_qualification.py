#!/usr/bin/env python3
"""Run the frozen Phase398 joint-factorial behavior denominator."""
...
from phase398_joint_factorial_protocol import MODELS, SURFACES  # noqa: E402


OUT = ROOT / "tests/gpt5/result/phase398_joint_binding"
CASES = OUT / "protocol/private/phase398_candidate_cases.jsonl"
EXPECTED_PER_MODEL = 3 * 24 * 16
...
def run(model: str, max_new_tokens: int) -> dict[str, Any]:
    ...
    if len(cases) != EXPECTED_PER_MODEL:
        raise RuntimeError(f"Expected {EXPECTED_PER_MODEL} Phase398 cases for {model}, got {len(cases)}")
    loaded = None
    rows = []
    try:
        ...
        if runtime_dtype != FROZEN_DTYPE[model]:
            raise RuntimeError(f"Phase398 dtype mismatch for {model}: {runtime_dtype}")
        for index, case in enumerate(cases, 1):
            result = generate_batch(loaded, [case], max_new_tokens)[0]
            rows.append({
                "schema_version": "72.1.0", "phase_id": "Phase398-BehaviorQualification", "created_at": now(),
                "model": model, "runtime_dtype": runtime_dtype, "execution_batch_size": 1,
            ...
            if index % 48 == 0 or index == len(cases):
                print(f"[{model}] Phase398 behavior {index}/{len(cases)} strict={sum(row['strict_behavior_correct'] for row in rows)}", flush=True)
            if index % 96 == 0:
                gc.collect()
        ...
        summary = {
            "schema_version": "72.1.0", "phase_id": "Phase398-BehaviorQualification", "created_at": now(),
            "model": model, "runtime_dtype": runtime_dtype, "execution_batch_size": 1,
            "max_new_tokens": max_new_tokens, "case_count": len(rows),
            ...
            "surfaces": [
                {"task_surface": surface, "case_count": counts[surface], "strict_correct_count": correct[surface], "exact_answer_match_count": exact[surface]}
                for surface in SURFACES
            ],
            "valid": len(rows) == EXPECTED_PER_MODEL,
            "claim_boundary": {"factorial_behavior_is_joint_binding_state": False, "failed_groups_replaceable": False},
        }
