#!/usr/bin/env python3
"""Run one Phase552 eight-cell behavior bank on CUDA."""
...
def rows_path(model: str) -> Path:
    return OUT_DIR / f"phase552_{model}_behavior_rows.jsonl"


def summary_path(model: str) -> Path:
    return OUT_DIR / f"phase552_{model}_behavior_execution.json"


def verify() -> None:
    audit = read_json(AUDIT_PATH)
    protocol = read_json(PROTOCOL_PATH)
    if not audit["valid"] or protocol["registered_cases_sha256"] != sha256_file(CASES_PATH):
        raise RuntimeError("Phase552 static registry failed or drifted")
    if protocol["evidence_boundaries"]["new_sealed_split_read"]:
        raise RuntimeError("Phase552 cannot read a sealed split")


def prepare(model: str, restart: bool) -> set[str]:
    output = rows_path(model)
    contract_path = OUT_DIR / f"phase552_{model}_behavior_contract.json"
    frozen = {
        "schema_version": "phase552_behavior_contract.v1",
        "phase_id": "Phase552",
        "created_at": now(),
        "model": model,
    ...
    if contract_path.exists():
        ...
        for key in ("model", "cases_sha256", "protocol_sha256", "do_sample"):
            if old[key] != frozen[key]:
                raise RuntimeError(f"Phase552 resume drift: {model}/{key}")
    else:
    ...
    try:
        if pending:
            if not torch.cuda.is_available():
                raise RuntimeError("Phase552 behavior execution requires CUDA")
            loaded, tokenizer, device = load_model(model_name)
            ...
            for start in range(0, len(pending), batch_size):
                ...
                for index, source_row in enumerate(batch):
                    ...
                    rows_out.append(row)
                append_jsonl(rows_path(model_name), rows_out)
                ...
                if start == 0 or generated_count % (batch_size * 16) == 0 or generated_count == len(pending):
                    print(
                        f"[{time.strftime('%H:%M:%S')}] {model_name} Phase552 "
                        f"{len(completed) + generated_count}/{len(source)}",
                        flush=True,
                    )
        final_rows = read_jsonl(rows_path(model_name)) if rows_path(model_name).exists() else []
        if len(final_rows) != len(source) or len({row["case_id"] for row in final_rows}) != len(source):
            raise RuntimeError(f"Incomplete Phase552 behavior: {model_name}")
        payload = {
            "schema_version": "phase552_behavior_execution.v1",
            "phase_id": "Phase552",
            "created_at": now(),
            "status": "complete" if source else "skipped_no_registered_contract",
            "model": model_name,
            "registered_case_count": len(source),
            "completed_case_count": len(final_rows),
            "rows_path": str(rows_path(model_name).relative_to(ROOT)) if source else None,
            "rows_sha256": sha256_file(rows_path(model_name)) if source else None,
