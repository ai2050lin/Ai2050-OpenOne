...
Phase 703 showed that source-restricted channel ranking transfers to holdout
cases. Phase 704 asks a stricter question: can another case's source-combo
channel state repair the target case, especially when the donor has the same
value but a different object/relation?
"""
...
OUT_ROOT = Path("results/glm5_phase704_cross_case_channel_donor_validation")
...
def run_model(args) -> dict[str, Any]:
    ...
    (OUT_ROOT / f"phase704_{args.model}_cross_case_donor_rows.jsonl").write_text(
        "\n".join(json.dumps(row, ensure_ascii=False, sort_keys=True) for row in rows) + "\n",
        encoding="utf-8",
    )
    payload = {
        "phase": 704,
        "title": "Cross-Case Source-Restricted Channel Donor Validation",
    ...
    (OUT_ROOT / f"phase704_{args.model}_cross_case_donor_summary.json").write_text(
...
def write_cross_summary() -> dict[str, Any]:
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    models = []
    for path in sorted(OUT_ROOT.glob("phase704_*_cross_case_donor_summary.json")):
        models.append(json.loads(path.read_text(encoding="utf-8")))
    payload = {
        "phase": 704,
        "title": "Cross-Case Source-Restricted Channel Donor Validation Cross-Model Summary",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "models": models,
    }
    (OUT_ROOT / "phase704_cross_model_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    lines = [
    ...
    for section, key in [("Best Restore", "best_restore_conditions"), ("Best Degradation", "best_degradation_conditions")]:
        ...
        for item in models:
            ...
            lines.append("")
    (OUT_ROOT / "phase704_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True), flush=True)
    return payload
