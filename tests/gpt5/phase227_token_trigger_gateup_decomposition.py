...
RESULT_ROOT = Path("tests/result/phase227_token_trigger_gateup_decomposition")
...
def token_trigger_variants(tokenizer, prompt: str, pattern_id: str, limit: int) -> dict[str, str]:
    variants = prompt_variants(prompt, pattern_id)
    text = str(prompt)
    ids = tokenizer.encode(text, add_special_tokens=False)
    decoded = [tokenizer.decode([int(tok_id)], skip_special_tokens=False) for tok_id in ids]
    keywords = ["Answer", ":", "because", "same", "twice", "comma", "exactly", "short", "reason"]
    candidate_indices: list[int] = []
    for idx, piece in enumerate(decoded):
        if idx >= max(0, len(decoded) - 18) or any(key.lower() in piece.lower() for key in keywords):
            candidate_indices.append(idx)
    seen: set[int] = set()
    for idx in candidate_indices:
        if idx in seen or len([k for k in variants if k.startswith("drop_tok_")]) >= int(limit):
            continue
        seen.add(idx)
        new_ids = ids[:idx] + ids[idx + 1 :]
        piece = decoded[idx].replace("\n", "\\n")
        if not piece.strip():
            piece = f"id{ids[idx]}"
        name = f"drop_tok_{idx}_{piece[:16]}"
        variants[name] = tokenizer.decode(new_ids, skip_special_tokens=False)
    return variants


def get_mlp_parts(layer: Any) -> tuple[Any | None, Any | None, Any | None]:
    mlp = getattr(layer, "mlp", None)
    if mlp is None:
        return None, None, None
    return getattr(mlp, "gate_proj", None), getattr(mlp, "up_proj", None), getattr(mlp, "down_proj", None)


def layer_activation_fn(layer: Any):
    mlp = getattr(layer, "mlp", None)
    return getattr(mlp, "act_fn", None) if mlp is not None else None
...
def capture_mlp_internal_hidden_logits(
    model,
    tokenizer,
    device: torch.device,
    text: str,
    ...
    for layer_idx in source_layers:
        gate_proj, up_proj, down_proj = get_mlp_parts(layers[int(layer_idx)])
        if down_proj is None:
            continue

        def make_gate_hook(li: int):
            def hook(_module: Any, _inputs: tuple[Any, ...], output: Any):
                if torch.is_tensor(output):
                    captured[int(li)]["gate"] = output[0, -1, :].detach().float().cpu()
                return None

            return hook

        def make_up_hook(li: int):
            def hook(_module: Any, _inputs: tuple[Any, ...], output: Any):
                if torch.is_tensor(output):
                    captured[int(li)]["up"] = output[0, -1, :].detach().float().cpu()
                return None

            return hook

        def make_down_pre_hook(li: int):
        ...
        if gate_proj is not None:
            handles.append(gate_proj.register_forward_hook(make_gate_hook(int(layer_idx))))
        if up_proj is not None:
            handles.append(up_proj.register_forward_hook(make_up_hook(int(layer_idx))))
        handles.append(down_proj.register_forward_pre_hook(make_down_pre_hook(int(layer_idx))))
    ...
    for layer_idx in source_layers:
        ...
        gate = parts.get("gate")
        up = parts.get("up")
        if gate is not None and up is not None and "recomputed_product" not in parts:
            act_fn = layer_activation_fn(layers[int(layer_idx)])
            if act_fn is not None:
                parts["recomputed_product"] = act_fn(gate) * up
            else:
                parts["recomputed_product"] = F.silu(gate) * up
...
def mean_mlp_internal(
    ...
    for row in rows:
        for step in range(1, int(max_steps) + 1):
            internal, _hidden, _logits = capture_mlp_internal_hidden_logits(
                model,
                tokenizer,
                device,
...
def build_trigger_rows(
    ...
    for row in rows:
        variants = token_trigger_variants(tokenizer, str(row.get("prompt") or ""), str(spec["pattern_id"]), int(token_variant_limit))
        full_metrics_by_step: dict[int, dict[str, Any]] = {}
        for step in range(1, int(max_steps) + 1):
            full_internal, full_hidden, full_logits = capture_mlp_internal_hidden_logits(
                model,
                tokenizer,
                device,
            ...
            for variant_name, variant_prompt in variants.items():
                if variant_name == "full":
                    internal_map, hidden_map, logits = full_internal, full_hidden, full_logits
                else:
                    internal_map, hidden_map, logits = capture_mlp_internal_hidden_logits(
                        model,
                        tokenizer,
                        device,
                ...
                for layer_idx in source_layers:
                    part_map = internal_map.get(int(layer_idx), {})
                    s_map = success_internal.get(int(step), {}).get(int(layer_idx), {})
                    d_map = drift_internal.get(int(step), {}).get(int(layer_idx), {})
                    if not part_map or not s_map or not d_map:
                        continue
                    for component in ["gate", "up", "product", "recomputed_product"]:
                        z = part_map.get(component)
                        s = s_map.get(component)
                        d = d_map.get(component)
                            ...
                            activation_rows.append(
                                {
                                    ...
                                    "row_kind": "phase227_token_trigger_gateup_row",
                                    "model": model_name,
                    ...
                    hidden_rows.append(
                        {
                            "phase": PHASE,
                            "source_phase": SOURCE_PHASE,
                            "row_kind": "phase227_token_trigger_hidden_projection_row",
                            "model": model_name,
                            "spec_id": spec["spec_id"],
                            "pattern_id": spec["pattern_id"],
                            "source_group": source_group,
                ...
                readout_rows.append(
                    {
                        "phase": PHASE,
                        "source_phase": SOURCE_PHASE,
                        "row_kind": "phase227_token_trigger_readout_row",
                        "model": model_name,
                        "spec_id": spec["spec_id"],
                        "pattern_id": spec["pattern_id"],
                        "source_group": source_group,
...
def summarize_activation(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for key, items in buckets.items():
        spec_id, source_group, variant, step, layer, component, k = key
        if variant == "full":
            full_means[(spec_id, source_group, step, layer, component, k)] = sum(finite_float(x.get("activation_axis")) for x in items) / len(items)
    out = []
    for key, items in buckets.items():
        spec_id, source_group, variant, step, layer, component, k = key
        ...
        full_axis = full_means.get((spec_id, source_group, step, layer, component, k), mean_axis)
        out.append(
            {
                "spec_id": spec_id,
                "source_group": source_group,
...
def eval_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        for spec in SPECS[args.model]:
            ...
            filter_rows.append(
                {
                    "phase": PHASE,
                    "row_kind": "phase227_source_row_count",
                    "model": args.model,
                    "spec_id": spec["spec_id"],
                    "pattern_id": spec["pattern_id"],
            ...
            success_internal = mean_mlp_internal(model, tokenizer, device, success_rows, source_layers, observe_layers, int(args.max_steps))
            drift_internal = mean_mlp_internal(model, tokenizer, device, drift_rows, source_layers, observe_layers, int(args.max_steps))
    ...
    payload = {
        "phase": PHASE,
        "source_phase": SOURCE_PHASE,
        "title": "Token trigger and gate/up decomposition",
        "status": "complete",
    ...
    write_json(out_dir / f"phase227_{args.model}_summary.json", payload)
    write_jsonl(out_dir / f"phase227_{args.model}_filter_rows.jsonl", filter_rows)
    write_jsonl(out_dir / f"phase227_{args.model}_activation_rows.jsonl", activation_rows)
    write_jsonl(out_dir / f"phase227_{args.model}_hidden_rows.jsonl", hidden_rows)
    write_jsonl(out_dir / f"phase227_{args.model}_readout_rows.jsonl", readout_rows)
    write_jsonl(out_dir / f"phase227_{args.model}_channel_score_rows.jsonl", channel_rows)
    write_jsonl(out_dir / f"phase227_{args.model}_activation_summary_rows.jsonl", activation_summary)
    write_jsonl(out_dir / f"phase227_{args.model}_hidden_summary_rows.jsonl", hidden_summary)
    write_jsonl(out_dir / f"phase227_{args.model}_readout_summary_rows.jsonl", readout_summary)
    print(
        json.dumps(
            {
                "phase": PHASE,
...
def summarize_round(round_name: str) -> dict[str, Any]:
    ...
    summaries = [read_json(out_dir / f"phase227_{model}_summary.json") for model in MODELS]
    summaries = [x for x in summaries if x]
    activation_summary = []
    hidden_summary = []
    readout_summary = []
    for model in MODELS:
        activation_summary.extend(p214.iter_jsonl(out_dir / f"phase227_{model}_activation_summary_rows.jsonl") or [])
        hidden_summary.extend(p214.iter_jsonl(out_dir / f"phase227_{model}_hidden_summary_rows.jsonl") or [])
        readout_summary.extend(p214.iter_jsonl(out_dir / f"phase227_{model}_readout_summary_rows.jsonl") or [])
    payload = {
        "phase": PHASE,
        "source_phase": SOURCE_PHASE,
        "title": "Cross-model token trigger and gate/up decomposition",
        "status": "complete",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    write_json(out_dir / "phase227_cross_model_summary.json", payload)
    lines = ["# Phase 227 token trigger and gate/up decomposition", ""]
    for key in ["activation_rows", "hidden_rows", "readout_rows", "channel_score_rows"]:
        lines.append(f"{key}: {payload[key]}")
    lines.extend(["", "## Component summary", "", "| spec | group | variant | step | layer | component | K | axis | delta | success closer |", "| --- | --- | --- | ---: | ---: | --- | ---: | ---: | ---: | ---: |"])
    for row in payload["top_activation_summary"][:60]:
        lines.append(
            f"| {row.get('spec_id')} | {row.get('source_group')} | {row.get('variant')} | {row.get('step')} | {row.get('source_layer')} | {row.get('component')} | {row.get('k')} | "
    ...
    (out_dir / "phase227_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(
        json.dumps(
            {
                "phase": PHASE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase227 token trigger and gate/up decomposition")
    parser.add_argument("--model", choices=MODELS)
    parser.add_argument("--summarize", action="store_true")
    parser.add_argument("--round-name", default="token_trigger_gateup_decomposition")
    ...
    parser.add_argument("--token-variant-limit", type=int, default=12)
