...
MODEL="${1:?usage: $0 MODEL [phase301 args...]}"
shift || true
...
RUN_ID="$(date +%Y%m%d_%H%M%S)_phase301normal_${MODEL}"
...
echo "=== Phase301 normal logged run ==="
date '+%Y-%m-%d %H:%M:%S %Z'
echo "run_id=${RUN_ID}"
echo "model=${MODEL}"
...
(
  while true; do
    ...
    ps -eo pid,etime,stat,pcpu,pmem,wchan:28,cmd | grep -E 'phase301_passive_factor_closure|python tests/gpt5|nvidia-smi|cuda|python main.py' | grep -v grep || true
    sleep 10
  done
) >"$LOG_DIR/gpu_process_monitor.log" 2>&1 &
GPU_MONITOR_PID="$!"

echo "=== Run Phase301 normal ==="
set +e
PYTHONUNBUFFERED=1 timeout --foreground --kill-after=30s "${MAX_SECONDS}s" \
  python tests/gpt5/phase301_passive_factor_closure.py "$MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --hard-exit-after-model \
    "$@"
RC="$?"
set -e
echo "phase301_exit_code=${RC}"
