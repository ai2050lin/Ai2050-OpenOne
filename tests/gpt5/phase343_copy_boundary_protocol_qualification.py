#!/usr/bin/env python3
"""Run Phase343 baseline qualification on the single-case official path."""
...
from phase343_copy_boundary_protocol_case_bank import (  # noqa: E402
...
def run_model(model: str, round_name: str = ROUND_DEFAULT) -> dict[str, Any]:
    root = OUT / round_name
    cases = [row for row in read_jsonl(root / "phase343_registered_cases.jsonl") if row["model"] == model]
    ...
    try:
        ...
        write_jsonl(model_root / "phase343_phrase_rows.jsonl", phrase_rows)
        write_jsonl(model_root / "phase343_rollout_rows.jsonl", rollout_rows)
        complete = {
            "schema_version": SCHEMA_VERSION, "phase_id": PHASE, "created_at": now(),
