...
Phase 698 found a sparse answer_last top-head subset whose o_proj-input slots
restore/degrade DS7B value output. It also observed that these heads attend more
to record-line target_value in terse_no_explain, but that was not causal.

This phase tests source-token causality inside the selected head slots:
  - compute each source group's contribution to answer_last head slots from
    real attention weights and v_proj outputs;
  - patch only the source-contribution delta between short and terse;
  - erase source contributions in terse as a necessity control.
...
OUT_ROOT = Path("results/glm5_phase699_answer_last_head_source_token_causal_patch")
...
def make_source_delta_patches(
    ...
    patches = []
    for li, heads in head_sets.items():
        delta = src_state["contributions"][li][group_name] - dst_state["contributions"][li][group_name]
...
def make_erase_patches(state: dict[str, Any], head_sets: dict[int, list[int]], group_name: str, scale: float = 1.0) -> list[dict[str, Any]]:
    patches = []
    for li, heads in head_sets.items():
        delta = -scale * state["contributions"][li][group_name]
...
def row_from_diag(
    ...
    return {
        ...
        "source_group_len": group_lengths.get(group_name),
...
def run_model(args) -> dict[str, Any]:
    ...
    try:
        ...
        groups = [g for g in SOURCE_GROUPS if g in args.groups.split(",")] if args.groups else SOURCE_GROUPS
        ...
        for idx, case_id in enumerate(paired_ids, 1):
            ...
            for group_name in groups:
                # Source contribution delta transplant.
                patched = run_patched(
                    model, tokenizer, device, short_prompt, "source_delta",
                    make_source_delta_patches(terse_state, short_state, head_sets, group_name),
                    direction, routes, expected_ids, layer_meta,
                )
                rows.append(row_from_diag(
                    meta, case_id, "restore", f"delta_{group_name}", group_name,
                    short_diag, terse_diag, patched, short_state["group_lengths"],
                ))

                patched = run_patched(
                    model, tokenizer, device, terse_prompt, "source_delta",
                    make_source_delta_patches(short_state, terse_state, head_sets, group_name),
                    direction, routes, expected_ids, layer_meta,
                )
                rows.append(row_from_diag(
                    meta, case_id, "degradation", f"delta_{group_name}", group_name,
                    short_diag, terse_diag, patched, terse_state["group_lengths"],
                ))

                # Necessity control: erase the source contribution from terse.
                patched = run_patched(
                    model, tokenizer, device, terse_prompt, "source_delta",
                    make_erase_patches(terse_state, head_sets, group_name),
                    direction, routes, expected_ids, layer_meta,
                )
                rows.append(row_from_diag(
                    meta, case_id, "erase", f"erase_{group_name}", group_name,
                    short_diag, terse_diag, patched, terse_state["group_lengths"],
                ))

            if idx % args.log_every == 0 or idx == len(paired_ids):
                log(f"{args.model}: source causal patched {idx}/{len(paired_ids)} paired cases")
    finally:
        release_model(model)
        del tokenizer
        gc.collect()
    ...
    (OUT_ROOT / f"phase699_{args.model}_source_patch_rows.jsonl").write_text(
        "\n".join(json.dumps(r, ensure_ascii=False, sort_keys=True) for r in rows) + "\n",
        encoding="utf-8",
    )
    payload = {
        "phase": 699,
        "title": "Answer-Last Head Source-Token Causal Patch Audit",
    ...
    (OUT_ROOT / f"phase699_{args.model}_source_patch_summary.json").write_text(
...
def write_cross_summary() -> dict[str, Any]:
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    models = []
    for path in sorted(OUT_ROOT.glob("phase699_*_source_patch_summary.json")):
        models.append(json.loads(path.read_text(encoding="utf-8")))
    payload = {
        "phase": 699,
        "title": "Answer-Last Head Source-Token Causal Patch Audit Cross-Model Summary",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "models": models,
    }
    (OUT_ROOT / "phase699_cross_model_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    lines = [
    ...
    for section, key in [
        ...
        for item in models:
            ...
            lines.append("")
    (OUT_ROOT / "phase699_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True), flush=True)
    return payload
...
def main() -> None:
    ...
    parser.add_argument("--groups", default=",".join(SOURCE_GROUPS))
