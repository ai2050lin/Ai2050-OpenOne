#!/usr/bin/env python3
"""Run serial three-model Phase575 five-variant behavior qualification."""
...
def rows_path(model: str) -> Path:
    return OUT_DIR / f"phase575_{model}_behavior_rows.jsonl.gz"


def summary_path(model: str) -> Path:
    return OUT_DIR / f"phase575_{model}_behavior_summary.json"


def registry_path(model: str) -> Path:
    return OUT_DIR / f"phase575_{model}_behavior_registry.json"


def contract_path(model: str) -> Path:
    return OUT_DIR / f"phase575_{model}_behavior_contract.json"
...
def run(model: str, max_new_tokens: int, restart: bool) -> Path:
    ...
    if not audit["valid"] or audit["open_cases_sha256"] != sha256_file(
        ...
        raise RuntimeError("Phase575 frozen protocol or case drift")
    if not torch.cuda.is_available():
        raise RuntimeError("Phase575 model execution requires CUDA")
    if restart:
        for path in (
    ...
    contract = {
        "schema_version": "phase575_behavior_contract.v1",
    ...
    if contract_path(model).exists():
        ...
        for key, value in contract.items():
            if key != "created_at" and existing[key] != value:
                raise RuntimeError(f"Phase575 behavior contract drift: {model}/{key}")
    else:
    ...
    five_variant_counts: dict[str, int] = {}
    ...
    try:
        ...
        if run_dtype != "torch.bfloat16":
            raise RuntimeError(f"Phase575 requires BF16, got {run_dtype}")
        ...
        if structure_pass:
            for split in ("structure_discovery", "structure_confirmation"):
                ...
                five_variant_counts[split] = len(eligible)
                ...
                structure_pass = structure_pass and len(final_selected[split]) >= frozen[
                    "behavior_gate"
                ]["minimum_five_variant_qualified_worlds_each_structure_split"]
        ...
        if structure_pass:
            ...
            heldout_pass = len(final_selected[split]) >= frozen[
                "behavior_gate"
            ]["minimum_five_variant_qualified_worlds_heldout"]
        ...
        registry = {
            "schema_version": "phase575_behavior_registry.v1",
            "phase_id": protocol.PHASE,
            "created_at": now(),
            "model": model,
            "relation_qualified_counts_by_split": relation_counts,
            "five_variant_qualified_counts_by_split": five_variant_counts,
            ...
            "authorized_for_natural_ledger": authorized,
        ...
        summary = {
            "schema_version": "phase575_behavior_summary.v1",
            "phase_id": protocol.PHASE,
            "created_at": now(),
            "status": "complete",
            "model": model,
