#!/usr/bin/env python3
"""Freeze an external continuation observer for candidate-free Phase581 prompts."""
...
OUT_DIR = ROOT / "tests/gpt5/result/phase582_external_continuation"
PROTOCOL_PATH = OUT_DIR / "phase582_frozen_protocol.json"
...
def register() -> dict[str, Any]:
    ...
    if not source_audit["valid"]:
        raise RuntimeError("Phase582 requires valid Phase581 source")
    if source_protocol["open_cases_sha256"] != sha256_file(source.OPEN_CASES_PATH):
        raise RuntimeError("Phase582 source cases drift")
    payload = {
        "schema_version": "phase582_external_continuation_protocol.v1",
        "phase_id": PHASE,
        "created_at": now(),
        "title": "External category-continuation observer on candidate-free prompts",
        ...
        "candidate_continuations": {
            category: category for category in source.CATEGORY_ALIASES
        },
        "noop_repeats": list(NOOP_REPEATS),
        "fixed_batch_size": FIXED_BATCH_SIZE,
        "score_definition": {
            "sequence_score": "mean conditional log probability over continuation tokens",
            "margin": "target sequence score minus relation foil sequence score",
        ...
        "evidence_policy": {
            ...
            "external_observer_not_parametric_storage_localization": True,
            "observer_trace_may_be_collected_after_gate": True,
