...
def fullseq_logprob_bridge(
    ...
    if len(full_ids) > max_length:
        return float("-inf")
    value_op, choice_op = parse_condition(condition)
    ...
    if value_op == "value_transplant" and donor_full_ids is not None:
    ...
    handles = []
    try:
        if value_op in {"value_zero", "value_transplant"}:
            handles.append(
                get_module(layers[value_layer], module_name(value_component)).register_forward_hook(
                    make_value_hook(value_op, value_positions, donor_value_output, donor_value_positions)
                )
            )
        if choice_op != "choice_clean":
            handles.append(
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    model, tokenizer, device = load_model(args.model, os.environ.get("PHASE101_ATTN_IMPLEMENTATIONS", args.attn_implementations))
    ...
    final_path = out_dir / f"{args.model}_phase101_value_choice_bridge_mapping.json"
    partial_path = out_dir / f"{args.model}_phase101_value_choice_bridge_mapping.partial.json"
    results: dict[str, Any] = {
        "phase": 101,
        "model": args.model,
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M"),
        "task": "value_choice_bridge_mapping",
    ...
    if args.resume:
        ...
        if resume_path.exists():
            ...
            if loaded.get("phase") == 101 and loaded.get("model") == args.model:
                results = loaded
    ...
    t0 = time.time()
    log(
        f"Phase101 model={args.model} items={len(items)} value=L{args.value_layer}:{args.value_component}:{args.value_position} "
    ...
    for idx, item in enumerate(items):
        ...
        clean_cache[idx] = {
            ...
            "score": joint_score(model, tokenizer, device, layers, item, None, candidates, letters, target_letter, args, "clean"),
        }
    for idx, item in enumerate(items):
        ...
        for condition in conditions:
            if condition == "clean" or (idx, condition) in completed:
                continue
            if "transplant" in condition and donor_item is None:
                continue
            patched = joint_score(model, tokenizer, device, layers, item, donor_item, cache["candidates"], cache["letters"], cache["target_letter"], args, condition)
            clean = cache["score"]
            row = {
                "item_idx": idx,
                "donor_idx": donor_idx,
...
def main() -> None:
    ...
    parser.add_argument("--conditions", default="value_zero,value_transplant,choice_transplant_heads,value_zero+choice_restore_clean_heads,value_transplant+choice_restore_clean_heads,value_transplant+choice_transplant_heads,value_transplant+choice_transplant_all_restore_clean_heads")
