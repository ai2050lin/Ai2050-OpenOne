...
OUT = ROOT / "tests/result/phase278_gap_recalibration_after_phase277"
...
def load_all_components() -> dict[tuple[str, str], dict[str, Any]]:
    ...
    for phase in ["phase275", "phase277"]:
...
def recalibrate(row: dict[str, Any], components: dict[tuple[str, str], dict[str, Any]], causal: dict[tuple[str, str], list[dict[str, Any]]]) -> dict[str, Any]:
    ...
    status = "filled_by_phase275_277" if not dimensions else ("partially_filled_by_phase275_277" if comp or crows else "still_open")
    return {
        **row,
        "schema_version": SCHEMA_VERSION,
        "phase278_created_at": now(),
        "phase278_resolution": {
        ...
        "priority_score_after_phase277": priority,
        "phase278_status": status,
    }


def select_next(rows: list[dict[str, Any]], max_total: int = 54) -> list[dict[str, Any]]:
    open_rows = [r for r in rows if r.get("phase278_status") != "filled_by_phase275_277"]
    open_rows.sort(key=lambda r: (-safe_float(r.get("priority_score_after_phase277")), str(r.get("family_id")), str(r.get("model")), str(r.get("case_id"))))
    ...
    for rank, row in enumerate(selected, start=1):
        row["phase278_next_batch_rank"] = rank
    return selected
...
def update_v2(summary: dict[str, Any]) -> None:
    ...
    files.update(
        {
            "phase278_recalibrated_gap_rows": "phase278_recalibrated_gap_rows.jsonl",
            "phase278_next_batch_rows": "phase278_next_batch_rows.jsonl",
            "phase278_summary": "phase278_summary.json",
            "phase278_report": "phase278_report.md",
        }
    )
    manifest["latest_gap_recalibration_phase"] = "Phase278"
    manifest["phase278_summary"] = summary
    write_json(V2 / "manifest.json", manifest)
    client = read_json(V2 / "client_index.json")
    for item in ["phase278_summary.json", "phase278_next_batch_rows.jsonl"]:
        if item not in client.setdefault("initial_files", []):
            client["initial_files"].append(item)
    client["phase278_summary_ref"] = "phase278_summary.json"
    client["phase278_next_batch_ref"] = "phase278_next_batch_rows.jsonl"
    write_json(V2 / "client_index.json", client)
    schema = read_json(V2 / "schema.json")
    tables = schema.setdefault("tables", {})
    tables["phase278_recalibrated_gap_rows"] = "Phase274 gaps recalibrated after Phase275 and Phase277 fills"
    tables["phase278_next_batch_rows"] = "next queue after two physical-path fill batches"
    write_json(V2 / "schema.json", schema)


def write_report(summary: dict[str, Any], next_batch: list[dict[str, Any]]) -> None:
    lines = [
        "# Phase278 Gap Recalibration After Phase277",
        "",
        ...
        f"- filled: {summary['phase278_status_counts'].get('filled_by_phase275_277', 0)}",
        f"- partially_filled: {summary['phase278_status_counts'].get('partially_filled_by_phase275_277', 0)}",
        f"- still_open: {summary['phase278_status_counts'].get('still_open', 0)}",
    ...
    for row in next_batch[:12]:
        lines.append(
            f"- rank={row.get('phase278_next_batch_rank')} {row.get('model')} {row.get('family_id')} {row.get('case_id')} "
            f"status={row.get('phase278_status')} priority={row.get('priority_score_after_phase277')} remaining={row.get('remaining_dimensions')}"
        )
    text = "\n".join(lines) + "\n"
    (V2 / "phase278_report.md").write_text(text, encoding="utf-8")
    (OUT / "phase278_report.md").write_text(text, encoding="utf-8")
...
def main() -> None:
    ...
    status_counts = Counter(str(r.get("phase278_status")) for r in recalibrated)
    ...
    summary = {
        ...
        "phase278_status_counts": dict(status_counts),
        ...
        "progress_estimate": {
            "pattern_family_atlas": 0.55,
            "physical_distribution_puzzle": 0.53,
            "component_path_coverage": 0.37,
            "causal_audit_coverage": 0.26,
            "closure": 0.20,
        },
    }
    OUT.mkdir(parents=True, exist_ok=True)
    write_jsonl(OUT / "phase278_recalibrated_gap_rows.jsonl", recalibrated)
    write_jsonl(OUT / "phase278_next_batch_rows.jsonl", next_batch)
    write_json(OUT / "phase278_summary.json", summary)
    write_jsonl(V2 / "phase278_recalibrated_gap_rows.jsonl", recalibrated)
    write_jsonl(V2 / "phase278_next_batch_rows.jsonl", next_batch)
    write_json(V2 / "phase278_summary.json", summary)
    update_v2(summary)
