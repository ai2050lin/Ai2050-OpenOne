#!/usr/bin/env python3
"""Audit Phase386 calibration artifacts without evaluating frozen candidates."""
...
STAGE_ROOT = COLLECTION_ROOT / "calibration"
...
def audit_model(model: str) -> dict[str, Any]:
    ...
    failures: list[str] = []
    layer_count = EXPECTED_LAYERS[model]
    if (
    ...
    if (
        ...
        or any(value > MAX_ERROR for value in manifest["gate_maxima"].values())
    ):
    ...
    if set(mechanisms.values()) != {16} or len(mechanisms) != 3:
    ...
    for row in manifest["files"]:
        ...
        if path.name == "multitime_meta.pt":
            ...
            if (
                [name for call in calls for name in call["coordinate_names"]]
                != list(COORDINATES)
            ...
            continue
        layer_files += 1
        frames = payload["attention"]["frames"]
        if (
            ...
            != 5
            or [name for frame in frames for name in frame["coordinate_names"]]
            != list(COORDINATES)
...
def main() -> None:
    ...
    summary = {
        "schema_version": "60.9.0",
        "phase_id": "Phase386-CalibrationCollectionAudit",
        ...
        "models": rows,
        "results": {
            "all_calibration_artifacts_valid": valid,
            "candidate_values_evaluated_by_this_audit": False,
            "physical_holdout_opened": False,
        },
        "authorization": {
            "frozen_candidate_evaluation": valid,
            "physical_holdout_collection": False,
            "causal_intervention": False,
        },
    }
    path = PHASE_ROOT / "phase386_calibration_collection_summary.json"
    path.write_text(
