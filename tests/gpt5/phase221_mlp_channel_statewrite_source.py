...
RESULT_ROOT = Path("tests/result/phase221_mlp_channel_statewrite_source")


SPECS = {
    "qwen3": [
        {"spec_id": "qwen3_explain_l29_l31_mlp_channels", "pattern_id": "answer_explain", "layers": [29, 31]},
        {"spec_id": "qwen3_repeat_l31_mlp_channels", "pattern_id": "answer_repeat", "layers": [31]},
    ],
    "glm4": [
        {"spec_id": "glm4_repeat_l28_l30_mlp_channels", "pattern_id": "answer_repeat", "layers": [28, 30]},
    ],
    "deepseek7b": [
        {"spec_id": "deepseek7b_explain_l24_mlp_channels", "pattern_id": "answer_explain", "layers": [24]},
    ],
}
...
def capture_mlp_z(
    ...
    for layer_idx in layers_to_scan:
        down_proj = get_down_proj(layers[int(layer_idx)])
        ...
        def make_hook(li: int):
            def hook(_module: Any, inputs: tuple[Any, ...]):
                if inputs and torch.is_tensor(inputs[0]):
                    captured[int(li)] = inputs[0][0, -1, :].detach().float().cpu()
                return None
...
def mean_mlp_z(
    ...
    bucket: dict[int, dict[int, list[torch.Tensor]]] = defaultdict(lambda: defaultdict(list))
    for row in rows:
        for step in range(1, int(max_steps) + 1):
            zmap = capture_mlp_z(model, tokenizer, device, p219.prefix_for_step(row, int(step)), layers)
            for layer_idx, vec in zmap.items():
                bucket[int(step)][int(layer_idx)].append(vec)
    out: dict[int, dict[int, torch.Tensor]] = defaultdict(dict)
    for step, layer_map in bucket.items():
        for layer_idx, vecs in layer_map.items():
            if vecs:
                out[int(step)][int(layer_idx)] = torch.stack(vecs, dim=0).mean(dim=0)
    return out


def channel_score_rows(
    model,
    model_name: str,
    spec: dict[str, Any],
    ...
    max_steps: int,
    top_channels: int,
) -> tuple[list[dict[str, Any]], dict[int, dict[int, list[int]]], dict[int, dict[int, torch.Tensor]]]:
    layers = get_layers(model)
    rows: list[dict[str, Any]] = []
    selected: dict[int, dict[int, list[int]]] = defaultdict(dict)
    ...
    for step in range(1, int(max_steps) + 1):
        for layer_idx in spec["layers"]:
            ...
            # W_down columns are channel output directions.
            col_dot = torch.mv(weight.t(), unit)
            signed = delta.float() * col_dot
            abs_score = torch.abs(signed)
            k = min(int(top_channels), int(abs_score.numel()))
            values, indices = torch.topk(abs_score, k=k)
            selected[int(step)][int(layer_idx)] = [int(i) for i in indices.tolist()]
                ...
                rows.append(
                    {
                        ...
                        "row_kind": "phase221_mlp_channel_score_row",
                        "model": model_name,
                        ...
                        "rank": int(rank),
                        "channel_id": c,
                        "abs_channel_score": float(score),
                        "signed_channel_score": float(signed[c].item()),
...
def install_channel_hook(
    ...
    if len(parts) != 4 or parts[0] != "mlpchan":
        raise ValueError(f"unknown condition: {condition}")
    mode = parts[1]
    layer_idx = int(parts[2][1:])
    k = int(parts[3][1:])
    channels = selected.get(int(step), {}).get(int(layer_idx), [])[:k]
    if not channels:
        return []
    layers = get_layers(model)
    down_proj = get_down_proj(layers[int(layer_idx)])
...
def forward_logits_condition(
    ...
    handles = install_channel_hook(model, condition, int(step), selected, z_delta, float(boost_scale))
    encoded = tokenizer([text], return_tensors="pt", add_special_tokens=False)
    input_ids = encoded["input_ids"].to(device)
    attention_mask = encoded["attention_mask"].to(device)
...
def summarize_channel_scores(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    buckets: dict[tuple[Any, ...], list[dict[str, Any]]] = defaultdict(list)
    for row in rows:
        buckets[(row.get("spec_id"), row.get("layer_idx"))].append(row)
    out = []
    for (spec_id, layer_idx), items in buckets.items():
        top = sorted(items, key=lambda r: float(r.get("abs_channel_score") or 0.0), reverse=True)[:12]
        out.append(
            {
                "spec_id": spec_id,
                "layer_idx": int(layer_idx),
                ...
                "top_channels": [
                    {
                        ...
                        "abs_channel_score": r.get("abs_channel_score"),
    ...
    out.sort(key=lambda row: str(row.get("spec_id")))
    return out
...
def eval_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        for spec in SPECS[args.model]:
                    ...
                    filter_rows.append(
                        {
                            "phase": PHASE,
                            "source_phase": SOURCE_PHASE,
                            "row_kind": "phase221_baseline_filter_row",
                            "model": args.model,
                            "spec_id": spec["spec_id"],
                            "pattern_id": spec["pattern_id"],
            ...
            success_z = mean_mlp_z(model, tokenizer, device, kept_success, layers, int(args.max_channel_steps))
            drift_z = mean_mlp_z(model, tokenizer, device, kept_drift, layers, int(args.max_channel_steps))
            spec_channel_rows, selected, z_delta = channel_score_rows(
                model,
                args.model,
                spec,
                residual_dirs,
            ...
            for layer_idx in layers:
                for k in [4, 16, 64]:
                    conditions.append(f"mlpchan_zero_L{int(layer_idx)}_K{k}")
                    conditions.append(f"mlpchan_boost_L{int(layer_idx)}_K{k}")
                        ...
                        rollout_rows.append(
                            {
                                "phase": PHASE,
                                "source_phase": SOURCE_PHASE,
                                "row_kind": "phase221_mlp_channel_rollout_row",
                                "model": args.model,
                                "spec_id": spec["spec_id"],
                                "pattern_id": spec["pattern_id"],
                                "source_group": source_group,
    ...
    payload = {
        "phase": PHASE,
        "source_phase": SOURCE_PHASE,
        "title": "MLP Channel StateWrite Source",
        "status": "complete",
    ...
    write_json(out_dir / f"phase221_{args.model}_summary.json", payload)
    write_jsonl(out_dir / f"phase221_{args.model}_filter_rows.jsonl", filter_rows)
    write_jsonl(out_dir / f"phase221_{args.model}_rollout_rows.jsonl", rollout_rows)
    write_jsonl(out_dir / f"phase221_{args.model}_summary_rows.jsonl", summary_rows)
    write_jsonl(out_dir / f"phase221_{args.model}_effect_rows.jsonl", effects)
    write_jsonl(out_dir / f"phase221_{args.model}_channel_score_rows.jsonl", channel_rows)
    write_jsonl(out_dir / f"phase221_{args.model}_channel_summary_rows.jsonl", channel_summary_rows)
    print(
        json.dumps(
            {
                "phase": PHASE,
...
def summarize_round(round_name: str) -> dict[str, Any]:
    ...
    summaries = [read_json(out_dir / f"phase221_{model}_summary.json") for model in MODELS]
    summaries = [summary for summary in summaries if summary]
    effects = []
    channel_summaries = []
    for model in MODELS:
        effects.extend(p214.iter_jsonl(out_dir / f"phase221_{model}_effect_rows.jsonl") or [])
        channel_summaries.extend(p214.iter_jsonl(out_dir / f"phase221_{model}_channel_summary_rows.jsonl") or [])
    payload = {
        "phase": PHASE,
        "source_phase": SOURCE_PHASE,
        "title": "Cross-model MLP Channel StateWrite Source",
        "status": "complete",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ...
    write_json(out_dir / "phase221_cross_model_summary.json", payload)
    lines = ["# Phase 221 MLP channel StateWrite source", ""]
    for key in [
        "spec_count",
        "filter_rows",
    ...
    for row in payload["top_effect_rows"][:35]:
        lines.append(
    ...
    for row in channel_summaries:
        lines.append(f"### {row.get('spec_id')} L{row.get('layer_idx')}")
        for ch in (row.get("top_channels") or [])[:8]:
            lines.append(
                f"- step={ch.get('step')} rank={ch.get('rank')} channel={ch.get('channel_id')} "
                f"score={ch.get('abs_channel_score')} signed={ch.get('signed_channel_score')}"
            )
    (out_dir / "phase221_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(
        json.dumps(
            {
                "phase": PHASE,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Phase221 MLP channel StateWrite source")
    parser.add_argument("--model", choices=MODELS)
    parser.add_argument("--summarize", action="store_true")
    parser.add_argument("--round-name", default="mlp_channel_statewrite_source")
    ...
    parser.add_argument("--top-channels", type=int, default=64)
