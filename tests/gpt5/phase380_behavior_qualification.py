#!/usr/bin/env python3
"""Run the fresh Phase380 behavior gate for one local model."""
...
CASES = OUT / "private/phase380_execution_cases.jsonl"
...
def process(model: str, batch_size: int, max_new_tokens: int) -> dict[str, Any]:
    ...
    if len(cases) != 384:
        raise RuntimeError(f"Expected 384 Phase380 cases for {model}, got {len(cases)}")
    loaded = None
    rows = []
    try:
        ...
        for _length, bucket in sorted(buckets.items()):
            for start in range(0, len(bucket), batch_size):
                ...
                for case, result in zip(selected, generated, strict=True):
                    rows.append(
                        {
                            "schema_version": "53.1.0",
                            "phase_id": "Phase380-Behavior",
                ...
                print(f"[{model}] Phase380 behavior {completed}/384", flush=True)
        path = OUT / "behavior/private/models" / model / "phase380_behavior_rows.jsonl"
