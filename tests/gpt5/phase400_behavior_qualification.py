#!/usr/bin/env python3
"""Run the frozen Phase400 four-surface behavior denominator."""
...
CASES = OUT / "protocol/private/phase400_candidate_cases.jsonl"
...
def run(model: str, batch_size: int, max_new_tokens: int) -> dict[str, Any]:
    ...
    if len(cases) != EXPECTED_PER_MODEL:
        raise RuntimeError(
            f"Expected {EXPECTED_PER_MODEL} Phase400 cases for {model}, got {len(cases)}"
        )
    loaded = None
    ...
    try:
        ...
        if runtime_dtype != FROZEN_DTYPE[model]:
            raise RuntimeError(f"Phase400 dtype mismatch for {model}: {runtime_dtype}")
        ...
        for selected in batches:
            ...
            for case, result in zip(selected, generated, strict=True):
                rows.append(
                    {
                        "schema_version": "74.2.0",
                        "phase_id": "Phase400-BehaviorQualification",
                        "created_at": now(),
                        "model": model,
                        ...
                        "selection_priority_private": case["selection_priority_private"],
            ...
            if completed % 128 == 0 or completed == len(cases):
                ...
                print(
                    f"[{model}] Phase400 behavior {completed}/{len(cases)} strict={strict}",
                    flush=True,
                )
            if completed % 256 == 0:
                gc.collect()
        ...
        correct = Counter(
            row["task_surface_private"] for row in rows if row["strict_behavior_correct"]
        )
        exact = Counter(
            row["task_surface_private"] for row in rows if row["exact_answer_match"]
        )
        payload = {
            "schema_version": "74.2.0",
            "phase_id": "Phase400-BehaviorQualification",
            ...
            "claim_boundary": {
                "behavior_is_a_dynamic_partial_order_mechanism": False,
