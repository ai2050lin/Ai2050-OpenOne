...
echo "=== Phase304 all-model role query template calibration ==="
date '+%Y-%m-%d %H:%M:%S %Z'
...
run_model() {
  ...
  echo
  echo "=== Run ${model} ==="
  MAX_SECONDS="$max_seconds" tests/gpt5/run_phase304_normal.sh "$model" "${COMMON_ARGS[@]}"
  echo "=== Completed ${model}; sleep for unload ==="
  sleep "${SLEEP_AFTER_MODEL:-10}"
}

run_model qwen3 "${QWEN3_MAX_SECONDS:-1800}"
run_model glm4 "${GLM4_MAX_SECONDS:-2400}"
run_model deepseek7b "${DEEPSEEK7B_MAX_SECONDS:-2400}"

python tests/gpt5/phase304_role_query_template_summary.py \
  --input-dir "$OUTPUT_DIR" \
  --output-dir "$OUTPUT_DIR"

echo "=== Phase304 all-model done ==="
date '+%Y-%m-%d %H:%M:%S %Z'
