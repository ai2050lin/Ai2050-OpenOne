...
RUN_ID="$(date +%Y%m%d_%H%M%S)_phase291_${MODEL}"
...
echo "=== Phase291 conservative logged run ==="
date '+%Y-%m-%d %H:%M:%S %Z'
echo "run_id=${RUN_ID}"
echo "model=${MODEL}"
...
PY
    ...
    ps -eo pid,ppid,etime,stat,pcpu,pmem,wchan:32,cmd | grep -E 'phase291_block_contract_scan|python tests/gpt5|nvidia-smi|cuda|ComfyUI|python main.py' | grep -v grep || true
...
(
  while true; do
    ...
    ps -eo pid,etime,stat,pcpu,pmem,wchan:28,cmd | grep -E 'phase291_block_contract_scan|python tests/gpt5|nvidia-smi|cuda|python main.py' | grep -v grep || true
    sleep 10
  done
...
PYTHONUNBUFFERED=1 timeout --foreground --kill-after=30s "${MAX_SECONDS}s" \
  python tests/gpt5/phase291_block_contract_scan.py "$MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --hard-exit-after-model \
    "$@"
RC="$?"
set -e
echo "phase291_exit_code=${RC}"
