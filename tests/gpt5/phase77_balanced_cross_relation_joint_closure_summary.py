...
def sorted_entries(mapping: dict[str, Any], limit: int = 160) -> list[dict[str, Any]]:
...
def add_table(lines: list[str], title: str, entries: list[dict[str, Any]], limit: int = 50) -> None:
    lines += [
        f"### {title}",
        "",
...
def main() -> None:
    ...
    lines = ["# Phase77 Balanced Cross-Relation Joint Closure Summary", ""]
    for model in MODELS:
        path = input_dir / f"{model}_phase77_balanced_cross_relation_joint_closure.json"
        lines += [f"## {model}", ""]
        if not path.exists():
            lines += ["missing", ""]
            continue
        ...
        sm = data.get("summary", {})
        by_condition = sorted_entries(sm.get("by_condition", {}), 50)
        by_condition_path = sorted_entries(sm.get("by_condition_path", {}), 80)
        by_condition_relation = sorted_entries(sm.get("by_condition_relation", {}), 160)
        ...
        lines += [
            f"objects={data.get('num_objects')}, items={data.get('num_items')}, rows={len(data.get('rows', []))}, layer_pairs={data.get('layer_pairs')}",
            f"relations={data.get('relations')}",
            "",
        ]
        add_table(lines, "By condition", by_condition, 50)
        add_table(lines, "Top condition paths", by_condition_path, 50)
        add_table(lines, "Top condition relations", by_condition_relation, 80)

    out_json = output_dir / "phase77_balanced_cross_relation_joint_closure_summary.json"
    out_md = output_dir / "PHASE77_BALANCED_CROSS_RELATION_JOINT_CLOSURE_SUMMARY.md"
