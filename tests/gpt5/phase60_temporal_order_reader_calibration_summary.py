...
def load_model(input_dir: Path, model: str) -> dict[str, Any] | None:
    path = input_dir / f"{model}_phase60_temporal_order_reader_calibration.json"
    if not path.exists():
        shard_paths = sorted(input_dir.glob(f"{model}_phase60_temporal_order_reader_calibration_shard*.json"))
        if not shard_paths:
            return None
        ...
        if not rows:
            return None
        # Reuse the per-shard summary logic by importing the phase script.
        import sys

        sys.path.insert(0, str(Path(__file__).resolve().parent))
        from phase60_temporal_order_reader_calibration import summarize  # type: ignore
...
def main() -> None:
    ...
    lines: list[str] = ["# Phase60 Temporal Order Reader Calibration Summary", ""]
    ...
    for model in MODELS:
        ...
        lines.append(f"## {model}")
        lines.append("")
        lines.append(f"cases={data['num_cases']}, rows={data['num_rows']}")
        lines.append("")
        lines.append("| rank | reader | acc | min_ctx | min_rel | margin | abs_margin | pass |")
        lines.append("|---:|---|---:|---:|---:|---:|---:|---|")
        ...
        cross_rows.append(
            {
                ...
                "min_relation_accuracy": min(r["min_relation_accuracy"] for r in rows),
    ...
    lines.append("")
    lines.append("| rank | reader | mean_acc | min_acc | min_ctx | min_rel | all_pass |")
    lines.append("|---:|---|---:|---:|---:|---:|---|")
    ...
    (output_dir / "phase60_temporal_order_reader_calibration_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (output_dir / "PHASE60_TEMPORAL_ORDER_READER_CALIBRATION_SUMMARY.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
    print(f"wrote {output_dir / 'phase60_temporal_order_reader_calibration_summary.json'}")
    print(f"wrote {output_dir / 'PHASE60_TEMPORAL_ORDER_READER_CALIBRATION_SUMMARY.md'}")
