#!/usr/bin/env python3
"""Summarize Phase 143 time interface and empirical suppressor tests."""
...
def load_result(model: str) -> dict[str, Any] | None:
    path = OUT_ROOT / f"phase143_{model}_time_interface_empirical_suppressor.json"
    if not path.exists():
        return None
...
def main() -> None:
    ...
    lines = ["# Phase 143 Cross-model Time Interface Empirical Suppressor Summary", ""]
    for model in MODELS:
        ...
        for key, item in result["category_results"].items():
            ...
            lines.append(
                ...
                f"{fmt(item.get('best_empirical_joint'))} |"
            )
        lines.append("")
    path = OUT_ROOT / "phase143_cross_model_summary.md"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(path)
