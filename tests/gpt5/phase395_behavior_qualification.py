#!/usr/bin/env python3
"""Run the frozen Phase395 natural same-token-multiset behavior denominator."""
...
CASES = OUT / "protocol/private/phase395_candidate_cases.jsonl"
...
def run(model: str, max_new_tokens: int) -> dict[str, Any]:
    ...
    if len(cases) != EXPECTED_PER_MODEL:
        raise RuntimeError(
            f"Expected {EXPECTED_PER_MODEL} Phase395 cases for {model}, got {len(cases)}"
        )
    loaded = None
    ...
    try:
        ...
        if runtime_dtype != FROZEN_DTYPE[model]:
            raise RuntimeError(
                f"Phase395 dtype mismatch for {model}: {runtime_dtype} != {FROZEN_DTYPE[model]}"
            )
        for index, case in enumerate(cases, 1):
            result = generate_batch(loaded, [case], max_new_tokens)[0]
            rows.append(
                {
                    "schema_version": "69.1.0",
                    "phase_id": "Phase395-BehaviorQualification",
                    "created_at": now(),
                    "model": model,
            ...
            if index % 24 == 0 or index == len(cases):
                print(
                    f"[{model}] Phase395 behavior {index}/{len(cases)} "
        ...
        summary = {
            ...
            "claim_boundary": {
                "natural_behavior_is_internal_binding_state": False,
                "same_token_multiset_is_same_token_positions": False,
