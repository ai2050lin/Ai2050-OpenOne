...
MODEL="${1:?usage: $0 MODEL [phase306 args...]}"
shift || true
...
RUN_ID="$(date +%Y%m%d_%H%M%S)_phase306normal_${MODEL}"
...
echo "=== Phase306 normal logged run ==="
date '+%Y-%m-%d %H:%M:%S %Z'
echo "run_id=${RUN_ID}"
echo "model=${MODEL}"
...
if [[ "$ENABLE_GPU_MONITOR" == "1" ]]; then
  ...
  (
    while true; do
      ...
      ps -eo pid,etime,stat,pcpu,pmem,wchan:28,cmd | grep -E 'phase306_symbolic_role_query_calibration|python tests/gpt5|nvidia-smi|cuda|python main.py' | grep -v grep || true
      sleep 10
    done
...
echo "=== Run Phase306 normal ==="
set +e
PYTHONUNBUFFERED=1 timeout --foreground --kill-after=30s "${MAX_SECONDS}s" \
  python tests/gpt5/phase306_symbolic_role_query_calibration.py "$MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --hard-exit-after-model \
    "$@"
RC="$?"
set -e
echo "phase306_exit_code=${RC}"
