...
MODEL="${1:?usage: $0 MODEL [phase300 args...]}"
shift || true

export OPENONE_NORMAL_ENV="${OPENONE_NORMAL_ENV:-openone-cuda121}"
export OUTPUT_DIR="${OUTPUT_DIR:-results/gpt5_phase300_voice_closure_pilot}"
...
RUN_ID="$(date +%Y%m%d_%H%M%S)_phase300normal_${MODEL}"
...
echo "=== Phase300 normal logged run ==="
date '+%Y-%m-%d %H:%M:%S %Z'
echo "run_id=${RUN_ID}"
echo "model=${MODEL}"
...
PY
    ...
    ps -eo pid,ppid,etime,stat,pcpu,pmem,wchan:32,cmd | grep -E 'phase300_voice_closure_pilot|python tests/gpt5|nvidia-smi|cuda|ComfyUI|python main.py' | grep -v grep || true
...
(
  while true; do
    ...
    ps -eo pid,etime,stat,pcpu,pmem,wchan:28,cmd | grep -E 'phase300_voice_closure_pilot|python tests/gpt5|nvidia-smi|cuda|python main.py' | grep -v grep || true
    sleep 10
  done
) >"$LOG_DIR/gpu_process_monitor.log" 2>&1 &
GPU_MONITOR_PID="$!"

echo "=== Run Phase300 normal ==="
set +e
PYTHONUNBUFFERED=1 timeout --foreground --kill-after=30s "${MAX_SECONDS}s" \
  python tests/gpt5/phase300_voice_closure_pilot.py "$MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --hard-exit-after-model \
    "$@"
RC="$?"
set -e
echo "phase300_exit_code=${RC}"
