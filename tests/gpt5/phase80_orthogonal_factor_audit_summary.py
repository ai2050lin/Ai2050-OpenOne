...
def sorted_entries(mapping: dict[str, Any], limit: int = 240) -> list[dict[str, Any]]:
...
def main() -> None:
    ...
    lines = ["# Phase80 Orthogonal Factor Audit Summary", ""]
    for model in MODELS:
        path = input_dir / f"{model}_phase80_orthogonal_factor_audit.json"
        lines += [f"## {model}", ""]
        if not path.exists():
            lines += ["missing", ""]
            continue
        ...
        sm = data.get("summary", {})
        by_condition = sorted_entries(sm.get("by_condition", {}), 120)
        by_condition_path = sorted_entries(sm.get("by_condition_path", {}), 160)
        ...
        lines += [
            ...
            f"module={data.get('module')}, contrast_rank={data.get('contrast_rank')}, nuisance_rank={data.get('nuisance_rank')}, relations={data.get('relations')}",
            "",
        ]
        add_table(lines, "By condition", by_condition, 120)
        add_table(lines, "Top condition paths", by_condition_path, 120)
        add_table(lines, "Top condition relations", by_condition_relation, 160)
    out_json = output_dir / "phase80_orthogonal_factor_audit_summary.json"
    out_md = output_dir / "PHASE80_ORTHOGONAL_FACTOR_AUDIT_SUMMARY.md"
