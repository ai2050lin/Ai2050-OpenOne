...
RUN_ID="$(date +%Y%m%d_%H%M%S)_phase302normal_${MODEL}"
...
echo "=== Phase302 normal logged run ==="
date '+%Y-%m-%d %H:%M:%S %Z'
echo "run_id=${RUN_ID}"
echo "model=${MODEL}"
...
if [[ "$ENABLE_GPU_MONITOR" == "1" ]]; then
  ...
  (
    while true; do
      ...
      ps -eo pid,etime,stat,pcpu,pmem,wchan:28,cmd | grep -E 'phase302_passive_token_role_closure|python tests/gpt5|nvidia-smi|cuda|python main.py' | grep -v grep || true
      sleep 10
    done
...
echo "=== Run Phase302 normal ==="
set +e
PYTHONUNBUFFERED=1 timeout --foreground --kill-after=30s "${MAX_SECONDS}s" \
  python tests/gpt5/phase302_passive_token_role_closure.py "$MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --hard-exit-after-model \
    "$@"
RC="$?"
set -e
echo "phase302_exit_code=${RC}"
