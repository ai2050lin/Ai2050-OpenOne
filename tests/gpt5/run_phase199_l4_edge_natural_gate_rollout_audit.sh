...
ROUND_NAME="${1:-l4_edge_natural_gate_rollout_audit}"
SCRIPT="tests/gpt5/phase199_l4_edge_natural_gate_rollout_audit.py"
...
run_model() {
  ...
  python "${SCRIPT}" \
    ...
    --max-edges-per-model 7 \
    --max-samples-per-edge 24 \
    --boost-factor 1.5 \
    --max-new-tokens 8 \
    --batch-size 8
}
