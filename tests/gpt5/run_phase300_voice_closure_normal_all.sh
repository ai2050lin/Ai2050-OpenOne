...
export MAX_PAIRS_PER_SUBTYPE="${MAX_PAIRS_PER_SUBTYPE:-24}"
export COMMON_ALPHAS="${COMMON_ALPHAS:-0,0.5,1.0,1.5}"
...
run_one() {
  ...
  local seconds="$3"
  echo
  echo "=== Phase300 voice closure normal: ${model} ==="
  date '+%Y-%m-%d %H:%M:%S %Z'
  echo "layers=${layers}"
  ...
  MAX_SECONDS="$seconds" OUTPUT_DIR="$OUTPUT_DIR" \
    tests/gpt5/run_phase300_normal.sh "$model" \
...
echo
echo "=== Phase300 voice closure normal all done ==="
date '+%Y-%m-%d %H:%M:%S %Z'
