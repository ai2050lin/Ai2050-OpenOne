#!/usr/bin/env python3
"""Summarize Phase 135 long-template source field decomposition."""
...
def load_result(model: str) -> dict[str, Any] | None:
    path = OUT_ROOT / f"phase135_{model}_long_template_source_field.json"
    if not path.exists():
        return None
...
def main() -> None:
    ...
    lines = ["# Phase 135 Cross-model Long-template Source Field Summary", ""]
    for model in MODELS:
        ...
        lines.append("")
        lines.append("| category | audit | reference | best | prefix | object | relation | bridge | tail | all-pre |")
        lines.append("|---|---|---|---|---|---|---|---|---|---|")
        for cat, item in result["category_results"].items():
            ...
            lines.append(
                ...
                f"{fmt(src(rows, 'prefix'))} | {fmt(src(rows, 'object_span'))} | "
                f"{fmt(src(rows, 'relation_phrase'))} | {fmt(src(rows, 'reasoning_bridge'))} | "
                f"{fmt(src(rows, 'answer_tail'))} | {fmt(src(rows, 'all_pre_answer'))} |"
            )
        lines.append("")
    path = OUT_ROOT / "phase135_cross_model_summary.md"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(path)
