...
OUT_ROOT = Path("results/glm5_phase757_multisite_downstream_carrier_closure")
...
def combo_defs_for(model_name: str, writer_site: str, n_layers: int, max_combos: int) -> list[dict[str, Any]]:
    ...
    off_sites = sites_for_layers(OFF_PATH_LAYERS.get(model_name, []), n_layers, writer_layer)
    ...
    if off_sites:
        combos.append({"combo_name": "off_path_same_count", "combo_kind": "off_path_control", "sites": off_sites[: max(1, len(primary_sites))]})
...
def classify_combo(vals: list[dict[str, Any]]) -> str:
    ...
    if "off_path_control" in kinds and (success >= 0.30 or mean_rec >= 0.08):
        return "off_path_control_suspicious"
    if success >= 0.45 and mean_rec >= 0.10 and mean_frac >= 0.25:
        if mean_rel >= 0.03:
            return "multisite_writer_guard_carrier_candidate"
        return "multisite_target_carrier_candidate"
    if success >= 0.25 and mean_rec >= 0.05 and mean_frac >= 0.15:
        return "partial_multisite_carrier_candidate"
    if mean_rec < -0.05:
...
def build_summary(args, rows: list[dict[str, Any]], candidates: list[dict[str, Any]], source_groups: list[str], all_combo_sites: list[str], attn_impl: str) -> dict[str, Any]:
    ...
    return {
        "phase": 757,
        "title": "Multi-Site Downstream Carrier Closure Test",
        "model": args.model,
        ...
        "strict_interpretation": "Multi-site restore tests coarse component closure. It still does not prove neuron-level coding or natural generation closure.",
    }
...
def run_model(args) -> dict[str, Any]:
    ...
    write_jsonl(out_dir / f"phase757_{args.model}_rows.jsonl", rows)
    write_json(out_dir / f"phase757_{args.model}_summary.json", summary)
    print(
        json.dumps(
            {
                "model": args.model,
...
def write_cross_summary(round_name: str) -> dict[str, Any]:
    ...
    summaries = []
    for model in MODELS:
        path = out_dir / f"phase757_{model}_summary.json"
        if path.exists():
    ...
    payload = {
        ...
        "strict_interpretation": "Coarse multi-site component restore. Strong only if primary multi-site beats single-site and off-path controls.",
    }
    write_json(out_dir / "phase757_cross_model_summary.json", payload)
    lines = [
        f"# Phase 757 Multi-Site Downstream Carrier Closure Test ({round_name})",
        "",
    ...
    (out_dir / "phase757_cross_model_summary.md").write_text("\n".join(lines), encoding="utf-8")
    print(json.dumps({"round": round_name, "status": payload["status"], "models": payload["models"]}, ensure_ascii=False, indent=2), flush=True)
    return payload


def dry_run(args) -> None:
    payload = {"phase": 757, "round": args.round_name, "pairs": len(select_global_pairs(args.max_pairs)), "source_groups": source_groups_for(args), "models": {}}
    for model_name in MODELS:
        payload["models"][model_name] = {
            "primary_layers": PRIMARY_PATH_LAYERS.get(model_name, []),
            "off_path_layers": OFF_PATH_LAYERS.get(model_name, []),
        }
