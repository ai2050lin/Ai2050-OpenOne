...
class Phase387To389CausalOrderTests(unittest.TestCase):
    ...
    def test_phase389_atlas_and_client_match_strict_latest_boundary(self) -> None:
        for root in (ATLAS, CLIENT):
            ...
            manifest_phase = int(manifest["last_phase"].split("Phase", 1)[1].split("-", 1)[0])
            progress_phase = int(progress["last_phase"].split("Phase", 1)[1].split("-", 1)[0])
            self.assertGreaterEqual(manifest_phase, 389)
            self.assertGreaterEqual(progress_phase, 389)
