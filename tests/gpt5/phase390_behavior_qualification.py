#!/usr/bin/env python3
"""Run Phase390 behavior qualification in the frozen single-sample model order."""
...
CASE_FILE = PHASE_ROOT / "protocol/private/phase390_candidate_execution_cases.jsonl"
MODELS = ("qwen3", "glm4", "deepseek7b")
EXPECTED_CASES_PER_MODEL = 6 * 24 * 4
FROZEN_DTYPE = {
    "qwen3": "float16",
    "glm4": "float16",
    "deepseek7b": "bfloat16",
...
def process(model: str, max_new_tokens: int) -> dict[str, Any]:
    ...
    if len(cases) != EXPECTED_CASES_PER_MODEL:
        raise RuntimeError(
            f"Expected {EXPECTED_CASES_PER_MODEL} Phase390 cases for {model}, got {len(cases)}"
        )
    loaded = None
    ...
    try:
        ...
        if runtime_dtype != FROZEN_DTYPE[model]:
            raise RuntimeError(
                f"Phase390 dtype mismatch for {model}: {runtime_dtype} != {FROZEN_DTYPE[model]}"
            )
        for index, case in enumerate(cases, 1):
            result = generate_batch(loaded, [case], max_new_tokens)[0]
            rows.append(
                {
                    "schema_version": "64.1.0",
                    "phase_id": "Phase390-BehaviorQualification",
                    "created_at": now(),
                    "model": model,
            ...
            if index % 24 == 0 or index == len(cases):
                print(
                    f"[{model}] Phase390 behavior {index}/{len(cases)} "
        ...
        summary = {
            ...
            "valid": len(rows) == EXPECTED_CASES_PER_MODEL,
            "claim_boundary": {
                "behavior_qualification_is_internal_path": False,
                "failed_cases_selectively_replaced": False,
                "exact_incremental_trace_must_requalify_target_decision": True,
            },
        }
        write_jsonl(
            PHASE_ROOT
