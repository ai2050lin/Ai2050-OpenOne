...
RESULT_ROOT = ROOT / "tests/result/phase279_third_gap_batch_physical_path_fill"
ROUND_DEFAULT = "third_gap_batch_physical_path_fill"
...
def normalize_queue_row(row: dict[str, Any]) -> dict[str, Any]:
    out = dict(row)
    out["batch_rank"] = row.get("phase278_next_batch_rank") or row.get("batch_rank")
...
def select_model_queue(model: str, max_cases: int, include_candidate_rows: bool) -> list[dict[str, Any]]:
    rows = [normalize_queue_row(r) for r in read_jsonl(V2 / "phase278_next_batch_rows.jsonl") if r.get("model") == model]
    if not include_candidate_rows:
        rows = [r for r in rows if "candidate_closure_verification" not in set(r.get("missing_dimensions") or [])]
    rows.sort(key=lambda r: (int(r.get("batch_rank") or 999999), -safe_float(r.get("priority_score_after_phase277")), str(r.get("family_id")), str(r.get("case_id"))))
...
def retag_rows(rows: list[dict[str, Any]], id_keys: list[str]) -> None:
    for row in rows:
        ...
        row["phase_id"] = "Phase279"
        row["created_at"] = utc_now()
        for key in id_keys:
            if key in row and isinstance(row[key], str):
                row[key] = row[key].replace("phase275:", "phase279:").replace("Phase275", "Phase279")
...
def update_case_detail(model: str, case_id: str, summary: dict[str, Any] | None, causal_rows: list[dict[str, Any]]) -> None:
    ...
    detail["phase279_component_summary"] = summary
    detail["phase279_causal_fill_rows"] = causal_rows
    detail["phase279_updated_at"] = utc_now()
...
def evaluate_model(args: argparse.Namespace, model: str) -> dict[str, Any]:
    ...
    try:
        ...
        for idx, queue_row in enumerate(selected, start=1):
            case = {
                ...
                "path_schema_id": queue_row.get("path_schema_id") or f"phase279:{queue_row['family_id']}:{queue_row['mode_id']}:{queue_row['variant_id']}",
            ...
            try:
                ...
                for rows, keys in [
                    ...
                    for row in rows:
                        row["source_phase278_gap_id"] = queue_row.get("gap_id")
                        row["source_phase278_batch_rank"] = queue_row.get("batch_rank")
                summary = {
                    **summary,
                    "schema_version": SCHEMA_VERSION,
                    "phase_id": "Phase279",
                    "created_at": utc_now(),
                    "source_phase278_gap_id": queue_row.get("gap_id"),
                    "source_phase278_batch_rank": queue_row.get("batch_rank"),
                    "component_summary_id": f"phase279:summary:{model}:{queue_row['case_id']}",
                }
                ...
                missing_rows.append(
                    {
                        ...
                        "phase_id": "Phase279",
                        "created_at": utc_now(),
                        "missing_id": f"phase279:missing:{model}:{queue_row['case_id']}",
                        "source_phase278_gap_id": queue_row.get("gap_id"),
                        "model": model,
                        "case_id": queue_row["case_id"],
                        "family_id": queue_row["family_id"],
            ...
            log(f"{model}: phase279 filled {idx}/{len(selected)} selected gap rows")
    ...
    write_json(out_dir / f"phase279_{model}_summary.json", payload)
    write_jsonl(out_dir / f"phase279_{model}_component_physical_path_rows.jsonl", component_rows)
    write_jsonl(out_dir / f"phase279_{model}_attention_contribution_rows.jsonl", attn_rows)
    write_jsonl(out_dir / f"phase279_{model}_mlp_contribution_rows.jsonl", mlp_rows)
    write_jsonl(out_dir / f"phase279_{model}_residual_accumulation_rows.jsonl", residual_rows)
    write_jsonl(out_dir / f"phase279_{model}_component_summary_rows.jsonl", summary_rows)
    write_jsonl(out_dir / f"phase279_{model}_causal_fill_rows.jsonl", causal_rows)
    write_jsonl(out_dir / f"phase279_{model}_rollout_fill_rows.jsonl", rollout_rows)
    write_jsonl(out_dir / f"phase279_{model}_missing_rows.jsonl", missing_rows)
...
def summarize_model(model: str, selected: list[dict[str, Any]], component_rows: list[dict[str, Any]], summaries: list[dict[str, Any]], causal_rows: list[dict[str, Any]], rollout_rows: list[dict[str, Any]], missing_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "schema_version": SCHEMA_VERSION,
        "phase_id": "Phase279",
        "title": "Phase278 third gap batch physical path fill",
        "status": "complete",
        "created_at": utc_now(),
        "model": model,
...
def update_v2(round_name: str, payload: dict[str, Any]) -> None:
    ...
    for table in table_names:
        ...
        for model in MODELS:
            rows.extend(read_jsonl(out_dir / f"phase279_{model}_{table}.jsonl"))
        write_jsonl(V2 / f"phase279_{table}.jsonl", rows)
    write_json(V2 / "phase279_cross_model_summary.json", payload)
    manifest = read_json(V2 / "manifest.json")
    files = manifest.setdefault("files", {})
    for table in table_names:
        files[f"phase279_{table}"] = f"phase279_{table}.jsonl"
    files["phase279_cross_model_summary"] = "phase279_cross_model_summary.json"
    files["phase279_report"] = "phase279_report.md"
    manifest["latest_fill_phase"] = "Phase279"
    manifest["phase279_summary"] = payload
    write_json(V2 / "manifest.json", manifest)
    client = read_json(V2 / "client_index.json")
    for item in ["phase279_cross_model_summary.json", "phase279_component_summary_rows.jsonl", "phase279_causal_fill_rows.jsonl"]:
        if item not in client.setdefault("initial_files", []):
            client["initial_files"].append(item)
    client["phase279_summary_ref"] = "phase279_cross_model_summary.json"
    client["phase279_component_summary_ref"] = "phase279_component_summary_rows.jsonl"
    client["phase279_causal_fill_ref"] = "phase279_causal_fill_rows.jsonl"
    write_json(V2 / "client_index.json", client)
    schema = read_json(V2 / "schema.json")
    tables = schema.setdefault("tables", {})
    tables["phase279_component_summary_rows"] = "Phase278 next queue component path fills"
    tables["phase279_causal_fill_rows"] = "Phase278 next queue low-side-effect and diagnostic causal rows"
    write_json(V2 / "schema.json", schema)


def summarize_round(round_name: str) -> dict[str, Any]:
    out_dir = RESULT_ROOT / round_name
    summaries = [read_json(out_dir / f"phase279_{model}_summary.json") for model in MODELS]
    summaries = [s for s in summaries if s]
    component_summary_rows: list[dict[str, Any]] = []
    causal_rows: list[dict[str, Any]] = []
    missing_rows: list[dict[str, Any]] = []
    for model in MODELS:
        component_summary_rows.extend(read_jsonl(out_dir / f"phase279_{model}_component_summary_rows.jsonl"))
        causal_rows.extend(read_jsonl(out_dir / f"phase279_{model}_causal_fill_rows.jsonl"))
        missing_rows.extend(read_jsonl(out_dir / f"phase279_{model}_missing_rows.jsonl"))
    low_side = [r for r in causal_rows if r.get("side_effect_level") == "lower"]
    payload = {
        "phase": PHASE,
        "schema_version": SCHEMA_VERSION,
    ...
    write_json(out_dir / "phase279_cross_model_summary.json", payload)
    ...
    report = "\n".join(lines) + "\n"
    (out_dir / "phase279_report.md").write_text(report, encoding="utf-8")
    (V2 / "phase279_report.md").write_text(report, encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2))
    return payload
