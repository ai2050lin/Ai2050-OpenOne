...
Phase 705 measured target-vs-donor competition with first-token value sets.
That metric is ambiguous when target and donor values share the same first
token. Phase 706 keeps only donor pairs whose target and donor expected
first-token sets are disjoint.
"""
...
OUT_ROOT = Path("results/glm5_phase706_first_token_disjoint_identity_competition_audit")
...
def run_model(args) -> dict[str, Any]:
    ...
    try:
        ...
        for case_id in paired_ids:
            ...
            if unrelated is not None and case_cache[case_id]["canonical_value_ids"].isdisjoint(case_cache[unrelated]["canonical_value_ids"]):
                donor_pairs.append({"target": case_id, "donor": unrelated, "donor_type": "unrelated_disjoint"})

        donor_summaries.append({
            "n_targets": len(paired_ids),
            "n_donor_pairs": len(donor_pairs),
            "same_value_pairs": 0,
            "unrelated_pairs": sum(1 for p in donor_pairs if p["donor_type"] == "unrelated_disjoint"),
            "filter": "target expected first-token set disjoint from donor expected first-token set",
    ...
    (OUT_ROOT / f"phase706_{args.model}_first_token_disjoint_rows.jsonl").write_text(
        "\n".join(json.dumps(row, ensure_ascii=False, sort_keys=True) for row in rows) + "\n",
        encoding="utf-8",
    )
    payload = {
        "phase": 706,
        "title": "First-Token-Disjoint Identity Competition Audit",
    ...
    (OUT_ROOT / f"phase706_{args.model}_first_token_disjoint_summary.json").write_text(
...
def write_cross_summary() -> dict[str, Any]:
    OUT_ROOT.mkdir(parents=True, exist_ok=True)
    models = []
    for path in sorted(OUT_ROOT.glob("phase706_*_first_token_disjoint_summary.json")):
        models.append(json.loads(path.read_text(encoding="utf-8")))
    payload = {
        "phase": 706,
        "title": "First-Token-Disjoint Identity Competition Audit Cross-Model Summary",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "models": models,
    }
    (OUT_ROOT / "phase706_cross_model_summary.json").write_text(
        json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True),
        encoding="utf-8",
    )
    lines = [
    ...
    for item in models:
        ...
        lines.append(
            ...
            f"{br.get('condition','')} | {br.get('success_change_rate',0.0):.3f} | {br.get('target_value_top1_rate',0.0):.3f} | {br.get('donor_value_top1_rate',0.0):.3f} | {br.get('mean_target_minus_donor') or 0.0:.3f} | "
            f"{bd.get('condition','')} | {bd.get('success_change_rate',0.0):.3f} | {bd.get('target_value_top1_rate',0.0):.3f} | {bd.get('donor_value_top1_rate',0.0):.3f} | {bd.get('mean_target_minus_donor') or 0.0:.3f} |"
        )
    for section, key in [("Best Restore", "best_restore_conditions"), ("Best Degradation", "best_degradation_conditions")]:
        ...
        for item in models:
            ...
            lines.append("")
            lines.append("| condition | change | target_top1 | donor_top1 | target_rank | donor_rank | target-donor | target-prose | n_channels | best_other |")
            lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|---|")
            for row in item["summary"][key][:24]:
                lines.append(
                    f"| {row['condition']} | {row['success_change_rate']:.3f} | {row['target_value_top1_rate']:.3f} | {row['donor_value_top1_rate']:.3f} | "
                    f"{(row['mean_target_value_rank'] or 0.0):.2f} | {(row['mean_donor_value_rank'] or 0.0):.2f} | "
                    f"{(row['mean_target_minus_donor'] or 0.0):.3f} | {(row['mean_target_minus_prose'] or 0.0):.3f} | "
                    f"{row['mean_n_channels']:.1f} | {row['patched_best_other_route']} |"
                )
            lines.append("")
    (OUT_ROOT / "phase706_cross_model_summary.md").write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(json.dumps(payload, ensure_ascii=False, indent=2, sort_keys=True), flush=True)
    return payload
