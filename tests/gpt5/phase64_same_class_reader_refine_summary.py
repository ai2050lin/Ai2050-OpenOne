...
def load_model(input_dir: Path, model: str) -> dict[str, Any] | None:
    path = input_dir / f"{model}_phase64_same_class_reader_refine.json"
    if not path.exists():
        shard_paths = sorted(input_dir.glob(f"{model}_phase64_same_class_reader_refine_shard*.json"))
        if not shard_paths:
            return None
        ...
        from phase64_same_class_reader_refine import summarize  # type: ignore
...
def main() -> None:
    ...
    lines: list[str] = ["# Phase64 Same Class Reader Refine Summary", ""]
    for model in MODELS:
        data = loaded[model]
        if data is None:
    ...
    (output_dir / "phase64_same_class_reader_refine_summary.json").write_text(
        json.dumps(summary, ensure_ascii=False, indent=2), encoding="utf-8"
    )
    (output_dir / "PHASE64_SAME_CLASS_READER_REFINE_SUMMARY.md").write_text(
        "\n".join(lines) + "\n", encoding="utf-8"
    )
    print(f"wrote {output_dir / 'phase64_same_class_reader_refine_summary.json'}")
    print(f"wrote {output_dir / 'PHASE64_SAME_CLASS_READER_REFINE_SUMMARY.md'}")
