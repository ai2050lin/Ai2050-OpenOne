#!/usr/bin/env python3
"""Summarize Phase 144 dirty-clean contrast and container re-audit tests."""
...
def load_result(model: str) -> dict[str, Any] | None:
    path = OUT_ROOT / f"phase144_{model}_dirty_clean_contrast_container.json"
    if not path.exists():
        return None
...
def main() -> None:
    ...
    lines = ["# Phase 144 Cross-model Dirty-Clean Contrast Container Summary", ""]
    for model in MODELS:
        ...
        for key, item in result["category_results"].items():
            ...
            lines.append(
                ...
                f"{fmt(item['best_constrained_clean'])} | {fmt(item['best_support'])} | "
                f"{fmt(item.get('best_contrast_joint'))} |"
            )
        lines.append("")
    path = OUT_ROOT / "phase144_cross_model_summary.md"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(path)
