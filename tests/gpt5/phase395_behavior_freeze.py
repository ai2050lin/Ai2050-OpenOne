#!/usr/bin/env python3
"""Freeze Phase395 natural binding splits without behavior backfilling."""
...
SALT = "phase395-natural-binding-freeze-v1"
...
def main() -> None:
    cases = read_jsonl(OUT / "protocol/private/phase395_candidate_cases.jsonl")
    case_by_id = {row["blind_case_id"]: row for row in cases}
    if len(case_by_id) != 864:
        raise RuntimeError(f"Expected 864 Phase395 cases, got {len(case_by_id)}")
    behavior: list[dict[str, Any]] = []
    for model in MODELS:
        complete = read_json(OUT / "behavior" / model / "complete.json")
        if not complete["valid"] or complete["execution_batch_size"] != 1:
            raise RuntimeError(f"Invalid Phase395 behavior output for {model}")
        behavior.extend(read_jsonl(OUT / "behavior/private" / model / "rows.jsonl"))
    if len(behavior) != 864:
        raise RuntimeError(f"Expected 864 Phase395 behavior rows, got {len(behavior)}")
        ...
        enriched.append(
            {
                **row,
                ...
                "natural_binding_behavior_qualified": bool(
    ...
    qualified_groups = {
        ...
        and all(row["natural_binding_behavior_qualified"] for row in rows)
    }
    ...
    for surface in eligible_surfaces:
        ...
        for split, count in SPLIT_COUNTS.items():
            ...
            for group_id in split_groups:
                assignment[group_id] = (split, "p395g_" + digest(f"{SALT}:{group_id}", 24))
            cursor += count
    ...
    for row in enriched:
        ...
        selected_cases.append(
            {
                **case,
                "schema_version": "69.2.0",
                "phase_id": "Phase395-FrozenDenominator",
                "phase395_split": split,
                "phase395_public_parallel_group_id": public_group,
    ...
    expected_selected = len(eligible_surfaces) * 24 * len(CONDITIONS) * len(MODELS)
    if len(selected_cases) != expected_selected:
        raise RuntimeError(
            f"Phase395 selected case mismatch: {len(selected_cases)} != {expected_selected}"
        )

    private = OUT / "protocol/private"
    write_jsonl(private / "phase395_frozen_execution_cases.jsonl", selected_cases)
    for split in SPLIT_COUNTS:
        write_jsonl(
            private / f"phase395_{split}_cases.jsonl",
            [row for row in selected_cases if row["phase395_split"] == split],
        )
    instrument_groups = {
        selected[surface]["discovery"][0] for surface in eligible_surfaces
    }
    instrument_cases = [
        {
            **row,
            "phase395_collection_stage": "instrument_audit",
            "instrument_reuse_scope": "engineering_conservation_only",
        }
        for row in selected_cases
        if row["anonymous_parallel_group_id"] in instrument_groups
    ]
    write_jsonl(private / "phase395_instrument_audit_cases.jsonl", instrument_cases)
    write_jsonl(
        OUT / "protocol/phase395_frozen_blind_groups.jsonl",
        [
            {
                "schema_version": "69.2.0",
                "phase_id": "Phase395-FrozenDenominator",
    ...
    summary = {
        "schema_version": "69.2.0",
        "phase_id": "Phase395-BehaviorFreeze",
        "created_at": now(),
        "denominator": {
        ...
        "claim_boundary": {
            "behavior_qualified_surface_is_binding_state": False,
            "excluded_surface_has_no_binding": False,
            "language_encoding_closed": False,
        },
    }
    write_json(OUT / "phase395_behavior_freeze_summary.json", summary)
