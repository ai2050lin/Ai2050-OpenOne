...
MODEL="${1:?usage: $0 MODEL [phase290 args...]}"
shift || true

export OPENONE_CONSERVATIVE_ENV="${OPENONE_CONSERVATIVE_ENV:-openone-cuda121}"
export OUTPUT_DIR="${OUTPUT_DIR:-results/gpt5_phase290_contract_break_scan}"
...
RUN_ID="$(date +%Y%m%d_%H%M%S)_phase290_${MODEL}"
...
echo "=== Phase290 conservative logged run ==="
date '+%Y-%m-%d %H:%M:%S %Z'
echo "run_id=${RUN_ID}"
echo "model=${MODEL}"
...
PY
    ...
    ps -eo pid,ppid,etime,stat,pcpu,pmem,wchan:32,cmd | grep -E 'phase290_contract_break_scan|python tests/gpt5|nvidia-smi|cuda|ComfyUI|python main.py' | grep -v grep || true
...
(
  while true; do
    ...
    ps -eo pid,etime,stat,pcpu,pmem,wchan:28,cmd | grep -E 'phase290_contract_break_scan|python tests/gpt5|nvidia-smi|cuda|python main.py' | grep -v grep || true
    sleep 10
  done
...
PYTHONUNBUFFERED=1 timeout --foreground --kill-after=30s "${MAX_SECONDS}s" \
  python tests/gpt5/phase290_contract_break_scan.py "$MODEL" \
    --output-dir "$OUTPUT_DIR" \
    --hard-exit-after-model \
    "$@"
RC="$?"
set -e
echo "phase290_exit_code=${RC}"
