#!/usr/bin/env python3
"""Phase449 v2 interface protocol freeze for GLM4 knowledge retest.

No model run, no CUDA, no physical trace. This script generates a small retest
manifest that removes Phase448's high-risk surface transforms.
"""
...
OUT_DIR = ROOT / "tests" / "gpt5" / "result" / "phase449_v2_interface_protocol"
PROTOCOL_PATH = OUT_DIR / "phase449_v2_protocol.json"
SAMPLES_PATH = OUT_DIR / "phase449_v2_glm4_knowledge_retest_samples.jsonl"
AUDIT_PATH = OUT_DIR / "phase449_v2_static_audit.json"
MANIFEST_PATH = OUT_DIR / "phase449_v2_manifest.json"
...
def select_rows(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    selected = [
        ...
        and row["split"] == "behavior_discovery"
...
def build_samples(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        item = {
            "sample_id": stable_hash("phase449", row["sample_id"]),
            ...
            "ability": row["ability"],
            "task": row["task"],
            "split": "phase449_v2_interface_retest",
...
def audit(samples: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "phase449_v2_static_audit.v1",
...
def main() -> None:
    ...
    source_rows = select_rows(load_jsonl(PHASE446_SAMPLES))
    samples = build_samples(source_rows)
    protocol = {
        "schema_version": "phase449_v2_interface_protocol.v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "status": "v2_interface_protocol_frozen_no_model_run",
        "target_model": "glm4",
        "target_task": "knowledge_network/relation_truth_judgment",
        "source": "phase446_behavior_discovery_first_32_pairs",
        "pairs": PAIRS,
        "surface_transforms": [
            "v2_lexical_frame",
            "v2_boundary_bullets",
        ...
        "removed_high_risk_transforms": ["boundary_rewrite", "order_rewrite", "query_rewrite"],
        "physical_trace_authorized": False,
    }
    ...
    manifest = {
        "schema_version": "phase449_v2_manifest.v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
