#!/usr/bin/env python3
"""Replicate Phase376 decision-aligned residual swaps on sealed calibration cases."""
...
FREEZE = OUT / "phase377_calibration_execution_freeze.json"
...
def model_groups(model: str) -> tuple[dict[str, dict[str, dict[str, Any]]], dict[str, str]]:
    ...
    if len(rows) != 44 or any(set(values) != {"A", "B", "C", "D"} for values in groups.values()):
        raise RuntimeError(f"Invalid Phase377 cases for {model}")
...
def process_model(model: str) -> dict[str, Any]:
    ...
    if not protocol["authorization"]["run_calibration_interventions"] or not freeze["valid"]:
        raise RuntimeError("Phase377 execution not authorized")
    templates = {
    ...
    try:
            ...
            for donor_condition, recipient_condition, transfer_name, transfer_class in TRANSFER_PAIRS:
                ...
                for template_name, template in templates.items():
                    ...
                    row = {
                        ...
                        "phase_id": "Phase377-CalibrationIntervention",
                        "model": model,
                    ...
                    rows.append(row)
            print(f"[{model}] calibration groups {group_index}/{len(groups)}", flush=True)
        private_dir = OUT / "phase377_intervention/models" / model / "private"
        write_jsonl(private_dir / "phase377_intervention_rows.jsonl", rows)
        ...
        candidates = [
            {
                ...
                "calibration_gate_pass": count >= minimum,
                "replicated_winner_flip_pass": winner_counts[key] >= minimum,
            }
            for key, count in sorted(pass_counts.items())
            if count >= minimum
        ]
        write_jsonl(private_dir / "phase377_group_gate_rows.jsonl", group_gates)
        write_jsonl(
            OUT / "phase377_intervention/models" / model / "phase377_model_candidates.jsonl",
            candidates,
        )
        summary = {
            "schema_version": "50.1.0",
        ...
        write_json(
            OUT / "phase377_intervention/models" / model / "phase377_model_summary.json",
            summary,
        )
        ...
        cross_rows.append(
            {
                ...
                "relative_depth": "late",
                "template": key[1],
                "calibration_models": sorted(models),
                "winner_flip_models": sorted(winner_models),
                "heterogeneous_level2_calibration_pass": level2,
                "level3_calibration_pass": level3,
                "physical_confirmation_completed": False,
                "language_mechanism_claimed": False,
            }
        )
    level2_rows = [row for row in cross_rows if row["heterogeneous_level2_calibration_pass"]]
    summary = {
        "schema_version": "50.2.0",
        ...
        "results": {
            "heterogeneous_level2_calibration_count": len(level2_rows),
            "level3_calibration_count": sum(
                row["level3_calibration_pass"] for row in cross_rows
            ),
            "physical_holdout_authorized": bool(level2_rows),
        ...
        "authorization": {
            "freeze_physical_confirmation_protocol": bool(level2_rows),
            "open_physical_before_protocol_freeze": False,
            "single_neuron_scan": False,
        },
        "next_decision": (
            "freeze_narrow_physical_confirmation_protocol"
            if level2_rows
            else "close_late_residual_carrier_replication"
        ),
    }
    write_jsonl(OUT / "phase377_intervention/phase377_cross_model_rows.jsonl", cross_rows)
    write_json(OUT / "phase377_intervention/phase377_calibration_summary.json", summary)
    print(json.dumps(summary, ensure_ascii=False, indent=2))
    return summary
