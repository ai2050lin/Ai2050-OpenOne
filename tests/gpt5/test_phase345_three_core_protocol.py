...
RESULT = ROOT / "tests/gpt5/result/phase345_three_core_protocol/three_core_protocol_qualification"
...
class Phase345ThreeCoreProtocolTests(unittest.TestCase):
    def test_denominator(self) -> None:
        cases = read_jsonl(RESULT / "phase345_registered_cases.jsonl")
        self.assertEqual(len(cases), 2592)
        self.assertEqual(len({row["case_id"] for row in cases}), 2592)
        self.assertEqual(len({row["mechanism_id"] for row in cases}), 12)
    ...
    def test_counts(self) -> None:
        validation = json.loads((RESULT / "phase345_case_bank_validation.json").read_text())
        ...
        self.assertEqual(validation["split_case_count"], {
            "discovery": 1296, "calibration": 540,
            "heldout": 432, "private_heldout": 324,
        })

    def test_execution_boundary(self) -> None:
        path = RESULT / "phase345_global_summary.json"
        if not path.exists():
            self.skipTest("Phase345 execution has not completed")
        ...
        self.assertEqual(len(read_jsonl(RESULT / "phase345_protocol_nodes.jsonl")), 36)
