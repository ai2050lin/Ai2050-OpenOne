#!/usr/bin/env python3
"""Phase463 GLM4 bridge behavior replicate."""
...
OUT_DIR = ROOT / "tests" / "gpt5" / "result" / "phase463_glm4_bridge_behavior"
GENERATIONS_PATH = OUT_DIR / "phase463_glm4_bridge_generations.jsonl"
SUMMARY_PATH = OUT_DIR / "phase463_glm4_bridge_summary.json"
...
def build_summary(records: list[dict[str, Any]]) -> dict[str, Any]:
    return {
        "schema_version": "phase463_glm4_bridge_behavior.v1",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "status": "bridge_behavior_complete_no_physical_trace",
        "model": "glm4",
        "target": "knowledge_network/bridge_marker_truth",
        "physical_collection_performed": False,
        "cuda_used": torch.cuda.is_available(),
        "model_weights_loaded": True,
        "overall": summarize(records),
        "core_track": track_summary(records, CORE_TRANSFORMS),
        "stress_track": track_summary(records, STRESS_TRANSFORMS),
        "authorization": {
            "physical_trace_authorized": False,
            "next_step": "phase464_bridge_failure_or_physical_precheck",
        },
    }
