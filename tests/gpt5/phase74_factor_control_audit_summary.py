...
def main() -> None:
    ...
    lines = ["# Phase74 Factor Control Audit Summary", ""]
    for model in MODELS:
        path = input_dir / f"{model}_phase74_factor_control_audit.json"
        lines += [f"## {model}", ""]
        if not path.exists():
            lines += ["missing", ""]
            continue
        ...
        by_control_relation_path = sorted_entries(sm.get("by_control_relation_path", {}), 100)
        summary["models"][model] = {
            ...
            "top_control_relation_paths": by_control_relation_path,
        }
        lines += [
        ...
        add_table(lines, "Top control relation paths", by_control_relation_path, 40)

    out_json = output_dir / "phase74_factor_control_audit_summary.json"
    out_md = output_dir / "PHASE74_FACTOR_CONTROL_AUDIT_SUMMARY.md"
