#!/usr/bin/env python3
"""Phase 964 GLM4 minimal: Task2(direct EOS inject) + Task1(last 3 layers diff) + diagnostic."""
...
from phase964_forward_diff import (
    ...
    EN_PROMPTS_50, MAX_TOKENS, RESULT_DIR
)

model_name = "glm4"

def run():
    log(f"Phase 964 GLM4 minimal")
    ...
    # ---- Diagnostic: print base EOS logit ----
    log("  Diagnostic: base EOS logit values...")
    for pi, prompt in enumerate(prompts):
        ...
        top1_logit = float(logits[top5_idx[0]])
        log(f"    p{pi} '{prompt[:25]}': EOS_logit={eos_logit:.3f}  top1={top1_logit:.3f}  "
            f"gap={top1_logit - eos_logit:.3f}  top5={[(tokenizer.decode([int(i)]), float(logits[i])) for i in top5_idx]}")
        results.setdefault("diagnostics", []).append({
            "prompt": prompt, "eos_logit": eos_logit, "top1_logit": top1_logit,
    ...
    inject_conditions = [("normal", 0, 0), ("b=5", 5, 0), ("b=10", 10, 0),
                         ("b=20", 20, 0), ("b=50", 50, 0), ("delayed2_b=20", 20, 2)]
    all_r2 = []
    for pi, prompt in enumerate(prompts):
        for cn, bias, delay in inject_conditions:
            ...
            log(f"    p{pi} {cn}: eos={he} clean={ce['strict_clean']} toks={ng} text={gen[:50]}")
    ...
    # ---- Task 1: Head diff search (last 3 layers only) ----
    log("  Task 1: Head diff search (last 3 layers)...")
    search_layers = list(range(max(0, n_layers - 3), n_layers))
    log(f"    Layers {search_layers[0]}-{search_layers[-1]}")
    ...
    results["task1"] = {"search_layers": search_layers,
        ...
        "all_heads": head_diff}
    log(f"    Top EOS promoters (ablate decreases EOS):")
    for k, ed in [(k, v.get("EOS", 0)) for k, v in eos_promoters if v.get("EOS", 0) < -0.01][:10]:
        v = head_diff[k]
        log(f"      {k}: ΔEOS_ablate={ed:.4f}  Δperiod={v.get('.', 0):.4f}")
    log(f"    Top EOS suppressors (ablate increases EOS):")
