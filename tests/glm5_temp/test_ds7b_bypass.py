...
方法：meta device创建模型 + safetensors直接加载权重 + accelerate dispatch
"""
import sys
...
def load_ds7b_bypass():
    ...
    for shard_file in shard_files:
        ...
        print(f"    Loading {shard_file} ({os.path.getsize(shard_path)/1e9:.2f}GB)...")
        shard_state = load_file(shard_path, device="cpu")
        # 转换为bfloat16
        for k, v in shard_state.items():
            state_dict[k] = v.to(torch.bfloat16)
        print(f"    Loaded {len(shard_state)} tensors, total={len(state_dict)}")
    print(f"    All weights loaded in {time.time()-t0:.1f}s, {len(state_dict)} tensors")

    # 5. 将权重加载到meta模型
    print("[5] Loading state dict into model...")
    t0 = time.time()
    # 需要将meta tensor替换为实际tensor
    model.load_state_dict(state_dict, strict=True, assign=True)
    print(f"    State dict loaded in {time.time()-t0:.1f}s")
    del state_dict
    gc.collect()

    # 6. 计算device_map：GPU放不下全部，部分留在CPU
    print("[6] Computing device map (GPU+CPU)...")
    gpu_mem = torch.cuda.get_device_properties(0).total_memory
    print(f"    GPU memory: {gpu_mem/1e9:.1f} GB")

    # 估算模型大小
    model_size = sum(p.numel() * p.element_size() for p in model.parameters()) 
    print(f"    Model size (bf16): {model_size/1e9:.2f} GB")

    # 用accelerate自动分配
    max_gpu = int(gpu_mem * 0.85)  # 保留15%给CUDA开销
    ...
    print(f"    Device: {device}, GPU allocated: {gpu_alloc:.2f} GB")
