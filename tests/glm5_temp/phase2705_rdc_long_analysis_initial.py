...
def decode_full_vocab(rows,predictions,truth_h,test):
    ...
    weight=checkpoint('lm_head.weight').float().cuda();gamma=checkpoint('model.norm.weight').float().cuda()
...
def main():
    ...
    immutable(OUT/'analysis_protocol.json',{'phase':2705,'source_sha':sha(Path(__file__)),'estimator_sha':sha(ROOT/'tests/glm5/rdc_conditional_estimators.py'),
