...
case "${ROUND_NAME}" in
  smoke)
    ...
    PREDICTORS="category_nonresidual_else_count_nonnegative,category_count_rank_improved,category_count_fullspan_margin_improved,category_count_fullspan_contrast_cleared,oracle_route_target_only"
    ;;
  main)
    MAX_GROUPS=16
    MAX_PAIRS=6
    ...
    PREDICTORS="category_nonresidual_else_count_nonnegative,category_count_rank_improved,category_count_fullspan_margin_improved,category_count_fullspan_rank_improved,category_count_fullspan_closure,category_count_fullspan_or_rank_improved,category_count_fullspan_contrast_cleared,category_count_fullspan_generic_cleared,oracle_route_target_only"
    ;;
  confirm)
    MAX_GROUPS=20
    MAX_PAIRS=8
...
for MODEL in qwen3 glm4 deepseek7b; do
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 836 ${ROUND_NAME}: start ${MODEL}"
  python tests/glm5/phase836_full_span_causal_blocker_profile.py \
    --model "${MODEL}" \
    "${COMMON_ARGS[@]}"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 836 ${ROUND_NAME}: done ${MODEL}"
done
