...
case "${ROUND_NAME}" in
  smoke)
    ...
    MAX_NEW_TOKENS=6
    PREDICTORS="signed_sum_positive,positive_count_majority,top_abs_positive"
    ;;
  main)
    MAX_GROUPS=16
    MAX_PAIRS=8
...
for MODEL in qwen3 glm4 deepseek7b; do
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 831 ${ROUND_NAME}: start ${MODEL}"
  python tests/glm5/phase831_internal_gate_predictor_search.py \
    --model "${MODEL}" \
    "${COMMON_ARGS[@]}"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 831 ${ROUND_NAME}: done ${MODEL}"
done
