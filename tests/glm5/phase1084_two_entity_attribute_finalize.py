#!/usr/bin/env python3
"""Finalize Phase1084 targeted descriptive evidence."""
...
if __name__ == "__main__":
    # The source finalizer interpolates each component/role to a shared depth
    # grid.  Restrict that grid to the independently preregistered middle band.
    ...
    automatic = {
        ...
        "reason": (
            "A full-role/full-depth map requires every preregistered protocol, "
            "behavior, repeatability, transfer, carrier-advantage, control-ratio, "
            "and numerical gate. Failed gates preserve descriptive evidence but "
            "cannot be bypassed by post-hoc region selection."
        ),
    }
    automatic["automatic_next_digest"] = protocol.digest(automatic)
    protocol.write_json(analysis_root / "automatic_next.json", automatic)
    final["schema_version"] = "phase1084_final_summary.v1"
