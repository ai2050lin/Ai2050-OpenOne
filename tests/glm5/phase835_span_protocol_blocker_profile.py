#!/usr/bin/env python3
from __future__ import annotations

import argparse
import gc
import json
import sys
from datetime import datetime
from pathlib import Path
from typing import Any

import torch

sys.stdout.reconfigure(encoding="utf-8")
ROOT = Path(__file__).resolve().parents[2]
sys.path.insert(0, str(ROOT / "tests" / "glm5"))
sys.path.insert(0, str(ROOT / "tests" / "gpt5"))

import phase816_multi_token_answer_span_rollout_closure as p816  # noqa: E402
import phase831_internal_gate_predictor_search as p831  # noqa: E402
import phase833_internal_route_boundary_predictor_search as p833  # noqa: E402
import phase834_blocker_aware_internal_route_boundary_predictor as p834  # noqa: E402
import phase828_cross_component_consistency_fiber_composition as p828  # noqa: E402
import phase829_non_interference_constrained_component_composition as p829  # noqa: E402


PHASE = 835
RESULT_ROOT = Path("tests/result/phase835_span_protocol_blocker_profile")
TARGET_CLASSES = {"target_equivalent"}


def log(msg: str) -> None:
    p829.log(msg)


def predictor_modes(args: argparse.Namespace) -> list[str]:
    return [x.strip() for x in str(args.predictor_modes).split(",") if x.strip()]


def compact_span(row: dict[str, Any] | None) -> dict[str, Any] | None:
    return p816.compact_span(row)


def score_candidates_with_first_logits(
    tokenizer,
    candidates: list[dict[str, Any]],
    baseline_scored: list[dict[str, Any]],
    first_logits: torch.Tensor,
    top_k: int,
) -> list[dict[str, Any]]:
    by_key: dict[tuple[int, ...], dict[str, Any]] = {
        tuple(int(x) for x in row["token_ids"]): row for row in baseline_scored
    }
    log_probs = torch.log_softmax(first_logits.detach().float().cpu(), dim=-1)
    out: list[dict[str, Any]] = []
    for cand in candidates:
        key = tuple(int(x) for x in cand["token_ids"])
        base = by_key.get(key)
        if not base or not cand["token_ids"]:
            continue
        first_id = int(cand["token_ids"][0])
        first_logit = float(first_logits[first_id].item())
        first_logprob = float(log_probs[first_id].item())
        first_rank_above = int((first_logits > first_logit).sum().item())
        base_logs = list(base.get("token_logs") or [])
        if not base_logs:
            continue
        base_first = base_logs[0]
        rest_logs = base_logs[1:]
        sum_logprob = float(base["score_sum_logprob"]) - float(base_first["logprob"]) + first_logprob
        sum_logit = float(base["score_sum_logit"]) - float(base_first["logit"]) + first_logit
        max_rank_above = max([first_rank_above, *[int(x.get("rank_above") or 0) for x in rest_logs]])
        step_all_top1 = first_rank_above == 0 and all(int(x.get("rank_above") or 0) == 0 for x in rest_logs)
        token_logs = [
            {
                "step": 0,
                "token_id": first_id,
                "token_text": tokenizer.decode([first_id], skip_special_tokens=False),
                "logit": first_logit,
                "logprob": first_logprob,
                "rank_above": first_rank_above,
                "top_tokens": p816.token_top_rows(tokenizer, first_logits, top_k),
            },
            *rest_logs,
        ]
        item = dict(cand)
        item.update(
            {
                "score_sum_logprob": sum_logprob,
                "score_mean_logprob": sum_logprob / max(len(cand["token_ids"]), 1),
                "score_sum_logit": sum_logit,
                "score_mean_logit": sum_logit / max(len(cand["token_ids"]), 1),
                "step_all_top1": step_all_top1,
                "max_rank_above": max_rank_above,
                "token_logs": token_logs,
            }
        )
        out.append(item)
    out.sort(key=lambda x: (float(x["score_mean_logprob"]), float(x["score_sum_logprob"])), reverse=True)
    return out


def span_profile(scored: list[dict[str, Any]]) -> dict[str, Any]:
    best_target = p816.best_by_class(scored, "target")
    best_contrast = p816.best_by_class(scored, "contrast")
    best_generic = p816.best_by_class(scored, "generic_blocker")
    non_target = [x for x in scored if x.get("candidate_class") != "target"]
    best_non_target = non_target[0] if non_target else None
    target_rank = None
    if best_target:
        key = tuple(int(x) for x in best_target.get("token_ids") or [])
        for idx, row in enumerate(scored, 1):
            if tuple(int(x) for x in row.get("token_ids") or []) == key:
                target_rank = idx
                break

    def score(row: dict[str, Any] | None) -> float | None:
        return None if not row else p816.finite(row.get("score_mean_logprob"))

    target_score = score(best_target)
    non_target_score = score(best_non_target)
    contrast_score = score(best_contrast)
    generic_score = score(best_generic)
    margin_non = None if target_score is None or non_target_score is None else target_score - non_target_score
    margin_contrast = None if target_score is None or contrast_score is None else target_score - contrast_score
    margin_generic = None if target_score is None or generic_score is None else target_score - generic_score
    return {
        "best_target": compact_span(best_target),
        "best_non_target": compact_span(best_non_target),
        "best_contrast": compact_span(best_contrast),
        "best_generic_blocker": compact_span(best_generic),
        "span_target_rank": target_rank,
        "span_target_margin_vs_non_target": margin_non,
        "span_target_margin_vs_contrast": margin_contrast,
        "span_target_margin_vs_generic": margin_generic,
        "span_score_closure": bool(margin_non is not None and margin_non > 0),
        "span_contrast_cleared": bool(margin_contrast is not None and margin_contrast > 0),
        "span_generic_cleared": bool(margin_generic is not None and margin_generic > 0),
        "span_target_step_all_top1": bool(best_target and best_target.get("step_all_top1")),
    }


def span_delta_features(prefix: str, base: dict[str, Any], patched: dict[str, Any]) -> dict[str, Any]:
    out: dict[str, Any] = {}
    for key in [
        "span_target_margin_vs_non_target",
        "span_target_margin_vs_contrast",
        "span_target_margin_vs_generic",
    ]:
        b = base.get(key)
        p = patched.get(key)
        out[f"{prefix}_{key}"] = p
        out[f"{prefix}_{key}_baseline"] = b
        out[f"{prefix}_{key}_improved"] = bool(p is not None and b is not None and float(p) > float(b))
    br = base.get("span_target_rank")
    pr = patched.get("span_target_rank")
    out[f"{prefix}_span_target_rank"] = pr
    out[f"{prefix}_span_target_rank_baseline"] = br
    out[f"{prefix}_span_target_rank_improved"] = bool(pr is not None and br is not None and int(pr) < int(br))
    out[f"{prefix}_span_score_closure"] = bool(patched.get("span_score_closure"))
    out[f"{prefix}_span_contrast_cleared"] = bool(patched.get("span_contrast_cleared"))
    out[f"{prefix}_span_generic_cleared"] = bool(patched.get("span_generic_cleared"))
    out[f"{prefix}_span_target_step_all_top1"] = bool(patched.get("span_target_step_all_top1"))
    out[f"{prefix}_best_target"] = patched.get("best_target")
    out[f"{prefix}_best_non_target"] = patched.get("best_non_target")
    return out


def base_category_count(features: dict[str, Any], group: dict[str, Any], donor_variant: str) -> bool:
    return p834.base_category_count(features, group, donor_variant)


def predict_gate(features: dict[str, Any], group: dict[str, Any], donor_variant: str, cls: str, mode: str) -> bool:
    base = base_category_count(features, group, donor_variant)
    if mode == "oracle_route_target_only":
        return cls in TARGET_CLASSES
    if mode == "category_nonresidual_else_count_nonnegative":
        return base
    if mode == "category_count_rank_improved":
        return base and bool(features.get("target_rank_improved"))
    if mode == "category_count_span_margin_improved":
        return base and bool(features.get("patch_span_target_margin_vs_non_target_improved"))
    if mode == "category_count_span_rank_improved":
        return base and bool(features.get("patch_span_target_rank_improved"))
    if mode == "category_count_span_closure":
        return base and bool(features.get("patch_span_score_closure"))
    if mode == "category_count_span_or_rank_improved":
        return base and (
            bool(features.get("target_rank_improved"))
            or bool(features.get("patch_span_target_margin_vs_non_target_improved"))
            or bool(features.get("patch_span_target_rank_improved"))
        )
    if mode == "category_count_span_and_rank_improved":
        return base and bool(features.get("target_rank_improved")) and bool(
            features.get("patch_span_target_margin_vs_non_target_improved")
        )
    if mode == "category_count_span_contrast_cleared":
        return base and bool(features.get("patch_span_contrast_cleared"))
    if mode == "category_count_span_generic_cleared":
        return base and bool(features.get("patch_span_generic_cleared"))
    raise ValueError(f"unknown predictor mode: {mode}")


def eval_pair(
    model,
    tokenizer,
    device: torch.device,
    standards: list[dict[str, Any]],
    case: dict[str, Any],
    pair: tuple[dict[str, Any], dict[str, Any]],
    args: argparse.Namespace,
) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    lookup = p828.p820.standard_lookup(standards)
    recipient_prompt = p828.p825.natural_prompt(case, args.recipient_prompt)
    recipient_ids = p831.p823.encode_prompt(tokenizer, recipient_prompt)
    baseline_text, baseline_ids = p831.p823.greedy_generate_with_subspace_patch(
        model, tokenizer, device, recipient_ids, args.max_new_tokens
    )
    baseline_boundary = p828.p825.boundary_for(lookup, case["case_id"], baseline_text)
    comp_data = p831.prepare_component_data(model, tokenizer, device, recipient_prompt, baseline_ids, case, pair, args)
    if not comp_data:
        return [], []

    candidates = p816.span_candidates(tokenizer, case, args)
    baseline_scored = p816.score_candidates(
        model, tokenizer, device, recipient_ids, candidates, int(args.batch_size), int(args.top_k)
    )
    baseline_span = span_profile(baseline_scored)
    target_id = comp_data[0]["readout_meta"].get("target_token_id") if comp_data else None
    baseline_id = int(baseline_ids[0]) if baseline_ids else None
    no_patch_logits = p834.first_step_logits(model, device, recipient_ids, [], float(args.alpha))
    no_patch_rank_profile = p834.rank_profile(no_patch_logits, target_id, baseline_id)

    a, b = pair
    modes = predictor_modes(args)
    decision_rows: list[dict[str, Any]] = []
    generation_rows: list[dict[str, Any]] = []
    for donor_variant in p828.parse_csv(args.search_donor_prompts):
        donor_prompt = p828.p825.natural_prompt(case, donor_variant)
        patch_items_by_mode: dict[str, list[dict[str, Any]]] = {mode: [] for mode in modes}
        active_labels_by_mode: dict[str, list[str]] = {mode: [] for mode in modes}
        for item in comp_data:
            group = item["group"]
            cls = p833.route_class(group, donor_variant)
            donor_state = p828.p822.capture_component_state(model, tokenizer, device, donor_prompt, int(group["layer_idx"]))
            donor_vec = p831.p823.component_vector(donor_state, item["spec"])
            if donor_vec is None:
                continue
            donor_vec = donor_vec.float().cpu()
            patch_item = {
                "layer_idx": int(group["layer_idx"]),
                "spec": item["spec"],
                "recipient_vec": item["recipient_vec"],
                "donor_vec": donor_vec,
                "selected_indices": item["selected_indices"],
            }
            features = p833.internal_features(
                item["recipient_vec"], donor_vec, item["effective_dir"], item["selected_indices"]
            )
            features.update(
                p834.blocker_features(
                    model,
                    tokenizer,
                    device,
                    recipient_ids,
                    patch_item,
                    args,
                    target_id,
                    baseline_id,
                    no_patch_rank_profile,
                )
            )
            patch_first_logits = p834.first_step_logits(model, device, recipient_ids, [patch_item], float(args.alpha))
            patched_scored = score_candidates_with_first_logits(
                tokenizer, candidates, baseline_scored, patch_first_logits, int(args.top_k)
            )
            patched_span = span_profile(patched_scored)
            features.update(span_delta_features("patch", baseline_span, patched_span))
            oracle_gate = cls in TARGET_CLASSES
            label = p828.compact_component_label(group)
            for mode in modes:
                pred = predict_gate(features, group, donor_variant, cls, mode)
                decision_rows.append(
                    {
                        "row_kind": "phase835_span_protocol_blocker_profile_decision",
                        "phase": PHASE,
                        "model": args.model,
                        "round": args.round_name,
                        "case_id": case["case_id"],
                        "object": case["object"],
                        "target_answer": case["answer"],
                        "pair_label": f"{p828.compact_component_label(a)} + {p828.compact_component_label(b)}",
                        "component_label_full": label,
                        "component_group": group,
                        "component_kind": p833.component_kind(group),
                        "donor_variant": donor_variant,
                        "predictor_mode": mode,
                        "uses_behavior_label_as_input": mode == "oracle_route_target_only",
                        "oracle_target_only_gate": bool(oracle_gate),
                        "predicted_gate": bool(pred),
                        "decision_correct_vs_oracle": bool(pred) == bool(oracle_gate),
                        "single_donor_class": cls,
                        "baseline_target_rank": no_patch_rank_profile.get("target_rank"),
                        "baseline_span": baseline_span,
                        "readout_meta": item.get("readout_meta"),
                        **features,
                    }
                )
                if pred:
                    patch_items_by_mode[mode].append(patch_item)
                    active_labels_by_mode[mode].append(label)

        for mode in modes:
            patched_text, patched_ids = p828.greedy_generate_with_multi_patch(
                model,
                tokenizer,
                device,
                recipient_ids,
                patch_items_by_mode[mode],
                args.max_new_tokens,
                args.alpha,
            )
            patched_boundary = p828.p825.boundary_for(lookup, case["case_id"], patched_text)
            delta_rank = int(patched_boundary["boundary_rank"]) - int(baseline_boundary["boundary_rank"])
            generation_rows.append(
                {
                    "row_kind": "phase835_span_protocol_blocker_profile_generation",
                    "phase": PHASE,
                    "model": args.model,
                    "round": args.round_name,
                    "source_round": args.source_round,
                    "case_id": case["case_id"],
                    "object": case["object"],
                    "target_answer": case["answer"],
                    "pair_label": f"{p828.compact_component_label(a)} + {p828.compact_component_label(b)}",
                    "component_a": a,
                    "component_b": b,
                    "budget": int(a["budget"]),
                    "donor_variant": donor_variant,
                    "predictor_mode": mode,
                    "validation_mode": "span_protocol_blocker_profile",
                    "active_component_labels": active_labels_by_mode[mode],
                    "n_active_components": len(active_labels_by_mode[mode]),
                    "baseline_generated": p828.p825.clean_generated(baseline_text),
                    "baseline_token_ids": baseline_ids,
                    "baseline_boundary_class": baseline_boundary.get("final_boundary_class"),
                    "baseline_boundary_rank": int(baseline_boundary["boundary_rank"]),
                    "patched_generated": p828.p825.clean_generated(patched_text),
                    "patched_token_ids": patched_ids,
                    "patched_boundary_class": patched_boundary.get("final_boundary_class"),
                    "patched_boundary_rank": int(patched_boundary["boundary_rank"]),
                    "delta_boundary_rank": delta_rank,
                    "improved_boundary": delta_rank > 0,
                    "degraded_boundary": delta_rank < 0,
                    "target_transition": patched_boundary.get("final_boundary_class") == "target_equivalent",
                    "single_a_exact_plus_multi": bool(a["single_exact_plus_multi"]),
                    "single_b_exact_plus_multi": bool(b["single_exact_plus_multi"]),
                    "single_a_natural_target_count": int(a["single_natural_target_count"]),
                    "single_b_natural_target_count": int(b["single_natural_target_count"]),
                    "n_components": 2,
                }
            )
    return decision_rows, generation_rows


def summarize_rows(
    decision_rows: list[dict[str, Any]],
    generation_rows: list[dict[str, Any]],
    groups: list[dict[str, Any]],
    pairs: list[tuple[dict[str, Any], dict[str, Any]]],
    diagnostics: dict[str, Any],
    args: argparse.Namespace,
    attn_impl: str | None = None,
) -> dict[str, Any]:
    base = p831.summarize_rows(decision_rows, generation_rows, groups, pairs, diagnostics, args, attn_impl)
    base["phase"] = PHASE
    base["title"] = "Span-Aware / Protocol-Aware Blocker Profile"
    base["boundary"] = (
        "This phase tests whether answer-span margins explain cases where first-token rank fails, especially DS7B."
    )
    return base


def run_model(args: argparse.Namespace) -> dict[str, Any]:
    out_dir = RESULT_ROOT / args.round_name
    out_dir.mkdir(parents=True, exist_ok=True)
    standards = p828.p820.standard_rows()
    groups = p828.load_component_groups(args.model, args)
    pairs, diagnostics = p829.build_constrained_pairs(groups, args)
    cmap = p828.p825.case_map()
    log(f"{args.model}/{args.round_name}: groups={len(groups)} pairs={len(pairs)} modes={predictor_modes(args)}")
    if args.dry_run:
        print(
            json.dumps(
                {
                    "model": args.model,
                    "diagnostics": diagnostics,
                    "pairs": [[p828.compact_component_label(a), p828.compact_component_label(b)] for a, b in pairs],
                },
                ensure_ascii=False,
                indent=2,
            )
        )
        return {"groups": groups, "pairs": pairs, "diagnostics": diagnostics}
    if not pairs:
        summary = summarize_rows([], [], groups, pairs, diagnostics, args, attn_impl=None)
        summary["skipped_model_load"] = True
        summary["skip_reason"] = "no eligible same-case pairs for span-aware blocker profile"
        p828.write_jsonl(out_dir / f"phase835_{args.model}_decision_rows.jsonl", [])
        p828.write_jsonl(out_dir / f"phase835_{args.model}_generation_rows.jsonl", [])
        p828.write_json(out_dir / f"phase835_{args.model}_summary.json", summary)
        print(json.dumps({"model": args.model, "round": args.round_name, "pairs": 0, "skipped_model_load": True}, ensure_ascii=False, indent=2))
        return summary

    model, tokenizer, device, attn_impl = p828.p796.load_model_bf16_prefer_flash(args.model, args.attn_implementations)
    if tokenizer.pad_token is None:
        tokenizer.pad_token = tokenizer.eos_token
    decision_rows: list[dict[str, Any]] = []
    generation_rows: list[dict[str, Any]] = []
    try:
        for idx, pair in enumerate(pairs, 1):
            case = cmap.get(str(pair[0]["case_id"]))
            if not case:
                continue
            drows, grows = eval_pair(model, tokenizer, device, standards, case, pair, args)
            decision_rows.extend(drows)
            generation_rows.extend(grows)
            if idx % int(args.log_every) == 0 or idx == len(pairs):
                log(f"{args.model}: evaluated {idx}/{len(pairs)} pairs; decisions={len(decision_rows)} generations={len(generation_rows)}")
    finally:
        p828.release_model(model)
        del tokenizer
        gc.collect()
        if torch.cuda.is_available():
            torch.cuda.empty_cache()

    summary = summarize_rows(decision_rows, generation_rows, groups, pairs, diagnostics, args, attn_impl)
    p828.write_jsonl(out_dir / f"phase835_{args.model}_decision_rows.jsonl", decision_rows)
    p828.write_jsonl(out_dir / f"phase835_{args.model}_generation_rows.jsonl", generation_rows)
    p828.write_json(out_dir / f"phase835_{args.model}_summary.json", summary)
    printable = {
        "model": args.model,
        "round": args.round_name,
        "pairs": summary["n_pairs"],
        "mode_summaries": {
            mode: {
                "accuracy": data["decision"].get("accuracy_vs_target_only"),
                "pair_exact_plus_multi": data["generation"].get("pair_exact_plus_multi"),
                "natural_target_rows": data["generation"].get("natural_target_rows"),
                "natural_degraded_rows": data["generation"].get("natural_degraded_rows"),
                "natural_category_target_rows": data["generation"].get("natural_category_target_rows"),
                "active": data["generation"].get("active_component_count_distribution"),
            }
            for mode, data in summary["mode_summaries"].items()
        },
    }
    print(json.dumps(printable, ensure_ascii=False, indent=2), flush=True)
    return summary


def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        f"# Phase 835 Span-Aware / Protocol-Aware Blocker Profile ({payload['round']})",
        "",
        "- Source: Phase 834 first-token blocker profile plus Phase 816 answer-span scoring.",
        "- Objective: test whether span margins explain cases where first-token rank fails.",
        "",
        "## Model Summary",
        "",
        "| model | predictor | decision acc | TP | TN | FP | FN | pair exact+multi | natural target | natural degraded | nat_category target | nat_category degraded | active components |",
        "|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    for model_name in p828.MODELS:
        data = payload.get("model_summaries", {}).get(model_name) or {}
        for mode, mode_data in (data.get("mode_summaries") or {}).items():
            dec = mode_data.get("decision") or {}
            gen = mode_data.get("generation") or {}
            acc = dec.get("accuracy_vs_target_only")
            acc_text = "" if acc is None else f"{float(acc):.3f}"
            lines.append(
                f"| {model_name} | `{mode}` | {acc_text} | {dec.get('tp')} | {dec.get('tn')} | "
                f"{dec.get('fp')} | {dec.get('fn')} | {gen.get('pair_exact_plus_multi')} | "
                f"{gen.get('natural_target_rows')} | {gen.get('natural_degraded_rows')} | "
                f"{gen.get('natural_category_target_rows')} | {gen.get('natural_category_degraded_rows')} | "
                f"`{json.dumps(gen.get('active_component_count_distribution') or {}, ensure_ascii=False)}` |"
            )
    lines += ["", "## Boundary", ""]
    lines.append(
        "Span-aware features are still target-aware probes. The phase is successful only if they explain DS7B without breaking qwen3."
    )
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")


def summarize_round(round_name: str) -> dict[str, Any]:
    out_dir = RESULT_ROOT / round_name
    payload: dict[str, Any] = {
        "phase": PHASE,
        "round": round_name,
        "status": "missing",
        "timestamp": datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
        "model_summaries": {},
        "models": [],
    }
    for model_name in p828.MODELS:
        path = out_dir / f"phase835_{model_name}_summary.json"
        if path.exists():
            payload["model_summaries"][model_name] = json.loads(path.read_text(encoding="utf-8"))
            payload["models"].append(model_name)
    payload["status"] = "complete" if len(payload["models"]) == len(p828.MODELS) else "partial"
    p828.write_json(out_dir / "phase835_cross_model_summary.json", payload)
    write_markdown(out_dir / "phase835_cross_model_summary.md", payload)
    return payload


def build_parser() -> argparse.ArgumentParser:
    parser = p829.build_parser()
    parser.add_argument(
        "--predictor-modes",
        default=(
            "category_nonresidual_else_count_nonnegative,category_count_rank_improved,"
            "category_count_span_margin_improved,category_count_span_rank_improved,"
            "category_count_span_closure,category_count_span_or_rank_improved,"
            "category_count_span_and_rank_improved,category_count_span_contrast_cleared,"
            "category_count_span_generic_cleared,oracle_route_target_only"
        ),
    )
    parser.add_argument("--max-span-candidates", type=int, default=48)
    parser.add_argument("--batch-size", type=int, default=16)
    parser.add_argument("--top-k", type=int, default=5)
    return parser


def main() -> None:
    args = build_parser().parse_args()
    if args.summarize_only:
        payload = summarize_round(args.round_name)
        print(json.dumps({"round": args.round_name, "status": payload["status"], "models": payload["models"]}, ensure_ascii=False, indent=2))
        return
    if not args.model:
        raise SystemExit("--model is required unless --summarize-only")
    run_model(args)


if __name__ == "__main__":
    main()
