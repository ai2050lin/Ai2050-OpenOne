#!/usr/bin/env python3
"""Freeze pair-specific Phase1108 behavior authorization."""
...
def main() -> None:
    ...
    models_by_pair = {
        pair: [
            ...
            if pair in result["passing_pairs"]
        ]
        for pair in protocol.RELATION_PAIRS
    }
    cross_model_pairs = [
    ...
    authorized_models = [
        model for model, result in models.items()
        if any(pair in cross_model_pairs for pair in result["passing_pairs"])
    ]
    hidden_scan_authorized = bool(cross_model_pairs) and len(authorized_models) >= 2
    result = {
        "schema_version": "phase1108_behavior_authorization.v1",
        ...
        "decision": (
            "run_phase1108_signed_event_map"
            if hidden_scan_authorized else "stop_at_behavior_gate"
        ),
        "authorization_scope": (
            "Only model-pair cells passing both independent splits are used in hidden analysis."
        ),
    }
