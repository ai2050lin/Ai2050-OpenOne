...
OUTPUT_DIR = os.environ.get("PHASE346_OUTPUT_DIR", "results/phase346_interaction_closure")
...
def load_model_bf16(model_name):
    ...
    impls = [
        item.strip()
        for item in os.environ.get("PHASE346_ATTN_IMPLEMENTATIONS", "flash_attention_2,sdpa,eager").split(",")
        if item.strip()
    ]
    for impl in impls:
        try:
...
def run_experiment(model_name):
    ...
    out_path = os.path.join(OUTPUT_DIR, f"{model_name}_phase346.json")
