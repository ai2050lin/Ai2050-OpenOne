#!/usr/bin/env python3
"""
Phase 554: cross-sample and cross-scaffold route restore audit.

Phase553 showed that GLM4 vehicle_tool remove drops can be restored by same
prompt/site activations.  This phase separates donor and recipient prompts:
same-object, cross-object, and cross-scaffold restores test whether the restored
signal is a category/paraphrase route or merely same-object surface state.
"""
...
OUT_ROOT = Path("results/glm5_phase554_cross_sample_route_restore")
...
DEFAULT_ROUTES = [
    ...
    "forbidden_definition:top_p<-forbidden_sentence_completion",
    "forbidden_natural_qa:top_p<-forbidden_sentence_completion",
    "forbidden_sentence_completion:temperature<-forbidden_natural_qa",
]
DEFAULT_CONDITIONS = [
    "baseline",
    "add_perp",
    ...
    "resid_same_base",
    "resid_same_add",
    "resid_cross_object_base",
    "resid_cross_object_add",
    "resid_cross_scaffold_base",
    "resid_cross_scaffold_add",
    "attn_cross_scaffold_add",
    "mlp_cross_scaffold_add",
]
...
def condition_plan(condition: str) -> dict[str, Any]:
    if condition == "baseline":
        return {"site": "none", "component": None, "remove": False, "add": False, "restore": None, "donor_relation": None}
    if condition == "add_perp":
        return {"site": "resid", "component": "residual_perp", "remove": False, "add": True, "restore": None, "donor_relation": None}
    if condition == "resid_remove_perp":
        return {"site": "resid", "component": "residual_perp", "remove": True, "add": False, "restore": None, "donor_relation": None}
    if condition == "resid_remove_random_perp":
        return {"site": "resid", "component": "random_perp", "remove": True, "add": False, "restore": None, "donor_relation": None}
    if condition == "attn_remove_perp":
        return {"site": "attn", "component": "residual_perp", "remove": True, "add": False, "restore": None, "donor_relation": None}
    if condition == "mlp_remove_perp":
        return {"site": "mlp", "component": "residual_perp", "remove": True, "add": False, "restore": None, "donor_relation": None}
    for site in ("resid", "attn", "mlp"):
        prefix = f"{site}_"
        if condition.startswith(prefix):
            tail = condition[len(prefix):]
            if tail == "same_base":
                return {"site": site, "component": "residual_perp", "remove": True, "add": False, "restore": "baseline", "donor_relation": "same"}
            if tail == "same_add":
                return {"site": site, "component": "residual_perp", "remove": True, "add": False, "restore": "add_perp", "donor_relation": "same"}
            if tail == "cross_object_base":
                return {"site": site, "component": "residual_perp", "remove": True, "add": False, "restore": "baseline", "donor_relation": "cross_object"}
            if tail == "cross_object_add":
                return {"site": site, "component": "residual_perp", "remove": True, "add": False, "restore": "add_perp", "donor_relation": "cross_object"}
            if tail == "cross_scaffold_base":
                return {"site": site, "component": "residual_perp", "remove": True, "add": False, "restore": "baseline", "donor_relation": "cross_scaffold"}
            if tail == "cross_scaffold_add":
                return {"site": site, "component": "residual_perp", "remove": True, "add": False, "restore": "add_perp", "donor_relation": "cross_scaffold"}
...
def compact_metrics(
    ...
    is_restore = any(x in condition for x in ("same_", "cross_object_", "cross_scaffold_"))
    if is_restore:
        if restore_gain >= 0.08 and label_delta <= 0.05:
            cls = "restore_success"
        elif restore_gain >= 0.04 and label_delta <= 0.08:
            cls = "weak_restore"
        else:
            cls = "restore_fail"
...
def donor_rows_for(
    ...
    if relation is None:
        return None
    if relation == "same":
        return prompt_sets[recipient_scaffold]
    if relation == "cross_object":
        return shifted_rows(prompt_sets[recipient_scaffold], 1)
    if relation == "cross_scaffold":
        return prompt_sets[donor_scaffold]
    raise ValueError(f"unknown donor relation: {relation}")
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if len(windows) != 1:
            raise ValueError(f"Phase554 expects one window, got {windows}")
        ...
        log(f"{args.model}: phase554 pair={pair}, combos={combos}, routes={[r['name'] for r in routes]}")
            ...
            for route in routes:
                ...
                for condition in conditions:
                    ...
                    donor_rows = donor_rows_for(
                        prompt_sets, recipient_scaffold, donor_scaffold, plan.get("donor_relation")
                    )
                    all_records = []
                    seed_rows = []
                    for seed in sample_seeds:
                        ...
                        for rec in records:
                            rec2 = {
                                ...
                                "mode": mode,
                                "condition": condition,
                                "donor_relation": plan.get("donor_relation") or "",
                                "seed": seed,
                                **rec,
                            }
                ...
                for condition, row in rows.items():
                    if condition == "baseline":
                        continue
                    if condition.startswith("attn_"):
                        remove_ref = rows.get("attn_remove_perp", base)
                    elif condition.startswith("mlp_"):
                        remove_ref = rows.get("mlp_remove_perp", base)
                    else:
                    ...
                    compact.append({
                        ...
                        "mode": mode,
                        "condition": condition,
                        "donor_relation": condition_plan(condition).get("donor_relation") or "",
                ...
                log(
                    ...
                    f"same={rows.get('resid_same_add', base)['clean_non_object_rate']:.2f}; "
                    f"xobj={rows.get('resid_cross_object_add', base)['clean_non_object_rate']:.2f}; "
                    f"xscaf={rows.get('resid_cross_scaffold_add', base)['clean_non_object_rate']:.2f}; "
                    f"attn_xscaf={rows.get('attn_cross_scaffold_add', base)['clean_non_object_rate']:.2f}; "
                    f"mlp_xscaf={rows.get('mlp_cross_scaffold_add', base)['clean_non_object_rate']:.2f}"
                )
        ...
        return {
            ...
            "restore_note": "same/cross_object/cross_scaffold restore writes dynamic donor activation at current last-token site after projection removal",
...
def write_tsv(result: dict[str, Any], out_dir: Path, model_name: str) -> None:
    fields = [
        ...
        "condition", "donor_relation", "seed", "object", "donor_object", "quality",
    ...
    path = out_dir / f"phase554_{model_name}_readable_samples.tsv"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_path = out_dir / f"phase554_{args.model}_cross_sample_route_restore.json"
