"""
Phase 402 Optimized: TYPE/SPEED Causal Decomposition (Optimized BF16)
=====================================================================

Optimized version with:
1. BF16 + device_map="auto" for all models
2. Flash attention disabled for compatibility
3. Progressive logging with timestamps
4. Memory monitoring and cleanup
5. Robust error handling

Usage:
  python tests/glm5/phase402_optimized.py qwen3
  python tests/glm5/phase402_optimized.py deepseek7b
  python tests/glm5/phase402_optimized.py glm4
"""
...
# Round 1: 8 objects covering fast/slow × 3 types (optimized for speed)
SPEED_OBJECTS = {
...
# Multi-candidate speed scale (8 candidates)
SPEED_CANDIDATES = [
...
def compute_direction_batch(model, tokenizer, layers_list, device, li, obj_name, obj_data, token_ids):
    """Compute speed direction efficiently with batch processing."""
...
def run_phase402_optimized(model_name):
    timestamp = time.strftime("%Y-%m-%d %H:%M")
    print(f"\n{'='*80}")
    print(f"=== Phase 402 Optimized: TYPE/SPEED Causal Decomposition ({model_name}) [{timestamp}] ===")
    print(f"{'='*80}")
    ...
    for li in layer_indices:
        ...
        for type_name, members in type_groups.items():
            ...
            print(f"    TYPE {type_name}: |dir|={np.linalg.norm(type_dir):.4f} members={len(members)}")
        ...
        print(f"    SPEED axis: |dir|={np.linalg.norm(speed_axis):.4f} fast={len(fast_objs)} slow={len(slow_objs)}")
        ...
        # Step section 5: Aggregate causal analysis
    ...
    out_path = out_dir / f"{model_name}_phase402_optimized.json"
...
if __name__ == "__main__":
    ...
    run_phase402_optimized(model_name)