#!/usr/bin/env python3
"""Run Phase1106 behavior replication for one local FP16 model."""
...
def run(model_name: str) -> None:
    ...
    if not audit["all_checks_passed"]:
        raise RuntimeError("Phase1106 protocol audit failed")
    ...
    try:
            ...
            for length in sorted(by_length):
                ...
                for start in range(0, len(panel), BATCH_SIZE[model_name]):
                    ...
                    for slot, row in enumerate(batch):
                        scores = {key: float(logits[slot, ids[0]].item()) for key, ids in row["candidate_first_token_ids"].items()}
                        ...
                        key = (str(row["split"]), str(row["template"]), str(row["route_type"]), str(row["congruence"]), str(row["target_relation"]))
                        totals[key] += 1
        ...
        summary = {
            "schema_version": "phase1106_behavior_summary.v1",
