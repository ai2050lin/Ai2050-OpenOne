...
for MODEL in qwen3 glm4 deepseek7b; do
  ...
  COMMON_ARGS=(
    ...
    --objective-canonical-surface-weight 8.0
    --objective-canonical-margin-weight 1.0
  ...
  echo "[$(date +%H:%M:%S)] phase813 $ROUND: start $MODEL cases=$MODEL_MAX_CASES combo_size=$MAX_COMBO_SIZE pool=$MAX_COMBO_CANDIDATES combos=$MAX_COMBOS"
  python tests/glm5/phase813_canonical_surface_token_selector.py \
    --model "$MODEL" \
    "${COMMON_ARGS[@]}"
  echo "[$(date +%H:%M:%S)] phase813 $ROUND: done $MODEL"
done
