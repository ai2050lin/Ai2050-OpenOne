...
for MODEL in qwen3 glm4 deepseek7b; do
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 828 ${ROUND_NAME}: start ${MODEL}"
  python tests/glm5/phase828_cross_component_consistency_fiber_composition.py \
    --model "${MODEL}" \
    "${COMMON_ARGS[@]}"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 828 ${ROUND_NAME}: done ${MODEL}"
done
