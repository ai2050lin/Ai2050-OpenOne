#!/usr/bin/env python3
"""Freeze exact-prompt held-out behavior calibration for Phase1072."""
...
PROMPT_STYLES = (0,)
STYLE_LABELS = {0: "frozen_exact_prompt_matrix"}
...
def calibration_names_for_case(
    ...
    ranked = sorted(
        ...
        key=lambda name: hashlib.sha256(
            f"phase1072-calibration|{key}|{name}".encode("utf-8")
        ).hexdigest(),
    )
    result = ranked[:6]
    if len(set(result)) != 6:
...
def audit_cases(
    ...
    checks = {
        ...
        "balanced_condition_query_template_path": all(
            ...
            for template in formal.TEMPLATES
            for path in PATH_NAMES.values()
        ),
        "only_reserved_calibration_names": all(
            set(row["cell_names"]) <= calibration_names
            for row in cases
        ),
        ...
        "prompt_skeleton_hash_present": all(
            len(str(row["prompt_skeleton_sha256"])) == 64
            for row in cases
        ),
    ...
    return {
        "schema_version": (
            "phase1072_calibration_protocol_model_audit.v1"
        ),
        "phase": PHASE,
        "model": model_name,
...
def build_protocol() -> dict[str, Any]:
    ...
    for model_name in MODELS:
        ...
        for condition in RELATION_NAMES:
            for query_type in QUERY_TYPES:
                for template_index in formal.TEMPLATES:
                    for path_index, (anchor, switch) in enumerate(
                        PATH_NAMES
                    ):
                        selection_key = (
                            f"{condition}|{query_type}|"
                            f"{template_index}|{anchor}|{switch}"
                        )
                        answer, lexical = selected_answer_lexical(
                            selection_key
                        )
                        state = (
                            f"a{anchor}_b{switch}_"
                            f"y{answer}_l{lexical}"
                        )
                        names = calibration_names_for_case(
                        ...
                        row["schema_version"] = (
                            "phase1072_calibration_case.v1"
                        )
                        row["prompt_style"] = 0
                        cases.append(row)
                        semantic_index += 1
        audit = audit_cases(
            model_name, cases, set(calibration_names)
        )
        if not audit["all_checks_passed"]:
            raise RuntimeError(
                f"Phase1072 calibration audit failed: "
                f"{model_name}: {audit}"
            )
        write_jsonl(
            CALIBRATION_ROOT
    ...
    payload = {
        "schema_version": (
            "phase1072_calibration_preregistration.v1"
        ),
        "phase": PHASE,
        ...
        "templates": list(formal.TEMPLATES),
        ...
        "selection_policy": (
            "No prompt is selected from hidden states or behavior. "
            "Natural and explicit branches are both frozen and carried "
            "unchanged into the formal protocol."
        ),
        "exact_transfer_definition": (
            "After replacing the six entity names by role-stable "
            "placeholders, every calibration prompt skeleton must have "
            "an identical formal prompt skeleton."
        ),
        "gates": {
            key: value
    ...
    write_json(
        ...
        {
            "schema_version": (
                "phase1072_calibration_protocol_audit.v1"
            ),
            "phase": PHASE,
...
def main() -> None:
    ...
    print(
        f"Phase1072 calibration frozen: "
