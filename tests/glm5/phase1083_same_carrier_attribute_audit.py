#!/usr/bin/env python3
"""Audit Phase1083 artifacts without changing frozen thresholds."""
...
def main() -> None:
    ...
    for model in protocol.MODELS:
        ...
        pilot = protocol.read_json(
            protocol.OUT_ROOT / "pilot" / f"{model}.json"
        )
        ...
        model_checks = {
            ...
            "pilot_protocol_digest": pilot["protocol_digest"]
            == prereg["protocol_digest"],
            "pilot_case_digest": pilot["case_digest"]
            == prereg["model_case_digests"][model],
            "pilot_behavior_gate": pilot["model_behavior_gate_passed"],
    ...
    required = (
        ...
        "automatic_next.json",
        "posthoc_carrier_diagnostic.json",
        "posthoc_scope_diagnostic.json",
        "top_regions.jsonl",
        "final_summary.json",
    )
    ...
    result = {
        "schema_version": "phase1083_result_audit.v1",
