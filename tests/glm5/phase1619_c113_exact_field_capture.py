#!/usr/bin/env python3
"""Phase1619 / C113: exact BF16 role-state field capture for the fourth lexicon."""
...
def main() -> None:
    ...
    if protocol["authorization"] != "execute_phase1619_c113_exact_field_capture" or not audit["all_checks_passed"]:
        raise RuntimeError("C113 capture authorization missing")
    if protocol["producer_sha256"] != core.sha(TESTS / "phase1618_c113_fourth_lexicon_role_lattice_contract.py"):
        raise RuntimeError("C113 frozen contract changed")
    ...
    if raw_path.exists() or logits_path.exists() or index_path.exists():
        raise RuntimeError("C113 raw archive already exists")
    ...
    try:
        ...
        for start in range(0, len(rows), batch_size):
            ...
            if (start // batch_size + 1) % 8 == 0:
                ...
                print(f"[phase1619] captured {start + len(batch)}/{len(rows)} cases", flush=True)
        ...
        for state_index, state in enumerate(output.hidden_states):
            for local in range(len(first_rows)):
                ...
                if not np.array_equal(old_bits, new_bits):
                    repeat_hidden = max(repeat_hidden, float(np.max(np.abs(capture_base.decode_bf16(old_bits) - capture_base.decode_bf16(new_bits)))))
        repeat_logits = 0.0
    ...
    for row_indices in by_unit.values():
        ...
        for row_index in row_indices[1:]:
            ...
            if not np.array_equal(left, right):
                causal_prefix = max(causal_prefix, float(np.max(np.abs(capture_base.decode_bf16(left) - capture_base.decode_bf16(right)))))
        ...
        for truth, surface, distractor in itertools.product((1, -1), repeat=3):
            ...
            for role in ("focus_pre", "focus_record", "focus_post", "query_focus", "query_anchor"):
                ...
                if not np.array_equal(left, right):
                    code_previsible = max(code_previsible, float(np.max(np.abs(capture_base.decode_bf16(left) - capture_base.decode_bf16(right)))))
    ...
    report = {
        "phase": 1619, "campaign": "C113", "created_at_utc": now(), "status": "fourth_lexicon_exact_bf16_field_capture_complete",
        ...
        "authorization": "run_phase1620_c113_field_adjudication",
    }
