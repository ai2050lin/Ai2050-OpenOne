...
RESULT_ROOT = Path("tests/result/phase813_canonical_surface_token_selector")
...
def objective(row: dict[str, Any], args: argparse.Namespace) -> float:
    ...
    canonical_margin = finite(row.get("after_canonical_surface_margin_vs_best_variant"), 0.0)
    return (
        float(args.objective_canonical_surface_weight) * variant_above
        ...
        - float(args.objective_canonical_margin_weight) * canonical_margin
    )
...
def label_solution(row: dict[str, Any], args: argparse.Namespace) -> str:
    ...
    if bool(row.get("after_answer_class_closure")) and bool(row.get("after_canonical_surface_closure")):
        return "canonical_surface_closure_no_strict_token"
    if bool(row.get("after_answer_class_closure")):
        if finite(row.get("after_surface_answer_variant_count_above_target")) > 0:
            if finite(row.get("canonical_surface_fragment_delta"), 0.0) < 0:
                return "canonical_surface_reducer_answer_closed"
    ...
    if finite(row.get("canonical_surface_fragment_delta"), 0.0) < 0:
        return "canonical_surface_reducer_answer_not_closed"
...
def row_from_eval(
    ...
    row: dict[str, Any] = {
        "row_kind": "phase813_canonical_surface_token_selector",
        "model": args.model,
    ...
    if not error:
        ...
        row["phase813_label"] = label_solution(row, args)
    else:
        row["objective_score"] = None
        row["phase813_label"] = "combo_error"
    row["phase813_boundary"] = (
        "This phase tests canonical surface token selection inside the answer equivalence class. "
        "A positive result requires preserving answer-class closure while making the target token beat all answer surface variants."
    )
    return row
...
def group_rows(rows: list[dict[str, Any]], fields: list[str]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row.get("row_kind") == "phase813_canonical_surface_token_selector" and not row.get("combo_error"):
            groups[tuple(row.get(f) for f in fields)].append(row)
    out: list[dict[str, Any]] = []
    for key, vals in groups.items():
        payload = {field: value for field, value in zip(fields, key)}
        labels = Counter(v.get("phase813_label") for v in vals)
        payload.update(
            {
                "n": len(vals),
...
def summarize(rows: list[dict[str, Any]], args: argparse.Namespace, attn_impl: str, routes: list[dict[str, Any]]) -> dict[str, Any]:
    valid = [r for r in rows if r.get("row_kind") == "phase813_canonical_surface_token_selector" and not r.get("combo_error")]
    best = sorted(
        valid,
        key=lambda r: (
            not bool(r.get("token_closure")),
            not bool(r.get("after_canonical_surface_closure")),
            not bool(r.get("after_answer_class_closure")),
            finite(r.get("after_surface_answer_variant_count_above_target"), 1e9),
    ...
    return {
        "phase": 813,
        "title": "Canonical Surface Token Selector",
        "model": args.model,
        "round": args.round_name,
        ...
        "best_rows": best,
        "by_label": dict(Counter(r.get("phase813_label") for r in valid)),
        ...
        "strict_boundary": (
            "This phase tests whether answer-class closure can be pushed into canonical surface token closure. "
            "The target token must beat answer surface variants while preserving answer-class closure."
        ),
    }
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_jsonl(out_dir / f"phase813_{args.model}_rows.jsonl", rows)
    write_json(out_dir / f"phase813_{args.model}_summary.json", summary)
    print(
        json.dumps(
            {
                "model": args.model,
...
def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        f"# Phase 813 Canonical Surface Token Selector ({payload['round']})",
        "",
    ...
    for model_name in MODELS:
        ...
        for row in data.get("best_rows", [])[:40]:
            ...
            lines.append(
                ...
                f"{fmt(row.get('after_canonical_surface_rank_within_answer_class'))} | "
                f"{fmt(row.get('after_canonical_surface_margin_vs_best_variant'))} | "
                f"{fmt(row.get('after_surface_answer_variant_count_above_target'))} | "
                f"{fmt(cm.get('class_weighted_delta'))} | "
                f"{int(bool(row.get('token_closure')))} | "
                f"{fmt(row.get('objective_score'))} | `{row.get('phase813_label')}` | `{items}` |"
            )
    lines += [
        "",
        "## By Label",
    ...
    for model_name in MODELS:
        ...
        lines.append(
            ...
            f"{data.get('answer_class_closure_rows')} | {data.get('canonical_surface_closure_rows')} | "
    ...
    for model_name in MODELS:
        ...
        for row in data.get("by_combo_size", [])[:40]:
            lines.append(
                ...
                f"{fmt(row.get('mean_canonical_surface_rank'))} | "
                f"{fmt(row.get('mean_canonical_surface_margin'))} | "
                f"{fmt(row.get('mean_surface_answer_variants_above_target'))} | "
                f"{fmt(row.get('mean_canonical_surface_fragment_delta'))} | "
...
def summarize_round(round_name: str) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {"phase": 813, "round": round_name, "status": "missing", "model_summaries": {}, "models": []}
    for model_name in MODELS:
        path = out_dir / f"phase813_{model_name}_summary.json"
        if not path.exists():
            continue
    ...
    write_json(out_dir / "phase813_cross_model_summary.json", payload)
    write_markdown(out_dir / "phase813_cross_model_summary.md", payload)
    return payload
