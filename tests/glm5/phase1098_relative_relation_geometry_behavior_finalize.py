#!/usr/bin/env python3
"""Finalize Phase1098 behavior authorization before hidden-state access."""
...
def main() -> None:
    ...
    for model_name in protocol.MODELS:
        ...
        for relation in protocol.RELATIONS:
            candidate_cells = [
                ...
                for surface in protocol.SURFACES for split in protocol.SPLITS for panel in protocol.PANELS
            ]
            generation_cells = [
                summary["per_generation_cell"]["|".join((relation, surface, split))]
                for surface in protocol.SURFACES for split in protocol.SPLITS
            ]
    ...
    result = {
        "schema_version": "phase1098_behavior_authorization.v1",
        "phase": protocol.PHASE,
        "protocol_digest": prereg["protocol_digest"],
        "models": model_records,
        "passing_models": passing_models,
        "hidden_scan_authorized": authorized,
        "decision": "run_relative_relation_geometry_scan" if authorized else "stop_before_hidden_scan_and_repair_behavior_base",
    }
