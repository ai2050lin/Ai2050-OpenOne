#!/usr/bin/env python3
"""Independent audit for Phase1522."""
...
OUT = TESTS / "result/phase1522_c089_unified_forward_capture"
sys.path.insert(0, str(TESTS))
import phase1331_relational_measurement_core as core


def finite_chunks(arr, chunk=24):
...
def main() -> None:
    ...
    groups = core.rows(OUT / "material/stratified_relation_sets.jsonl")
    arr = np.load(OUT / "raw/all_role_field.float16.npy", mmap_mode="r")
    py_compile.compile(str(TESTS / "phase1522_c089_unified_forward_capture.py"), doraise=True)
    checks = {
        "shape": list(arr.shape) == [360, 37, 4, 2560],
        ...
        "qualification": final["behavior_qualified_families"] == summary["behavior_qualified_families"],
        ...
        "authorization": final["authorization"] == "run_phase1523_c089_truth_contrast_atlas",
    }
    result = {"phase": 1522, "campaign": "C089", "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values())}
