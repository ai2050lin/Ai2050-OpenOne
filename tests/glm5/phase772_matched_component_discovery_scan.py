...
OUT_ROOT = Path("results/glm5_phase772_matched_component_discovery_scan")
RESULT_ROOT = Path("tests/result/phase772_matched_component_discovery_scan")
...
def control_head(layer: int, head: int, n_heads: int, offset: int) -> int:
    if n_heads <= 1:
        return head
                ...
                rows.append(
                    {
                        ...
                        "candidate_kind": "scan_top_component",
                        "selection": "direct_score_scan",
                    }
                )
...
def add_controls(
    ...
    out = list(selected)
    seen = {(r["layer"], r["head"], r["source_group"]) for r in out}
    for row in selected:
        attn = get_attention_module(get_layers(model)[row["layer"]])
        n_heads = get_num_heads(model, attn)
        h = control_head(row["layer"], row["head"], n_heads, args.control_offset)
...
def audit_case(
    ...
    selected = direct_rows[: args.top_components_per_case]
    ...
    for rank, row in enumerate(selected, 1):
        ...
        out.append(
            {
                "row_kind": "matched_component_discovery",
...
def summarize_rows(rows: list[dict[str, Any]], args: argparse.Namespace, model_name: str, attn_impl: str, scan_layers: list[int]) -> dict[str, Any]:
    return {
        "phase": 772,
        "title": "Matched Component Discovery Scan",
        "model": model_name,
        ...
        "top_components": group_summary(rows, ["component_key"])[:32],
        "strict_interpretation": "This is a direct-score discovery scan followed by top-component causal removal. It is head/source-level, not neuron/channel-level.",
    }
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        for idx, item in enumerate(selected, 1):
            ...
            if idx % args.log_every == 0 or idx == len(selected):
                log(f"{args.model}: component discovery {idx}/{len(selected)} cases; rows={len(rows)}")
    finally:
        release_model(model)
        del tokenizer
        gc.collect()
    ...
    for root in (out_dir, result_dir):
        write_jsonl(root / f"phase772_{args.model}_rows.jsonl", rows)
        write_json(root / f"phase772_{args.model}_summary.json", summary)
    print(json.dumps({"model": args.model, "round": args.round_name, "n_cases": summary["n_cases"], "top_components": summary["top_components"][:5]}, ensure_ascii=False, indent=2), flush=True)
    return summary


def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        f"# Phase 772 Matched Component Discovery Scan ({payload['round']})",
        "",
    ...
    for model in MODELS:
        ...
        for row in data["by_candidate_kind"]:
            lines.append(
                f"| {model} | `{row['candidate_kind']}` | {row['n']} | {row['case_n']} | {fmt(row['mean_scan_score'])} | "
    ...
    for model in MODELS:
        ...
        for row in data["by_fiber_bucket"]:
            lines.append(
                f"| {model} | `{row['fiber_bucket']}` | {row['n']} | {row['case_n']} | "
                f"{fmt(row['mean_target_logit_drop'])} | {fmt(row['mean_margin_drop_target_vs_contrast'])} | "
                f"{fmt(row['top1_loss_rate'])} | {fmt(row['mean_direct_target_boost'])} | "
                f"{fmt(row['mean_direct_route_suppression'])} |"
            )
    lines += [
        "",
        "## Top Components",
    ...
    for model in MODELS:
        ...
        if not data:
            continue
        for row in data["top_components"][:12]:
            lines.append(
                f"| {model} | `{row['component_key']}` | {row['n']} | {row['case_n']} | {fmt(row['mean_scan_score'])} | "
...
def write_cross_summary(round_name: str) -> dict[str, Any]:
    by_model = {}
    for model in MODELS:
        path = OUT_ROOT / round_name / f"phase772_{model}_summary.json"
        if path.exists():
    ...
    for root in (OUT_ROOT, RESULT_ROOT):
        ...
        write_json(out_dir / "phase772_cross_model_summary.json", payload)
        write_markdown(out_dir / "phase772_cross_model_summary.md", payload)
...
def main() -> None:
    ...
    parser.add_argument("--focus", choices=["all_matched", "clean", "fiber_high", "clean_fiber_high"], default="all_matched")
    ...
    parser.add_argument("--top-components-per-case", type=int, default=3)
