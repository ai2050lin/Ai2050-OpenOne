#!/usr/bin/env python3
"""Independent audit for Phase1418."""
...
def main() -> None:
    ...
    expected = "run_phase1419_c068_four_role_camera" if summary["behavior_qualified"] else "close_c068_at_behavior_gate"
    checks = {
        "rows": len(rows) == 4320,
        "cells": Counter(row["cell"] for row in rows) == {cell: 540 for cell in ("aa", "ab", "ac", "ad", "bb", "ba", "bc", "bd")},
        "truth": Counter(row["truth"] for row in rows) == {True: 1080, False: 3240},
    ...
    result = {"phase": 1418, "campaign": "C068", "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values())}
