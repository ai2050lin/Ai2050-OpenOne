#!/usr/bin/env python3
"""Independent pre/post audit for Phase1318; never imports the execution script."""
...
T = ROOT / "tests/glm5"
PHASE, CAMPAIGN = 1318, "C036"
OUT = T / "result/phase1318_c036_qwen3_behavior"
PARENT = T / "result/phase1317_c036_embedding_field_contract"
...
MAIN = T / "phase1318_c036_qwen3_behavior.py"
SCRIPT = Path(__file__).resolve()
MATERIAL = PARENT / "material/frozen_forward_lookup_pairs.jsonl"
...
def rate(values: Any) -> float:
    values = list(values); return float(np.mean(values)) if values else 0.0
...
def recompute(candidate: list[dict[str, Any]], generation: list[dict[str, Any]], th: dict[str, float]) -> dict[str, Any]:
    ...
    for x in candidate:
        if x["panel"] == "active": active[x["pair_key"]].append(x)
    gen_groups: dict[str, list[dict[str, Any]]] = defaultdict(list)
    for x in generation: gen_groups[x["pair_key"]].append(x)
    metrics = {
        "finite_fraction": rate(x["finite"] for x in candidate), "candidate_accuracy": rate(x["correct"] for x in candidate),
...
def base(protocol: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    add(checks, "parent_authorization", load(PARENT / "analysis/final.json").get("authorization") == "phase1318_qwen3_behavior_only"
        and load(PARENT / "audit/independent_final_audit.json").get("all_checks_passed"), "Phase1317")
    expected = {"parent_protocol": sha(PARENT / "protocol/preregistration.json"), "parent_final": sha(PARENT / "analysis/final.json"),
                "parent_audit": sha(PARENT / "audit/independent_final_audit.json"), "material": sha(MATERIAL),
                "generation_manifest": sha(M)}
    add(checks, "dependencies", protocol["dependencies"] == expected, protocol["dependencies"])
    material, generation = rows(MATERIAL), rows(M)
    add(checks, "material_counts", len(material) == 576 and protocol["material"]["state_count"] == 1152, len(material))
    add(checks, "generation_count_hash", len(generation) == 192 and protocol["material"]["generation_manifest_sha256"] == sha(M), len(generation))
    add(checks, "generation_scope", Counter(x["partition"] for x in generation) == Counter({"confirmation": 96, "holdout": 96})
        and all(x["binding_state"] in {0, 1} for x in generation), dict(Counter(x["partition"] for x in generation)))
    add(checks, "strict_behavior_only", protocol["hidden_states_read"] is False and protocol["success_authorization"] == "phase1319_embedding_field_only"
...
def preaudit() -> None:
    checks = base(load(P)); add(checks, "formal_outputs_absent", not any(x.exists() for x in (RAW, GEN, S, F, C)), "clear")
    write(PRE, checks, "pre_model", "run_phase1318_once")


def postaudit() -> None:
    protocol = load(P); checks = base(protocol); candidate, generation = rows(RAW), rows(GEN)
    add(checks, "raw_counts", len(candidate) == 1152 and len(generation) == 192, {"candidate": len(candidate), "generation": len(generation)})
    add(checks, "raw_balance", Counter(x["partition"] for x in candidate) == Counter({p: 384 for p in PARTITIONS})
    ...
    add(checks, "independent_gates", summary["gates"] == result["gates"] and summary["all_gates_passed"] == result["all_gates_passed"], result["gates"])
    authorization = "phase1319_embedding_field_only" if result["all_gates_passed"] else "close_c036_without_hidden"
    final = load(F)
    add(checks, "verdict_authorization", final["authorization"] == authorization and final["all_gates_passed"] == result["all_gates_passed"]
        and final["hidden_states_read"] is False, final)
    add(checks, "raw_hashes", summary["raw_hashes"] == {"candidate": sha(RAW), "generation": sha(GEN)}, summary["raw_hashes"])
    qa = summary["model_audit"]
    add(checks, "fp16_cuda", qa["has_fp16_parameters"] and not qa["has_quantized_modules"] and summary["cuda_peak_allocated_bytes"] > 0,
        {"qa": qa, "peak": summary["cuda_peak_allocated_bytes"]})
    complete = load(C)
    add(checks, "formal_budget_consumed", complete["formal_runs_consumed"] == 1 and complete["protocol_digest"] == protocol["protocol_digest"], complete)
