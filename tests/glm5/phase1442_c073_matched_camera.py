#!/usr/bin/env python3
"""Phase1442: calibrate the frozen C073 matched semantic-side/phase camera."""
...
def direction_rows(case: dict, compiled: dict[str, dict], source: str, target: str, direction: str) -> tuple[dict, dict]:
    if direction == "true_to_false":
        return compiled[case[f"{target}_true_recipient"]], compiled[case[f"{source}_false_donor"]]
    return compiled[case[f"{target}_false_recipient"]], compiled[case[f"{source}_true_donor"]]
...
def known_truth(count: int, registry: list[dict]) -> list[dict]:
    ...
    for seed in range(count):
        generator = torch.Generator().manual_seed(730000 + seed)
    ...
    try:
        ...
        for case in cases:
            for route in protocol["camera"]["routes"]:
                source_surface, target_surface, order = route_surfaces(route, protocol)
                for direction in DIRECTIONS:
                    ...
                    def hook(_module, args):
                        ...
                        for index, permutation in enumerate(registry, start=3):
                            ...
                            measured[permutation["permutation_id"]] = {
                                "write_max_abs_diff": role_error,
                                "complement_max_abs_diff": complement_error,
                                "self_role_max_abs_diff": self_error,
                                "quartet_size": len(set(arm_points.values())),
                    ...
                    for permutation in registry:
                        records.append({
                            ...
                            "source_length": len(donor["prompt_ids"]),
...
def main() -> None:
    if (OUT / "analysis/final.json").exists():
        raise RuntimeError("Phase1442 exists")
    ...
    if behavior_final["authorization"] != "run_phase1442_c073_matched_camera" or not behavior_audit["all_checks_passed"]:
    ...
    all_registry = core.rows(CONTRACT / "material/permutation_registry.jsonl")
    ids = set(protocol["camera"]["permutation_ids"])
    registry = [row for row in all_registry if row["permutation_id"] in ids]
    ...
    core.write_rows(OUT / "raw/qwen_matched_camera.jsonl", qwen)
    gate = protocol["camera"]
    shape_ok = all((row["source_length"] == row["target_length"]) == (row["route_order"] == "same_order") for row in qwen)
    checks = {
        "known_count": len(known) == gate["known_truth_systems"] * len(registry),
        "known_exact": all(row["roles_exact"] and row["complement_exact"] for row in known),
        "registry": [row["permutation_id"] for row in registry] == gate["permutation_ids"],
        "qwen_sets": len(cases) == gate["qwen_discovery_sets"] and len({row["family"] for row in cases}) == 6,
        "qwen_count": len(qwen) == gate["qwen_discovery_sets"] * len(gate["routes"]) * len(gate["directions"]) * len(registry),
        "discovery_only": {row["partition"] for row in qwen} == {"response_discovery"},
        "permutations": {row["permutation_id"] for row in qwen} == ids,
    ...
    authorization = "run_phase1443_c073_side_phase_competition" if summary["camera_qualified"] else "close_c073_at_camera_gate"
