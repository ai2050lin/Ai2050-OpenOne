#!/usr/bin/env python3
"""Phase1305: C033 Qwen3 behavior qualification on wholly new material."""
from __future__ import annotations
import argparse,hashlib,inspect,json,re,shutil,sys,time
...
PHASE=1305;CAMPAIGN="C033";SCRIPT=Path(__file__).resolve();AUDITOR=T/"phase1305_c033_qwen3_behavior_audit.py";PARENT=T/"result/phase1304_c033_role_typed_causal_graph_contract";PARENT_PROTOCOL=PARENT/"protocol/preregistration.json";PARENT_FINAL=PARENT/"analysis/final.json";PARENT_AUDIT=PARENT/"audit/independent_final_audit.json";MATERIAL=PARENT/"material/frozen_role_typed_lookup_cases.jsonl";OUT=T/"result/phase1305_c033_qwen3_behavior";PROTOCOL=OUT/"protocol/preregistration.json";PRE=OUT/"audit/independent_preaudit.json";POST=OUT/"audit/independent_final_audit.json";RAW=OUT/"raw/candidate_scores.jsonl";GEN=OUT/"raw/list_free_generations.jsonl";SUMMARY=OUT/"analysis/behavior_summary.json";FINAL=OUT/"analysis/final.json";COMPLETE=OUT/"protocol/formal_run_complete.json";SYSTEM="Use only the supplied catalog. Reply exactly as requested and do not explain.";SCORE_BATCH=32;GEN_BATCH=8;MAX_NEW=8
...
def preregister(force):
 if load(PARENT_FINAL).get("authorization")!="phase1305_qwen3_behavior_only" or not load(PARENT_AUDIT).get("all_checks_passed"):raise RuntimeError("parent auth")
 if OUT.exists() and not force:raise RuntimeError(f"{OUT} exists")
 if OUT.exists():shutil.rmtree(OUT)
 cp=load(PARENT_PROTOCOL);timeless={"phase":PHASE,"campaign":CAMPAIGN,"schema_version":"phase1305.c033.behavior.v1","model":"qwen3-4b-fp16-cuda-no-quantization","formal_run_budget":1,"runtime":{"compiler":"right_padding","batching":"exact token-length buckets; physical start column zero","score_batch_max":SCORE_BATCH,"generation_batch_max":GEN_BATCH},"material":{"sha256":sha(MATERIAL),"case_count":6912,"generation_filter":"confirmation or holdout and candidate_order=0","generation_count":1536},"thresholds":cp["behavior_thresholds"],"zero_models":cp["zero_models"],"scoring":"three frozen one-token continuations by log probability","generation":"greedy, max 8 new tokens, frozen exact-first-line parser","hidden_states_read":False,"success_authorization":"phase1306_frozen_hidden_only","failure_authorization":"close_c033_without_hidden","dependencies":{"parent_protocol":sha(PARENT_PROTOCOL),"parent_final":sha(PARENT_FINAL),"parent_audit":sha(PARENT_AUDIT),"material":sha(MATERIAL),"helper":sha(T/"phase1295_c030_qwen3_grounded_lookup_behavior.py")},"source_hashes":{"main":sha(SCRIPT),"auditor":sha(AUDITOR)},"model_weights_loaded":False};p={**timeless,"created_at_utc":datetime.now(timezone.utc).isoformat(),"protocol_digest":digest(timeless)};save(PROTOCOL,p);print(canonical({"digest":p["protocol_digest"],"cases":6912}))
def prepare(tok,rr,key):
 out=[]
 for i,r in enumerate(rr):
  rendered=chat(tok,r[key]);out.append({"index":i,"row":r,"rendered":rendered,"ids":tok.encode(rendered,add_special_tokens=False)})
 return out
def buckets(items):
 d=defaultdict(list)
...
def score(model,tok,device,rr):
 ...
 for bucket in buckets(prepared):
  for start in range(0,len(bucket),SCORE_BATCH):
   ...
   lp=torch.log_softmax(model(**kw).logits[:,-1,:].float(),dim=-1)
   for j,x in enumerate(b):
    r=x["row"];cids=[]
    for name in r["candidates"]:
     full=tok.encode(x["rendered"]+" "+name,add_special_tokens=False)
     if full[:len(x["ids"])]!=x["ids"] or len(full)!=len(x["ids"])+1:raise RuntimeError("candidate token drift")
     cids.append(full[-1])
