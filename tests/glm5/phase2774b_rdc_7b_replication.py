"""Phase 2774b: carrier-ablation replication on DS-R1-Distill-Qwen-7B (third scale).  NOTE: original target was Qwen3-14B; on this machine transformers 5.14.1 + WDDM reproduce an access violation in any bf16 build that materializes >~13-16 GB (segfault in torch storage __getitem__ during safetensors get_tensor), and even persistent-handle loads hit spurious CUDA OOM.  Substituted DS-R1-Distill-Qwen-7B (Qwen2ForCausalLM, 7.6B, same BPE family) so the scale axis becomes 1.7B -> 4B -> 7B.

Loader note: transformers 5.14.1 from_pretrained(device_map='auto') segfaults
on this machine in core_model_loading.materialize_tensors whenever the first
non-GPU tensor is materialized after large CUDA allocations (reproduced with
cpu/disk quotas and single-worker loader).  All 8 shards verified readable
via safetensors.  Therefore we bypass: build model on meta device, assign
every parameter manually from the original shards, fix meta buffers
(inv_freq), then dispatch with an explicit GPU(20 layers)+CPU(20 layers)
plan.  Non-quantized.

Replicates the Phase 2769 core: native wrong rows of the 320
controlled_relation diagnostic, per-row rival direction, top-4 positive-gain
carrier heads in modules M={23..27} (last 5 of 28), targeted o_proj-input
ablation at the final position, random-head null, union-set collateral.

Prompts re-tokenised: qwen4 ids -> text (qwen4 decode) -> qwen14 ids.
Targets from row['target_text'] (first 7B-model token).

Preregistered (frozen before any 7B-model forward):
  ...
  verdict: carrier_replicated_7b iff E1 and E2.
...
OUT = BASE / 'phase2774' / 'ds7b_replication'
M_LAYERS = [23, 24, 25, 26, 27]  # last 5 of 28
TOP_HEADS = 4
N_NULL_ROWS = 20
N_NULL_DRAWS = 10
NULL_SEED = 27740
N_COLL_ROWS = 20
N_GPU_LAYERS = 20  # GPU budget: embed+lm_head+20x0.47GiB ~= 11.6GiB of 14.8
...
def build_model_manual():
    ...
    path = str(ROOT / 'models' / 'hf' / 'deepseek-r1-distill-qwen-7b')
    ...
    plan = {'model.embed_tokens': 0, 'lm_head': 0, 'model.norm': 'cpu'}
    for i in range(cfg.num_hidden_layers):
        plan['model.layers.%d' % i] = 0 if i < N_GPU_LAYERS else 'cpu'
    ...
    # keep ONE safe_open handle per shard for the whole loop: rapid open/
    # unmap cycles of 3.45GB views fragment the address space on this
    # machine (access violation after ~13-16GB cumulative reads)
    shard_handles = {}
    ...
    for mod_name, mod in model.named_modules():
        for pname, p in list(mod.named_parameters(recurse=False)):
            ...
            # Re-wrap as Parameter (primitive proven safe) so accelerate's
...
def main():
    ...
    tok14 = AutoTokenizer.from_pretrained(
        str(ROOT / 'models' / 'hf' / 'deepseek-r1-distill-qwen-7b'),
    ...
    execution = {'timestamp': fc.stamp(), 'source': cc.snapshot(__file__),
                 ...
                 'loader': 'manual meta+shard assign (persistent shard handles), dispatch GPU20/CPU8',
    ...
    for l in M_LAYERS:
        ...
        handles.append(model.model.layers[l].self_attn.v_proj.
    ...
    print('P2774B NATIVE_DONE wrong_total=%d %s'
    ...
    for j, i in enumerate(wrong_idx):
        ...
        if (j + 1) % 10 == 0:
            print('P2774B CARRIER %d/%d flips=%d'
    ...
    verdict = {'E1_pass': e1_pass, 'E2_pass': e2_pass,
               'carrier_replicated_7b': bool(e1_pass and e2_pass),
    ...
    result = {'phase': '2774b', 'prereg': PREREG, 'verdict': verdict,
    ...
    fc.npz(OUT / 'carrier_stats_7b.npz', wrong_idx=np.array(wrong_idx),
           arg_native=arg_native, tgt_ids=tgt_ids,
           null_totals=null_totals)
    for h in handles:
        h.remove()
    print('P2774B VERDICT %s' % json.dumps(verdict), flush=True)
