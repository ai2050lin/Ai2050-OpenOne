...
def fisher_vs_pca_alignment(pca_results, fisher_results, target_layers, d_model):
    ...
    for l in target_layers:
        if l not in fisher_results:
            continue
        
        pca_comp = pca_results[l]['components']  # (d_model, d_model)
        fisher_dirs = np.array(fisher_results[l]['top_fisher_dirs'])  # (N_TOP_FISHER, d_model)
...
# Exp 3: W_U行空间与信号子空间的对齐
# ============================================================
def w_u_alignment_analysis(model, pca_results, fisher_results, target_layers, d_model):
    """
    ...
    for l in target_layers:
        ...
        fisher_dirs = np.array(fisher_results[l]['top_fisher_dirs'])  # (N_TOP_FISHER, d_model)
        fisher_vals = np.array(fisher_results[l]['top_fisher_values'])
        ...
        for idx in fisher_results[l]['sorted_indices'][:N_TOP_FISHER]:
            if idx >= 50:  # PCA排名50以后
...
def directional_ablation_experiment(model, tokenizer, device, model_info,
    ...
    for l in target_layers:
        ...
        print(f"  Layer {l}...")
        pca_comp = pca_results[l]['components']
        fisher_vals = np.array(fisher_results[l]['fisher_values'])
...
def main():
    ...
    wu_results = w_u_alignment_analysis(
        model, pca_results, fisher_results, target_layers, model_info.d_model
    )
