#!/usr/bin/env python3
"""C118: identifiable default inheritance and exception override campaign."""
...
RESULT = TESTS / "result"
OUT = RESULT / "phase1640_c118_identifiable_default_override_campaign"
...
def build() -> tuple[list[dict], list[dict]]:
    ...
    for index, values in enumerate(inventory()):
        ...
        unit = {
            "unit_id": f"c118-override-{index:02d}",
            "family": FAMILY,
...
def contract() -> None:
    if OUT.exists():
        raise RuntimeError(f"C118 already exists: {OUT}")
    parent = core.load(C117 / "analysis/closure.json")
    parent_audit = core.load(C117 / "audit/independent_closure_audit.json")
    if not parent_audit["all_checks_passed"] or not parent["next_authorization"].startswith("C118"):
        raise RuntimeError("C118 authorization missing")
    units, cases = build()
    ...
    protocol = {
        "phase": 1640, "campaign": CAMPAIGN, "created_at_utc": now(),
        "status": "identifiable_default_override_contract_frozen",
        "model": "Qwen3-4B local BF16 CUDA nonquantized",
        "object": "default inheritance and item-specific conflicting exception override",
        ...
        "common_residual": {"role": "boundary", "state": 30, "reference_families": ["C115_attribute", "C115_agent", "C116_negation", "C117_explicit_log"], "definition": "leave-C118-out normalized sum of normalized family means", "status": "registered component intervention; no semantic-name gate; wrong-family residual unavailable under matched geometry"},
        ...
        "source_hashes": {"c117_closure": core.sha(C117 / "analysis/closure.json"), "c117_audit": core.sha(C117 / "audit/independent_closure_audit.json")},
        "material_digest": core.digest([*units, *cases]), "producer_sha256": core.sha(Path(__file__)),
        "authorization": "execute_phase1641_c118_cuda_capture",
    }
    core.save(OUT / "protocol/preregistration.json", protocol)
    report = {"phase": 1640, "campaign": CAMPAIGN, "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values()), "zero_models": zero, "occurrences": len(occurrences), "max_width": max(len(row["prompt_ids"]) for row in compiled), "authorization": protocol["authorization"]}
...
def capture() -> None:
    ...
    if not core.load(OUT / "audit/independent_contract_audit.json")["all_checks_passed"]:
        raise RuntimeError("C118 independent contract audit missing")
    ...
    if any(path.exists() for path in (raw_path, logits_path, index_path)):
        raise RuntimeError("C118 raw output already exists")
    ...
    try:
        ...
        for start in range(0, len(rows), BATCH):
            ...
            if (start // BATCH + 1) % 12 == 0:
                ...
                print(f"[phase1641] captured {start + len(batch)}/{len(rows)}", flush=True)
    ...
    report = {
        "phase": 1641, "campaign": CAMPAIGN, "created_at_utc": now(), "status": "cuda_capture_complete",
        ...
        "authorization": "run_phase1642_c118_discovery" if all(gate_checks.values()) else "close_hidden_state_route_and_continue_campaign_missingness_ledger",
    }
...
def derive_fields(partitions: set[str], path: Path) -> tuple[np.ndarray, list[dict]]:
    ...
    for state in range(STATES):
        ...
        if state % 6 == 0 or state == 36:
            fields.flush(); print(f"[C118 fields] {sorted(partitions)} state {state}/36", flush=True)
    fields.flush()
    return fields, units
...
def discover() -> None:
    ...
    if not core.load(OUT / "audit/independent_capture_audit.json")["all_checks_passed"] or not capture_report["behavior_gate_passed"]:
        raise RuntimeError("C118 behavior qualification missing")
    ...
    report = {"phase": 1642, "campaign": CAMPAIGN, "created_at_utc": now(), "status": "override_candidate_frozen", "winner": winner, "support_k": 256, "checks": {"units": len(units) == 8, "shape": list(fields.shape) == [8, 5, 9, 37, 2560], "candidates": len(candidates) == 270, "eligible": bool(eligible), "support": len(set(support)) == 256}, "field_sha256": core.sha(path), "nomination_sha256": core.sha(nomination_path), "authorization": "execute_phase1643_c118_validation"}
...
def validate() -> None:
    ...
    if not core.load(OUT / "audit/independent_discovery_audit.json")["all_checks_passed"]:
        raise RuntimeError("C118 discovery audit missing")
    ...
    residual_metrics = {
        "definition": "leave-C118-out G at boundary@state30; E=R-dot(R,G)G; names remain geometric",
    ...
    try:
        ...
        for (partition, output_format, length), group in sorted(grouped.items()):
            for start in range(0, len(group), BATCH):
                ...
                print(f"[phase1643] {partition}/format={output_format}/len={length} {start + len(batch)}/{len(group)}", flush=True)
    ...
    if not all(checks.values()):
        raise RuntimeError(checks)
    report = {
        "phase": 1643, "campaign": CAMPAIGN, "created_at_utc": now(), "status": "confirmation_lockbox_validation_complete",
        ...
        "leave_c118_out_common_residual": residual_metrics, "predictions": predictions,
        ...
        "authorization": "run_phase1644_c118_synthesis_visualization_and_closure",
    }
...
def synthesize() -> None:
    ...
    if not core.load(OUT / "audit/independent_validation_audit.json")["all_checks_passed"]:
        raise RuntimeError("C118 validation audit missing")
        ...
        for effect in ("truth", "default_inheritance", "exception_read", "override", "same_hit_control"):
            ...
            for r, role in enumerate(ROLES):
                for state in display_states:
                    effect_rows.append({"dataset": "C118", "family": FAMILY, "partition": partition, "role": role, "state": state, "state_kind": "embedding" if state == 0 else "hidden_state", "effect": effect, "values": np.asarray(mean[e, r, state], dtype=np.float32).tolist()})
    ...
    closure = {
        "phase": 1644, "campaign": CAMPAIGN, "created_at_utc": now(), "status": "identifiable_default_override_campaign_complete",
        "headline": {"behavior": behavior, "discovery_winner": discovery_report["winner"], "field_metrics": validation["field_metrics"], "field_checks": validation["field_checks"], "effect_metrics": validation["effect_metrics"], "coordinate_and_component_predictions": validation["predictions"], "leave_c118_out_common_residual": validation["leave_c118_out_common_residual"]},
        "new_puzzles": {
            "K310": "behavioral separation of default inheritance and conflicting item-specific exception override under two output vocabularies",
            "K311": "discovery-frozen full-coordinate override contrast and held-out field result",
            "K312": "coordinate-assignment and leave-C118-out boundary projection component intervention result, with geometric names only",
        },
        "theory_update": "RDC now separates a natural run, researcher-defined contrasts, candidate coordinate implementations and interventions. C118 repairs the semantic object that C117 did not identify; projection components remain observations until selective deletion, wrong-residual and rescue controls exist under matched geometry.",
        "unified_formula": "H_s=Phi_{theta,<s}(E(x),kappa); R_e=E[w_e(x) H_s(x)]; Gamma=(s,C,S,V); y_I=O(Phi_{theta,>=s}(I_Gamma(H_s)))",
        "problems": ["controlled synthetic English and one Qwen3", "machine naturalness only", "the exception sentence remains explicit when it hits, although miss cases require default inheritance", "discovery searches states1-30 and may prefer late decision states", "activation patching can be off-distribution", "K256 and eight permutations do not prove minimality or semantic specificity", "leave-C118-out G is researcher-defined and output-boundary aligned", "wrong-family residual is not identifiable under matched prompt geometry", "no attention/MLP, weights, cross-model or natural paraphrase test"],
        ...
        "next_authorization": "C119 matched-output comparison-family observation campaign, while C118 missingness retains human naturalness, natural paraphrases, cross-model replication and matched wrong-family residual rescue",
    }
    core.save(OUT / "analysis/closure.json", closure)
    checks = {"validation": True, "effects": len(effect_rows) == 3 * 5 * 9 * len(display_states), "raw": len(raw_rows) == 3 * 5 * len(RAW_STATES), "coordinates": all(len(row["values"]) == 2560 for row in [*effect_rows, *raw_rows]), "asset": core.sha(canonical) == core.sha(PUBLIC), "batch": "c118_batch" in payload, "boundary": "does not identify weights" in payload["claim_boundary"] and "attention/MLP" in payload["claim_boundary"]}
    if not all(checks.values()):
        raise RuntimeError(checks)
    audit = {"phase": 1644, "campaign": CAMPAIGN, "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values()), "asset_sha256": core.sha(PUBLIC), "authorization": "run_independent_closure_and_client_audits_then_append_memo"}
