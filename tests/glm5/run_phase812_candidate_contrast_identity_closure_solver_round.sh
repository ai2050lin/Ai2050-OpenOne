...
for MODEL in qwen3 glm4 deepseek7b; do
  ...
  COMMON_ARGS=(
    ...
    --objective-surface-fragment-weight 0.25
  ...
  echo "[$(date +%H:%M:%S)] phase812 $ROUND: start $MODEL cases=$MODEL_MAX_CASES combo_size=$MAX_COMBO_SIZE pool=$MAX_COMBO_CANDIDATES combos=$MAX_COMBOS"
  python tests/glm5/phase812_candidate_contrast_identity_closure_solver.py \
    --model "$MODEL" \
    "${COMMON_ARGS[@]}"
  echo "[$(date +%H:%M:%S)] phase812 $ROUND: done $MODEL"
done
