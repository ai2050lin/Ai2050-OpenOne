...
case "${ROUND_NAME}" in
  smoke)
    ...
    MAX_NEW_TOKENS=6
    PREDICTORS="readout_signed_sum,readout_count,route_target_only,route_target_else_signed_sum,route_target_else_count"
    ;;
  main)
    MAX_GROUPS=16
    MAX_PAIRS=8
    ...
    MAX_NEW_TOKENS=8
    PREDICTORS="readout_signed_sum,readout_count,route_target_only,route_target_else_signed_sum,route_target_else_count,route_clean_signed_sum,route_clean_count"
    ;;
  confirm)
    MAX_GROUPS=20
    MAX_PAIRS=12
...
for MODEL in qwen3 glm4 deepseek7b; do
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 832 ${ROUND_NAME}: start ${MODEL}"
  python tests/glm5/phase832_gate_signal_expansion_beyond_readout.py \
    --model "${MODEL}" \
    "${COMMON_ARGS[@]}"
  echo "[$(date '+%Y-%m-%d %H:%M:%S')] Phase 832 ${ROUND_NAME}: done ${MODEL}"
done
