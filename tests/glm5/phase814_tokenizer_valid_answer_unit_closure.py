...
RESULT_ROOT = Path("tests/result/phase814_tokenizer_valid_answer_unit_closure")
...
def answer_equivalence_ids(
    ...
    answer_ids.add(int(target_id))
    answer_ids = expand_equivalence_ids_from_top(
        args,
        vals,
        answer_ids,
        answer_norm,
        int(getattr(args, "answer_variant_scan_topk", 512)),
    )
    return sorted(tid for tid in answer_ids if 0 <= int(tid) < vals.numel())


def choose_context_answer_unit(
    args: argparse.Namespace,
    logits: torch.Tensor,
    target_id: int,
    target_answer: str,
    ...
    vals = logits.detach().float().cpu()
    answer_ids = answer_equivalence_ids(args, vals, target_id, target_answer)
    if not answer_ids:
        answer_ids = [int(target_id)]
    answer_logits = vals[answer_ids]
    best_val, best_idx = torch.max(answer_logits, dim=0)
    best_id = int(answer_ids[int(best_idx.item())])
    target_logit = float(vals[int(target_id)].item())
    best_logit = float(best_val.item())
    return {
        "context_answer_unit_id": best_id,
        "context_answer_unit_text": p798.cached_token_text(args, best_id),
        "context_answer_unit_source": "recipient_base_answer_class_best",
        "context_answer_unit_baseline_logit": best_logit,
        "context_answer_unit_baseline_margin_vs_raw_target": best_logit - target_logit,
        "context_answer_unit_equiv_size": len(answer_ids),
        "context_answer_unit_equiv_token_texts": [
            p798.cached_token_text(args, tid) for tid in answer_ids[: int(args.max_answer_equiv_ids_saved)]
        ],
    }
...
def answer_class_snapshot(
    ...
    context_unit_id = getattr(args, "_context_answer_unit_id", None)
    if context_unit_id is None or int(context_unit_id) not in answer_id_list:
        context_unit_id = best_answer_id
        context_unit_source = "snapshot_answer_class_best_fallback"
    else:
        context_unit_id = int(context_unit_id)
        context_unit_source = str(getattr(args, "_context_answer_unit_source", "recipient_base_answer_class_best"))
    context_unit_logit = float(vals[int(context_unit_id)].item())
    noncanonical_answer_ids = [tid for tid in answer_id_list if tid != target_id]
    if noncanonical_answer_ids:
        noncanonical_answer_logits = vals[noncanonical_answer_ids]
        noncanonical_best_val, noncanonical_best_idx = torch.max(noncanonical_answer_logits, dim=0)
        noncanonical_best_logit = float(noncanonical_best_val.item())
        noncanonical_best_id = int(noncanonical_answer_ids[int(noncanonical_best_idx.item())])
        canonical_surface_margin = target_logit - noncanonical_best_logit
    else:
        noncanonical_best_logit = None
        noncanonical_best_id = None
        canonical_surface_margin = None
    ...
    answer_unit_variant_above = []
    for tid in answer_id_list:
        if tid == context_unit_id:
            continue
        logit = float(vals[tid].item())
        if logit > context_unit_logit:
            answer_unit_variant_above.append(
                {
                    "token_id": tid,
                    "token_text": p798.cached_token_text(args, tid),
                    "logit": logit,
                    "gap_above_unit": logit - context_unit_logit,
                }
            )
    answer_unit_variant_above.sort(key=lambda row: row["logit"], reverse=True)
    nonunit_answer_ids = [tid for tid in answer_id_list if tid != context_unit_id]
    if nonunit_answer_ids:
        nonunit_logits = vals[nonunit_answer_ids]
        nonunit_best_val, nonunit_best_idx = torch.max(nonunit_logits, dim=0)
        nonunit_best_logit = float(nonunit_best_val.item())
        nonunit_best_id = int(nonunit_answer_ids[int(nonunit_best_idx.item())])
        answer_unit_margin = context_unit_logit - nonunit_best_logit
    else:
        nonunit_best_logit = None
        nonunit_best_id = None
        answer_unit_margin = None
    ...
    return {
        ...
        "context_answer_unit_id": context_unit_id,
        "context_answer_unit_text": p798.cached_token_text(args, context_unit_id),
        "context_answer_unit_source": context_unit_source,
        "context_answer_unit_logit": context_unit_logit,
        "context_answer_unit_is_raw_target": context_unit_id == target_id,
        "context_answer_unit_is_best_answer_variant": best_answer_id == context_unit_id,
        "answer_unit_closure": len(answer_unit_variant_above) == 0,
        "answer_unit_rank_within_answer_class": len(answer_unit_variant_above) + 1,
        "answer_unit_margin_vs_best_variant": answer_unit_margin,
        "answer_unit_best_nonunit_token_id": nonunit_best_id,
        "answer_unit_best_nonunit_token_text": (
            p798.cached_token_text(args, nonunit_best_id) if nonunit_best_id is not None else None
        ),
        "answer_unit_best_nonunit_logit": nonunit_best_logit,
        "answer_unit_variant_count_above_unit": len(answer_unit_variant_above),
        "answer_unit_variant_examples": answer_unit_variant_above[: int(args.max_surface_variants_saved)],
        "canonical_surface_token_id": target_id,
        "canonical_surface_token_text": p798.cached_token_text(args, target_id),
        "canonical_surface_closure": len(answer_variant_above_target) == 0,
        "canonical_surface_rank_within_answer_class": len(answer_variant_above_target) + 1,
        "canonical_surface_margin_vs_best_variant": canonical_surface_margin,
        "canonical_surface_best_noncanonical_token_id": noncanonical_best_id,
        "canonical_surface_best_noncanonical_token_text": (
            p798.cached_token_text(args, noncanonical_best_id) if noncanonical_best_id is not None else None
        ),
        "canonical_surface_best_noncanonical_logit": noncanonical_best_logit,
...
def objective(row: dict[str, Any], args: argparse.Namespace) -> float:
    ...
    unit_variant_above = finite(row.get("after_answer_unit_variant_count_above_unit"), 0.0)
    answer_class_loss = 0.0 if bool(row.get("after_answer_class_closure")) else 1.0
    answer_unit_margin = finite(row.get("after_answer_unit_margin_vs_best_variant"), 0.0)
    return (
        float(args.objective_answer_unit_weight) * unit_variant_above
        + float(args.objective_answer_class_preserve_weight) * answer_class_loss
        + float(args.objective_answer_class_weight) * answer_above
        + float(args.objective_contrast_class_weight) * contrast_above
        + float(args.objective_surface_fragment_weight) * unit_variant_above
        ...
        - float(args.objective_answer_unit_margin_weight) * answer_unit_margin
    )
...
def label_solution(row: dict[str, Any], args: argparse.Namespace) -> str:
    ...
    if bool(row.get("after_answer_class_closure")) and bool(row.get("after_answer_unit_closure")):
        return "answer_unit_closure_no_strict_token"
    if bool(row.get("after_answer_class_closure")):
        if finite(row.get("after_answer_unit_variant_count_above_unit")) > 0:
            if finite(row.get("answer_unit_variant_above_unit_delta"), 0.0) < 0:
                return "answer_unit_reducer_answer_closed"
            return "answer_class_closure_unit_fragmented"
        return "answer_class_closure_no_token"
    if bool(row.get("semantic_answer_class_closure")) and not bool(row.get("after_answer_class_closure")):
        return "answer_class_loss_deformer"
    if bool(row.get("after_answer_unit_closure")):
        return "answer_unit_closure_answer_not_closed"
    if finite(row.get("answer_unit_variant_above_unit_delta"), 0.0) < 0:
        return "answer_unit_reducer_answer_not_closed"
...
def row_from_eval(
    ...
    row: dict[str, Any] = {
        "row_kind": "phase814_tokenizer_valid_answer_unit_closure",
        "model": args.model,
        ...
        "combo_error": error,
        "context_answer_unit_id": (getattr(args, "_context_answer_unit_payload", {}) or {}).get("context_answer_unit_id"),
        "context_answer_unit_text": (getattr(args, "_context_answer_unit_payload", {}) or {}).get("context_answer_unit_text"),
        "context_answer_unit_source": (getattr(args, "_context_answer_unit_payload", {}) or {}).get("context_answer_unit_source"),
        "context_answer_unit_baseline_logit": (getattr(args, "_context_answer_unit_payload", {}) or {}).get(
            "context_answer_unit_baseline_logit"
        ),
        "context_answer_unit_baseline_margin_vs_raw_target": (getattr(args, "_context_answer_unit_payload", {}) or {}).get(
            "context_answer_unit_baseline_margin_vs_raw_target"
        ),
        "context_answer_unit_equiv_size": (getattr(args, "_context_answer_unit_payload", {}) or {}).get(
            "context_answer_unit_equiv_size"
        ),
        "context_answer_unit_equiv_token_texts": (getattr(args, "_context_answer_unit_payload", {}) or {}).get(
            "context_answer_unit_equiv_token_texts"
        ),
        ...
        "semantic_answer_unit_closure": semantic_snapshot.get("answer_unit_closure"),
        "semantic_answer_unit_rank_within_answer_class": semantic_snapshot.get("answer_unit_rank_within_answer_class"),
        "semantic_answer_unit_margin_vs_best_variant": semantic_snapshot.get("answer_unit_margin_vs_best_variant"),
        "semantic_answer_unit_variant_count_above_unit": semantic_snapshot.get("answer_unit_variant_count_above_unit"),
        "semantic_canonical_surface_closure": semantic_snapshot.get("canonical_surface_closure"),
        "semantic_canonical_surface_rank_within_answer_class": semantic_snapshot.get("canonical_surface_rank_within_answer_class"),
        "semantic_canonical_surface_margin_vs_best_variant": semantic_snapshot.get("canonical_surface_margin_vs_best_variant"),
        ...
        "full_closer_answer_unit_closure": full_snapshot_row.get("answer_unit_closure"),
        "full_closer_answer_unit_rank_within_answer_class": full_snapshot_row.get("answer_unit_rank_within_answer_class"),
        "full_closer_answer_unit_margin_vs_best_variant": full_snapshot_row.get("answer_unit_margin_vs_best_variant"),
        "full_closer_answer_unit_variant_count_above_unit": full_snapshot_row.get("answer_unit_variant_count_above_unit"),
        "full_closer_canonical_surface_closure": full_snapshot_row.get("canonical_surface_closure"),
        "full_closer_canonical_surface_rank_within_answer_class": full_snapshot_row.get("canonical_surface_rank_within_answer_class"),
        "full_closer_canonical_surface_margin_vs_best_variant": full_snapshot_row.get("canonical_surface_margin_vs_best_variant"),
    ...
    if not error:
        row.update(
            {
                ...
                "after_answer_unit_id": snapshot.get("context_answer_unit_id"),
                "after_answer_unit_text": snapshot.get("context_answer_unit_text"),
                "after_answer_unit_source": snapshot.get("context_answer_unit_source"),
                "after_answer_unit_is_raw_target": bool(snapshot.get("context_answer_unit_is_raw_target")),
                "after_answer_unit_is_best_answer_variant": bool(snapshot.get("context_answer_unit_is_best_answer_variant")),
                "after_answer_unit_closure": bool(snapshot.get("answer_unit_closure")),
                "after_answer_unit_rank_within_answer_class": snapshot.get("answer_unit_rank_within_answer_class"),
                "after_answer_unit_margin_vs_best_variant": snapshot.get("answer_unit_margin_vs_best_variant"),
                "after_answer_unit_best_nonunit_token_id": snapshot.get("answer_unit_best_nonunit_token_id"),
                "after_answer_unit_best_nonunit_token_text": snapshot.get("answer_unit_best_nonunit_token_text"),
                "after_answer_unit_best_nonunit_logit": snapshot.get("answer_unit_best_nonunit_logit"),
                "after_answer_unit_variant_count_above_unit": snapshot.get("answer_unit_variant_count_above_unit"),
                "after_answer_unit_variant_examples": snapshot.get("answer_unit_variant_examples"),
                "after_canonical_surface_token_id": snapshot.get("canonical_surface_token_id"),
                "after_canonical_surface_token_text": snapshot.get("canonical_surface_token_text"),
                "after_canonical_surface_closure": bool(snapshot.get("canonical_surface_closure")),
                "after_canonical_surface_rank_within_answer_class": snapshot.get("canonical_surface_rank_within_answer_class"),
                "after_canonical_surface_margin_vs_best_variant": snapshot.get("canonical_surface_margin_vs_best_variant"),
                "after_canonical_surface_best_noncanonical_token_id": snapshot.get("canonical_surface_best_noncanonical_token_id"),
                "after_canonical_surface_best_noncanonical_token_text": snapshot.get("canonical_surface_best_noncanonical_token_text"),
                "after_canonical_surface_best_noncanonical_logit": snapshot.get("canonical_surface_best_noncanonical_logit"),
        ...
        row["answer_unit_variant_above_unit_delta"] = finite(
            row.get("after_answer_unit_variant_count_above_unit")
        ) - finite(row.get("semantic_answer_unit_variant_count_above_unit"))
        row["answer_unit_margin_delta"] = finite(row.get("after_answer_unit_margin_vs_best_variant")) - finite(
            row.get("semantic_answer_unit_margin_vs_best_variant")
        )
        row["canonical_surface_fragment_delta"] = row["surface_answer_variant_above_target_delta"]
        row["canonical_surface_margin_delta"] = finite(
            row.get("after_canonical_surface_margin_vs_best_variant")
        ) - finite(row.get("semantic_canonical_surface_margin_vs_best_variant"))
        row["class_metrics"] = class_metrics(row, args)
        row["objective_score"] = objective(row, args)
        row["phase814_label"] = label_solution(row, args)
    else:
        row["objective_score"] = None
        row["phase814_label"] = "combo_error"
    row["phase814_boundary"] = (
        "This phase tests tokenizer-valid answer unit closure. "
        "A positive result requires preserving answer-class closure while the prompt-conditioned answer unit beats other answer variants."
    )
    return row
...
def audit_case_route(
    ...
    context_unit = choose_context_answer_unit(args, recipient_state["logits"], target_id, str(case.get("answer", "")))
    setattr(args, "_context_answer_unit_id", int(context_unit["context_answer_unit_id"]))
    setattr(args, "_context_answer_unit_source", str(context_unit["context_answer_unit_source"]))
    setattr(args, "_context_answer_unit_payload", context_unit)
...
def group_rows(rows: list[dict[str, Any]], fields: list[str]) -> list[dict[str, Any]]:
    ...
    for key, vals in groups.items():
        ...
        labels = Counter(v.get("phase814_label") for v in vals)
        payload.update(
            {
                "n": len(vals),
                ...
                "answer_unit_closure_rate": p809.safe_rate([v.get("after_answer_unit_closure") for v in vals]),
                "mean_answer_unit_rank": p809.safe_mean([v.get("after_answer_unit_rank_within_answer_class") for v in vals]),
                "mean_answer_unit_margin": p809.safe_mean([v.get("after_answer_unit_margin_vs_best_variant") for v in vals]),
                "mean_answer_unit_fragment_delta": p809.safe_mean([v.get("answer_unit_variant_above_unit_delta") for v in vals]),
                "canonical_surface_closure_rate": p809.safe_rate([v.get("after_canonical_surface_closure") for v in vals]),
                "mean_canonical_surface_rank": p809.safe_mean(
                    [v.get("after_canonical_surface_rank_within_answer_class") for v in vals]
                ),
                "mean_canonical_surface_margin": p809.safe_mean(
                    [v.get("after_canonical_surface_margin_vs_best_variant") for v in vals]
                ),
                "mean_canonical_surface_fragment_delta": p809.safe_mean(
                    [v.get("canonical_surface_fragment_delta") for v in vals]
                ),
                ...
                "mean_answer_unit_variants_above_unit": p809.safe_mean(
                    [v.get("after_answer_unit_variant_count_above_unit") for v in vals]
                ),
...
def summarize(rows: list[dict[str, Any]], args: argparse.Namespace, attn_impl: str, routes: list[dict[str, Any]]) -> dict[str, Any]:
    valid = [r for r in rows if r.get("row_kind") == "phase814_tokenizer_valid_answer_unit_closure" and not r.get("combo_error")]
    best = sorted(
        valid,
        key=lambda r: (
            not bool(r.get("token_closure")),
            not bool(r.get("after_answer_unit_closure")),
            not bool(r.get("after_answer_class_closure")),
            finite(r.get("after_answer_unit_variant_count_above_unit"), 1e9),
    ...
    return {
        "phase": 814,
        "title": "Tokenizer Valid Answer Unit Closure",
        "model": args.model,
        "round": args.round_name,
        ...
        "answer_unit_closure_rows": sum(1 for r in valid if r.get("after_answer_unit_closure")),
        "valid_answer_unit_closure_rows": sum(
            1 for r in valid if r.get("after_answer_class_closure") and r.get("after_answer_unit_closure")
        ),
        "canonical_surface_closure_rows": sum(1 for r in valid if r.get("after_canonical_surface_closure")),
        "best_rows": best,
        "by_label": dict(Counter(r.get("phase814_label") for r in valid)),
        ...
        "strict_boundary": (
            "This phase tests whether answer-class closure becomes surface-unit closure after replacing raw target token "
            "with a prompt-conditioned tokenizer-valid answer unit."
        ),
    }
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    write_jsonl(out_dir / f"phase814_{args.model}_rows.jsonl", rows)
    write_json(out_dir / f"phase814_{args.model}_summary.json", summary)
    print(
        json.dumps(
            {
                "model": args.model,
...
def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        f"# Phase 814 Tokenizer Valid Answer Unit Closure ({payload['round']})",
        "",
    ...
    for model_name in MODELS:
        ...
        for row in data.get("best_rows", [])[:40]:
            ...
            lines.append(
                f"| {model_name} | {row.get('search_stage')} | {row.get('case_id')} | "
                f"`{row.get('after_answer_unit_text')}` | {int(bool(row.get('after_answer_unit_is_raw_target')))} | "
                f"{row.get('combo_size')} | "
                ...
                f"{int(bool(row.get('after_answer_unit_closure')))} | "
                f"{fmt(row.get('after_answer_unit_rank_within_answer_class'))} | "
                f"{fmt(row.get('after_answer_unit_margin_vs_best_variant'))} | "
                f"{fmt(row.get('after_answer_unit_variant_count_above_unit'))} | "
                f"{int(bool(row.get('after_canonical_surface_closure')))} | "
                f"{int(bool(row.get('token_closure')))} | "
                f"{fmt(row.get('objective_score'))} | `{row.get('phase814_label')}` | `{items}` |"
            )
    lines += [
        "",
        "## By Label",
    ...
    for model_name in MODELS:
        ...
        lines.append(
            ...
            f"{data.get('valid_answer_unit_closure_rows')} | {data.get('canonical_surface_closure_rows')} | "
    ...
    for model_name in MODELS:
        ...
        for row in data.get("by_combo_size", [])[:40]:
            lines.append(
                ...
                f"{fmt(row.get('answer_unit_closure_rate'))} | "
                f"{fmt(row.get('mean_answer_unit_rank'))} | "
                f"{fmt(row.get('mean_answer_unit_margin'))} | "
                f"{fmt(row.get('mean_answer_unit_variants_above_unit'))} | "
                f"{fmt(row.get('mean_answer_unit_fragment_delta'))} | "
                f"{fmt(row.get('canonical_surface_closure_rate'))} | "
...
def summarize_round(round_name: str) -> dict[str, Any]:
    ...
    payload: dict[str, Any] = {"phase": 814, "round": round_name, "status": "missing", "model_summaries": {}, "models": []}
    for model_name in MODELS:
        path = out_dir / f"phase814_{model_name}_summary.json"
        if not path.exists():
            continue
    ...
    write_json(out_dir / "phase814_cross_model_summary.json", payload)
    write_markdown(out_dir / "phase814_cross_model_summary.md", payload)
    return payload
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--objective-canonical-surface-weight", type=float, default=8.0)
    parser.add_argument("--objective-canonical-margin-weight", type=float, default=1.0)
    parser.add_argument("--objective-answer-unit-weight", type=float, default=8.0)
    parser.add_argument("--objective-answer-unit-margin-weight", type=float, default=1.0)
    parser.add_argument("--objective-answer-class-preserve-weight", type=float, default=50.0)
