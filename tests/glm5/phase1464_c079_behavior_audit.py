#!/usr/bin/env python3
"""Independent audit for Phase1464 C079 behavior."""
...
def main() -> None:
    ...
    expected = [row for row in composition if all(by_case[row[key]]["correct"] for key in keys)]
    expected_auth = "run_phase1465_c079_discovery_full_field_capture" if summary["behavior_qualified"] else "close_c079_at_behavior_gate"
    checks = {
        "rows": len(rows) == 3456,
        ...
        "eligible_exact": [row["set_id"] for row in eligible] == [row["set_id"] for row in expected],
        "eligible_complete": all(all(by_case[row[key]]["correct"] for key in keys) for row in eligible),
    ...
    result = {"phase": 1464, "campaign": "C079", "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values())}
