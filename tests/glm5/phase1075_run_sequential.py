#!/usr/bin/env python3
"""Run all Phase1075 stages sequentially without model overlap."""
...
def main() -> None:
    ...
    parser.add_argument(
        "--resume",
        action="store_true",
        help="Skip a completed model stage.",
    )
    args = parser.parse_args()
    analysis_dir = protocol.OUT_ROOT / "analysis"
    analysis_dir.mkdir(parents=True, exist_ok=True)
    manifest = {
        "schema_version": "phase1075_run_manifest.v1",
    ...
    manifest["protocol_elapsed_seconds"] = run_command([
        "tests/glm5/phase1075_relation_polarity_protocol.py"
    ])
    protocol.write_json(
    ...
    for model in protocol.MODELS:
        ...
        if not stage["skipped_existing"]:
            stage["elapsed_seconds"] = run_command([
                "tests/glm5/phase1075_behavior_scan.py",
                "--model",
                model,
            ])
    ...
    manifest["behavior_finalize_elapsed_seconds"] = run_command([
        "tests/glm5/phase1075_behavior_finalize.py"
    ])
    ...
    if decision["should_run_internal_mapping"]:
        for model in protocol.MODELS:
            if model not in decision["selected_models"]:
                continue
            summary_path = (
                protocol.OUT_ROOT
                / "internal"
            ...
            if not stage["skipped_existing"]:
                stage["elapsed_seconds"] = run_command([
                    "tests/glm5/phase1075_internal_scan.py",
                    "--model",
                    model,
                ])
            stage["completed_at_utc"] = timestamp()
    ...
    manifest["finalize_elapsed_seconds"] = run_command([
        "tests/glm5/phase1075_finalize.py"
    ])
    ...
    manifest["audit_elapsed_seconds"] = run_command([
        "tests/glm5/phase1075_result_audit.py"
    ])
    protocol.write_json(
        analysis_dir / "run_manifest.json", manifest
    )
