#!/usr/bin/env python3
"""Execute every Phase1072 stage with strictly sequential model loading."""
...
def new_manifest() -> dict[str, Any]:
    return {
        "schema_version": "phase1072_run_manifest.v1",
...
def main() -> None:
    ...
    parser.add_argument(
        "--resume-formal",
        action="store_true",
        help="reuse completed calibration and restart formal stages",
    )
    ...
    if args.resume_formal:
        ...
        payload["resumed_after_protocol_metadata_repair"] = True
        payload["resume_started_at_utc"] = timestamp()
        write_manifest(payload)
        run_formal(payload)
        print("Phase1072 resumed formal run complete")
        return
    ...
    run_formal(payload)
    print("Phase1072 sequential run complete")
