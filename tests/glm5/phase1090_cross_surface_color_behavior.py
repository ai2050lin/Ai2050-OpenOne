#!/usr/bin/env python3
"""Run Phase1090 cross-surface behavior screen, one FP16 model at a time."""
...
def generation_selection(rows: list[dict]) -> list[dict]:
    ...
    for key in sorted(by_cell_split):
        ...
        choices = (
            (unit_ids[0], "cactive_m0_q0"),
            (unit_ids[1], "cfield_null_m1_q0"),
        )
...
def run(model_name: str) -> None:
    ...
    if not audit["all_checks_passed"]:
        raise RuntimeError("Phase1090 static protocol audit failed")
    ...
    try:
            ...
            for length in sorted(by_length):
                ...
                for start in range(0, len(panel), BATCH_SIZE[model_name]):
                    ...
                    for slot, row in enumerate(batch):
                        ...
                        key = (
                            row["surface_route"], row["operation"],
        ...
        for row in generation_rows:
            ...
            key = (row["surface_route"], row["panel"])
            gen_totals[key] += 1
        ...
        for route in protocol.SURFACE_ROUTES:
            ...
            generation_by_route[route] = panels

        result = {
            "schema_version": "phase1090_behavior_result.v1",
            "phase": protocol.PHASE,
            "model": model_name,
            ...
            "per_cell": per_cell,
            "generation_by_route": generation_by_route,
