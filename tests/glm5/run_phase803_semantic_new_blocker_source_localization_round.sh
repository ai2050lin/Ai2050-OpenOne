...
case "$ROUND" in
  ...
  confirm)
    ...
    MAX_ROUTES=2
    ROUTE_SIZES="6,8"
    ALPHA_GRID="0,0.5,0.75,1.0,1.25"
    MAX_SEMANTIC=64
    ;;
  *)
...
COMMON_ARGS=(
  ...
  --min-true-semantic-suppression 0.25
  --max-semantic-still-rate 0.20
  --min-threshold-gain 0.75
  --component-source
  --component-source-alpha 0.0
...
for MODEL in qwen3 glm4 deepseek7b; do
  echo "[$(date +%H:%M:%S)] phase803 $ROUND: start $MODEL"
  python tests/glm5/phase803_semantic_new_blocker_source_localization.py \
    --model "$MODEL" \
    "${COMMON_ARGS[@]}"
  echo "[$(date +%H:%M:%S)] phase803 $ROUND: done $MODEL"
done
