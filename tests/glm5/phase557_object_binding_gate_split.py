...
Phase556 showed that GLM4 restore is a category-route-gate mixed state and
that shuffled vehicle donors can still restore strongly. This phase separates
object binding from category state and tests whether same-norm random cached
activations can open the same generic gate.
"""
...
OUT_ROOT = Path("results/glm5_phase557_object_binding_gate_split")
...
DEFAULT_CONDITIONS = [
    ...
    "resid_donor_vehicle_repeat_add",
...
def condition_plan(condition: str) -> dict[str, Any]:
    ...
    if condition.startswith(prefix):
        ...
        variants = {"same", "near", "far", "shuffle", "repeat"}
    ...
    for start in range(0, len(texts), batch_size):
        ...
        if plan["site"] != "none":
            if plan["restore"] is not None:
                ...
                if plan.get("donor_variant") == "random_cache":
                    for lid, cached in list(restore_cache.items()):
                        gen = torch.Generator(device=cached.device)
                        gen.manual_seed(917000 + lid * 1000 + start)
                        rand = torch.randn(
                            cached.shape, generator=gen, device=cached.device, dtype=torch.float32
                        )
                        rand = rand / (rand.norm(dim=-1, keepdim=True) + 1e-8)
                        norms = cached.float().norm(dim=-1, keepdim=True)
                        restore_cache[lid] = (rand * norms).to(cached.dtype)
...
def build_category_donor_rows(
    ...
    if repeat and objects:
        objects = [objects[0] for _ in range(len(objects))]
    if objects:
        n = len(objects)
...
def donor_rows_for(
    ...
    if donor_variant in {"same", "random_cache"}:
        ...
        repeat = False
    elif donor_variant == "repeat":
        shift = 0
        repeat = True
    else:
        shift = 1 if donor_category == PAIR_SPECS[pair][0] else 0
        repeat = False
    return build_category_donor_rows(pair, donor_scaffold, donor_category, test_n, shift, repeat)
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if len(windows) != 1:
            raise ValueError(f"Phase557 expects one window, got {windows}")
        ...
        log(f"{args.model}: phase557 pair={pair}, combos={combos}, routes={[r['name'] for r in routes]}")
        ...
        return {
            ...
            "restore_note": "object-binding split writes same/near/far/shuffle/repeat/random-cache donor activations after projection removal",
...
def write_tsv(result: dict[str, Any], out_dir: Path, model_name: str) -> None:
    ...
    path = out_dir / f"phase557_{model_name}_readable_samples.tsv"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_path = out_dir / f"phase557_{args.model}_object_binding_gate_split.json"
