#!/usr/bin/env python3
"""Independently audit all frozen Phase1114 artifacts."""
...
def check_model(model_name: str, preregistration: dict[str, Any]) -> dict[str, Any]:
    ...
    finite_details = [row for row in details if row["finite"]]
    recomputed_hits = [
        row["expected_margin"] > 0.0 for row in finite_details
    ]
    checks = {
        "case_digest": protocol.digest(cases)
        == preregistration["case_digests"][model_name],
        "summary_digest": protocol.digest(
            {key: value for key, value in summary.items() if key != "summary_digest"}
        )
        == summary["summary_digest"],
        "counts": len(cases) == len(details) == summary["candidate_count"] == 432,
        ...
        "scores_recompute_z": all(
            ...
            for row in finite_details
        ),
        "finite_fraction_recomputed": abs(
            len(finite_details) / len(details)
            - summary["candidate_finite_fraction"]
        )
        < 1e-12,
        "accuracy_recomputed": abs(
            sum(recomputed_hits) / max(len(finite_details), 1)
...
def main() -> None:
    ...
    global_checks = {
        ...
        "protocol_digest_matches_audit": preregistration["protocol_digest"]
        == protocol_audit["protocol_digest"],
        "protocol_digest_recomputed": protocol.digest(
            {
                key: value
        ...
        "final_digest_recomputed": protocol.digest(
            {key: value for key, value in final.items() if key != "final_digest"}
        )
        == final["final_digest"],
        "authorization_digest_recomputed": protocol.digest(
            {
                key: value
        ...
        "authorization_matches_final": authorization["qualified_models"]
        == final["qualified_models"]
    ...
    audit = {
        "schema_version": "phase1114_result_audit.v1",
