#!/usr/bin/env python3
"""Independent audit for Phase1376."""
...
SCRIPT = TESTS / "phase1376_c059_qwen_behavior_qualification.py"
...
def main() -> None:
    ...
    expected_auth = ("run_phase1377_c059_instrument_calibration" if qualified
    ...
    checks = {
        ...
        "active_accuracy": abs(summary["active"]["accuracy"] -
                               sum(r["correct"] for r in active) / len(active)) < 1e-12,
        "status_accuracy": abs(summary["status"]["accuracy"] -
                               sum(r["correct"] for r in status) / len(status)) < 1e-12,
        "selected_matches_gate": (len(selected) == expected_selected and
                                  len({r["pair_id"] for r in selected}) == expected_selected)
        ...
        "cell_matches_gate": (set(Counter((r["target_family"], r["partition"], r["surface"])
                                          for r in selected).values()) == {per_cell})
                             if qualified else True,
    ...
    audit = {"phase": 1376, "campaign": "C059", "checks": checks,
