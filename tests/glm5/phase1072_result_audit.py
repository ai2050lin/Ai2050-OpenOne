#!/usr/bin/env python3
"""Strict integrity audit for all Phase1072 artifacts."""
...
SCRIPT_NAMES = (
    "phase1072_bidirectional_pattern_protocol.py",
    "phase1072_behavior_calibration_protocol.py",
    "phase1072_behavior_calibration_scan.py",
    "phase1072_behavior_calibration_finalize.py",
    "phase1072_bidirectional_pattern_scan.py",
    "phase1072_finalize.py",
    "phase1072_posthoc_diagnostics.py",
    "phase1072_run_sequential.py",
    "phase1072_result_audit.py",
)
...
def main() -> None:
    ...
    checks["source_phase1071_preserved"] = bool(
        not source_automatic["should_continue_automatically"]
        and source_automatic["route"]
        == "stop_at_exposure_pattern_atlas"
        and source_audit["all_integrity_checks_passed"]
    )

    expected_formal_cases = next(
        iter(prereg["case_counts"].values())
    )
    expected_calibration_cases = next(
    ...
    for model in protocol.MODELS:
        ...
        skeletons_valid = calibration_skeletons <= formal_skeletons
        all_formal_skeletons = (
            all_formal_skeletons and skeletons_valid
        )
        ...
        calibration_model_summary = protocol.read_json(
            calibration_atlas / "summary.json"
        )
        formal_model_summary = protocol.read_json(
            formal_atlas / "summary.json"
        )
        precision = formal_model_summary["precision"]
        model_precision_valid = bool(
            ...
            and formal_model_summary["placement"]["quantization"]
            == "none"
        )
        ...
        counts = {
            ...
            "readout_metrics": line_count(
                formal_atlas / "local_readout_metrics.jsonl"
            ),
        }
        expected_response = (
            len(protocol.RELATION_NAMES)
            ...
            * int(formal_model_summary["event_count"])
        ...
        model_counts_valid = bool(
            counts["formal_cases"] == expected_formal_cases
            and counts["calibration_cases"]
            == expected_calibration_cases
            and counts["calibration_candidate"]
            == expected_calibration_cases
            and counts["calibration_natural"]
            == expected_calibration_cases
            and counts["formal_candidate"]
            == expected_formal_cases
            and counts["formal_natural"] == expected_natural
            and counts["response_metrics"] == expected_response
            and counts["readout_metrics"] == expected_readout
            and int(formal_model_summary["case_count"])
            == expected_formal_cases
            and int(calibration_model_summary["case_count"])
            == expected_calibration_cases
        )
        record_counts_valid = (
            record_counts_valid and model_counts_valid
        )
        model_details[model] = {
            "counts": counts,
            "expected_response_metrics": expected_response,
            "expected_readout_metrics": expected_readout,
            "counts_valid": model_counts_valid,
            "prompt_skeleton_linkage_valid": skeletons_valid,
            "fp16_no_quantization_valid": model_precision_valid,
            "elapsed_seconds": {
                "calibration": calibration_model_summary[
                    "elapsed_seconds"
                ],
                "formal": formal_model_summary["elapsed_seconds"],
            },
        }
    ...
    checks["hard_negative_gate_consistent"] = all(
        bool(row["checks"]["hard_negative"])
        == (
            row["hard_negative_process_did_max"]
            is not None
            and float(row["hard_negative_process_did_max"])
    ...
    checks["automatic_decision_consistent"] = bool(
        ...
        and automatic["repeated_model_count"]
        == len(selected_models)
        and automatic["should_continue_automatically"]
        == expected_continue
        and automatic["next_phase"]
        == (1073 if expected_continue else None)
    )
    ...
    result = {
        "schema_version": "phase1072_integrity_audit.v1",
