#!/usr/bin/env python3
"""Independent audit for Phase1431."""
...
def main() -> None:
    ...
    expected = "run_phase1432_c071_role_map_camera" if summary["behavior_qualified"] else "close_c071_at_behavior_gate"
    ...
    checks = {
        "rows": len(rows) == 2880,
        "surfaces": Counter(row["surface"] for row in rows) == {surface: 1440 for surface in protocol["material"]["surfaces"]},
        "cells": Counter(row["cell"] for row in rows) == {cell: 360 for cell in ("aa", "ab", "ac", "ad", "bb", "ba", "bc", "bd")},
        ...
        "partition_balance": all(sum(row["family"] == family and row["partition"] == partition for row in selected) == 4 for family in summary["qualified_families"] for partition in protocol["material"]["partitions"]),
        "surface_gate": all(value["balanced_accuracy"] >= 0.95 for value in summary["surface_global"].values()),
    ...
    result = {
        "phase": 1431, "campaign": "C071", "checks": checks,
