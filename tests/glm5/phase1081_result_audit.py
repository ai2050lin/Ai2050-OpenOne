#!/usr/bin/env python3
"""Audit Phase1081 artifacts and seal the automatic-next decision."""
...
def main() -> None:
    ...
    required = [
        ...
        protocol.OUT_ROOT / "analysis" / "exact_assignments.json",
        protocol.OUT_ROOT / "analysis" / "behavior_audit.json",
    ...
    vector_fields = (
        "active_route", "duplicate_route", "content_route",
        "content_label0", "content_label1", "answer",
        "query_active", "query_duplicate",
    )
    scalar_fields = (
        "label_swap", "shell", "content_cross_label",
        "content_cross_shell", "active_duplicate_cosine",
    ...
    for model in protocol.MODELS:
        ...
        expected_generation_count = (
            ...
            * int(prereg["generation_units_per_family_split"])
        )
        checks = {
            ...
            "generation_active_panel_only": all(
                row["state"].split("_")[1] == "cactive"
                for row in generations
            ),
            "generation_query_coverage": {
                int(row["state"].split("_")[3][1:])
                for row in generations
            } == {0, 1},
            ...
            "pre_query_causal_order_zero": (
                float(summary["pre_query_global_max_abs"])
    ...
    global_checks = {
        ...
        "posthoc_cannot_change_frozen_predictions": final[
            "posthoc_role_diagnostics"
        ]["status"] == "posthoc_cannot_change_frozen_predictions",
    }
    all_checks = all(global_checks.values()) and all(
        row["all_checks_passed"] for row in model_audits.values()
    )
    empirical_continue = bool(
        automatic["continue_to_local_causal_sampling"]
    )
    automatic["integrity_audit_passed"] = all_checks
    automatic["continue_to_local_causal_sampling"] = (
        empirical_continue and all_checks
    )
    if not all_checks:
        automatic["reason"] = (
            "Integrity audit failed. Automatic continuation is forbidden."
        )
    elif automatic["continue_to_local_causal_sampling"]:
        automatic["reason"] = (
    ...
    audit = {
        "schema_version": "phase1081_result_integrity_audit.v1",
    ...
    protocol.write_json(
        protocol.OUT_ROOT / "analysis" / "result_integrity_audit.json", audit
    )
    if not all_checks:
    ...
    print({
        ...
        "automatic_continue": automatic[
            "continue_to_local_causal_sampling"
        ],
