#!/usr/bin/env python3
"""Independent C131 contract and final evidence audit."""
...
OUT = TESTS / "result/phase1665_c131_composed_precedence_repaired_transition"
...
import phase1665_c131_composed_precedence_repaired_transition as c131
...
def contract_audit() -> None:
    ...
    checks = {
        ...
        "surface_repair": all("comes before" in row["prompt"] and "appears earlier" not in row["prompt"] for row in cases),
        "roles": all(set(row["role_positions"]) == set(c131.ROLES) for row in compiled),
    ...
    report = {"phase": 1665, "campaign": "C131", "stage": "contract", "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values()), "authorization": "run_c131_behavior" if all(checks.values()) else "stop"}
...
def final_audit() -> None:
    ...
    batch = core.load(PUBLIC)["c131_composed_precedence_repaired_transition_batch"]
    checks = {
    ...
    report = {"phase": 1665, "campaign": "C131", "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values()), "scientific_gates": {"behavior": core.load(OUT / "analysis/behavior_gate.json")["gate_passed"], "within_family": confirmation["within_family"]["all_gates_passed"], "cross_family_common_response": confirmation["cross_family_common_response"]["all_gates_passed"], "composition_residual": confirmation["composition_residual"]["all_gates_passed"]}, "authorization": "integrate_client_append_phase1665_and_consider_c132" if all(checks.values()) else "stop"}
...
def failure_audit() -> None:
    ...
    checks = {
        ...
        "closure": core.load(OUT / "audit/internal_closure_audit.json")["all_checks_passed"],
        "boundary": "no HiddenState or mechanism result" in closure["claim_boundary"],
    }
    report = {"phase": 1665, "campaign": "C131", "stage": "behavior_failure_closure", "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values()), "scientific_gate_passed": False, "authorization": "start_c132_fixed_main_frame" if all(checks.values()) else "stop"}
