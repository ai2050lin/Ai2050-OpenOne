...
RESULT_ROOT = Path("tests/result/phase932_fixed_gear_repair_case_residual_audit")
...
def ordered_union(*groups: list[int]) -> list[int]:
    out: list[int] = []
    seen: set[int] = set()
    for group in groups:
        for raw in group:
...
def channels_from_feature_row(row: dict[str, Any]) -> list[int]:
    return [int(x) for x in row.get("margin_support_pos_64_channels") or []]


def case_residual_inventory(model_name: str, args: argparse.Namespace, fixed_top64: list[int]) -> dict[str, Any]:
    ...
    for row in rows:
        group = channels_from_feature_row(row)
        if group:
    ...
    for case_id, case_rows in sorted(rows_by_case.items()):
        groups = [channels_from_feature_row(row) for row in case_rows]
        ...
        for group in groups:
            ...
            intersection &= unique
            union |= unique
        topfreq64 = [int(ch) for ch, _count in counter.most_common(64)]
        cases[case_id] = {
            "state_count": len(case_rows),
            "intersection_size": len(intersection),
            "union_size": len(union),
            "topfreq64_size": len(topfreq64),
            ...
            "topfreq64_minus_fixed_top64": [int(ch) for ch in topfreq64 if ch not in fixed],
            "top_channels": [{"channel": int(ch), "count": int(count)} for ch, count in counter.most_common(20)],
        }
    return {
...
def build_specs(args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    group_kinds = [
        ...
        "fixed_plus_chair_inter_residual",
        "fixed_plus_chair_union_residual",
        "fixed_plus_case_inter_residual",
        "fixed_plus_case_union_residual",
        "state_specific_margin_support_pos_64",
    ]
    for factor in p928.parse_floats(args.factors):
        for group_kind in group_kinds:
            control_class = "state_specific_reference" if group_kind == "state_specific_margin_support_pos_64" else "fixed_repair_candidate"
            specs.append(
                {
                    "control_label": f"{group_kind}_{factor:g}",
                    "control_family": "fixed_gear_repair_case_residual",
...
def residual_group(inventory: dict[str, Any], case_id: str, residual_name: str) -> list[int]:
    case_item = (inventory.get("cases") or {}).get(case_id) or {}
    return [int(x) for x in case_item.get(residual_name) or []]
...
def group_for_spec(
    ...
    chair_inter = residual_group(inventory, chair_case_id, "intersection_minus_fixed_top64")
    chair_union = residual_group(inventory, chair_case_id, "union_minus_fixed_top64")
    case_inter = residual_group(inventory, case_id, "intersection_minus_fixed_top64")
    case_union = residual_group(inventory, case_id, "union_minus_fixed_top64")
    if group_kind == "coordinate_only":
        return [], "coordinate_only"
    if group_kind == "fixed_topfreq_64":
        return fixed_top64, "fixed_topfreq_64"
    if group_kind == "chair_inter_residual_only":
        return chair_inter, f"{chair_case_id}:intersection_minus_fixed_top64"
    if group_kind == "chair_union_residual_only":
        return chair_union, f"{chair_case_id}:union_minus_fixed_top64"
    if group_kind == "case_inter_residual_only":
        return case_inter, f"{case_id}:intersection_minus_fixed_top64"
    if group_kind == "case_union_residual_only":
        return case_union, f"{case_id}:union_minus_fixed_top64"
    if group_kind == "fixed_plus_chair_inter_residual":
        return ordered_union(fixed_top64, chair_inter), f"fixed_topfreq_64+{chair_case_id}:intersection_minus_fixed_top64"
    if group_kind == "fixed_plus_chair_union_residual":
        return ordered_union(fixed_top64, chair_union), f"fixed_topfreq_64+{chair_case_id}:union_minus_fixed_top64"
    if group_kind == "fixed_plus_case_inter_residual":
        return ordered_union(fixed_top64, case_inter), f"fixed_topfreq_64+{case_id}:intersection_minus_fixed_top64"
    if group_kind == "fixed_plus_case_union_residual":
        return ordered_union(fixed_top64, case_union), f"fixed_topfreq_64+{case_id}:union_minus_fixed_top64"
...
def state_coverage(rows: list[dict[str, Any]], group_kind: str, factor: float | None = None, case_id: str | None = None) -> int:
    ...
    for row in rows:
        if row.get("candidate_group_kind") != group_kind:
            continue
        if factor is not None and abs(float(row.get("candidate_group_factor") or 0.0) - float(factor)) > 1e-9:
            continue
        if case_id is not None and str(row.get("target_case_id")) != case_id:
            continue
        if row.get("new_top1_vs_coordinate_base") or row.get("new_margin_closure_vs_coordinate_base"):
            keys.add(str(row.get("target_state_key")))
    return len(keys)
...
def evidence_label(selected_count: int, rows: list[dict[str, Any]]) -> str:
    ...
    repair_rows = [row for row in rows if str(row.get("control_class")) == "fixed_repair_candidate"]
    if any(row.get("new_strict_vs_coordinate_base") for row in repair_rows):
        return "fixed_repair_strict_clean_positive"
    fixed_cov = state_coverage(rows, "fixed_topfreq_64", 2.25)
    repaired_cov = max(
        state_coverage(rows, "fixed_plus_case_inter_residual", 2.25),
        state_coverage(rows, "fixed_plus_case_union_residual", 2.25),
    )
    reference_cov = state_coverage(rows, "state_specific_margin_support_pos_64", 2.25)
    if repaired_cov >= reference_cov and repaired_cov > fixed_cov:
        return "case_residual_repair_matches_state_specific_without_strict_clean"
    if repaired_cov > fixed_cov:
        return "case_residual_repair_partial_positive_without_strict_clean"
    if reference_cov > fixed_cov:
        return "case_residual_repair_negative_state_specific_remains_needed"
    if fixed_cov > 0:
        return "fixed_common_positive_no_residual_gain"
    return "no_fixed_repair_positive"
...
def eval_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    inventory = case_residual_inventory(args.model, args, fixed_top64)
    ...
    if args.dry_run or not selected or not fixed_top64:
        ...
        payload = {
            "phase": PHASE,
            "title": "Fixed Gear Repair and Case-Specific Residual Audit",
            "model": args.model,
            "status": status,
            ...
            "case_residual_inventory": inventory,
            "overall": p928.summarize_rows([]),
            "repair_comparison": [],
            "evidence_label": status,
        }
        p846.write_json(out_dir / f"phase932_{args.model}_summary.json", payload)
        p846.write_jsonl(out_dir / f"phase932_{args.model}_rows.jsonl", [])
        p846.write_jsonl(out_dir / f"phase932_{args.model}_selected_seeds.jsonl", selected)
    ...
    try:
        ...
        for idx, seed in enumerate(selected, 1):
            ...
            if state is not None:
                states.append(state)
            log(f"{args.model}/{args.round_name}: reconstructed_repair_seed={idx}/{len(selected)} kept={len(states)}")
        for state_idx, state in enumerate(states, 1):
            ...
            for alpha, protocol_factor in coords:
                for base_spec in specs:
                    ...
                    candidate_group, group_source = group_for_spec(
                        state,
                        fixed_top64,
                        inventory,
                        group_kind,
                        str(args.chair_case_id),
                    )
                    ...
                    row["phase"] = PHASE
                    row["row_kind"] = "phase932_fixed_gear_repair_case_residual_row"
            ...
            if state_idx % max(1, int(args.log_every)) == 0 or state_idx == len(states):
                log(f"{args.model}/{args.round_name}: repair_state={state_idx}/{len(states)} rows={len(rows)}")
    finally:
    ...
    payload = {
        ...
        "by_group_factor": p928.summarize_by(rows, ["candidate_group_kind", "candidate_group_factor"], limit=240),
        ...
        "boundary": "fixed common gear repair and case residual causal audit only; no natural gate or strict-clean closure claim",
    }
    p846.write_json(out_dir / f"phase932_{args.model}_summary.json", payload)
    p846.write_jsonl(out_dir / f"phase932_{args.model}_rows.jsonl", rows)
    p846.write_jsonl(out_dir / f"phase932_{args.model}_selected_seeds.jsonl", selected)
...
def summarize_round(round_name: str) -> dict[str, Any]:
    ...
    summaries = [read_json(out_dir / f"phase932_{model}_summary.json") for model in MODELS]
    ...
    for summary in summaries:
        ...
        inventories[model_name] = summary.get("case_residual_inventory")
    ...
    payload = {
        ...
        "case_residual_inventories": inventories,
        "top_group_factor_rows": group_rows[:160],
        "top_repair_comparison_rows": repair_rows[:120],
        "model_summaries": summaries,
    }
    p846.write_json(out_dir / "phase932_cross_model_summary.json", payload)
    write_summary_md(out_dir / "phase932_cross_model_summary.md", payload)
    return payload


def write_summary_md(path: Path, payload: dict[str, Any]) -> None:
    lines = ["# Phase 932 fixed gear repair and case-specific residual audit", "", "## Overall", ""]
    ...
    for key, value in sorted((payload.get("evidence_label_counts") or {}).items()):
        lines.append(f"- {key}: {value}")
    lines += ["", "## Case Residual Inventory", ""]
    for model_name, inventory in sorted((payload.get("case_residual_inventories") or {}).items()):
        lines.append(f"### {model_name}")
        lines.append("")
        for case_id, item in sorted(((inventory or {}).get("cases") or {}).items()):
            lines.append(
                f"- {case_id}: states={item.get('state_count')}, intersection={item.get('intersection_size')}, union={item.get('union_size')}, inter_residual={len(item.get('intersection_minus_fixed_top64') or [])}, union_residual={len(item.get('union_minus_fixed_top64') or [])}"
            )
        lines.append("")
    ...
    for row in payload.get("top_repair_comparison_rows") or []:
        lines.append(
            "| {model} | {candidate_group_kind} | {candidate_group_factor} | {state_coverage_all} |".format(
                **row
            )
        )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--round-name", default="fixed_gear_repair_case_residual_audit")
    ...
    parser.add_argument("--chair-case-id", default="p856_035_object_chair")
