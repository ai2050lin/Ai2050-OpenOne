#!/usr/bin/env python3
"""Independent pre/post audit for Phase1301."""
...
import numpy as np
ROOT=Path(__file__).resolve().parents[2];T=ROOT/"tests/glm5";OUT=T/"result/phase1301_c032_qwen3_behavior";PARENT=T/"result/phase1300_c032_execution_compiler_competition";CONTRACT=T/"result/phase1299_c032_execution_compiler_contract";P=OUT/"protocol/preregistration.json";PRE=OUT/"audit/independent_preaudit.json";POST=OUT/"audit/independent_final_audit.json";RAW=OUT/"raw/candidate_scores.jsonl";GEN=OUT/"raw/list_free_generations.jsonl";S=OUT/"analysis/behavior_summary.json";F=OUT/"analysis/final.json";C=OUT/"protocol/formal_run_complete.json";MAIN=T/"phase1301_c032_qwen3_behavior.py";SCRIPT=Path(__file__).resolve()
...
def base(p):
 c=[];timeless={k:v for k,v in p.items() if k not in {"created_at_utc","protocol_digest"}};add(c,"digest",digest(timeless)==p["protocol_digest"],p["protocol_digest"]);add(c,"sources",p["source_hashes"]=={"main":sha(MAIN),"auditor":sha(SCRIPT)},p["source_hashes"]);add(c,"parent",load(PARENT/"analysis/final.json")["authorization"]=="phase1301_qwen3_behavior_only" and load(PARENT/"audit/independent_final_audit_after_erratum.json")["all_checks_passed"],"parent");rt=load(PARENT/"protocol/frozen_runtime.json");add(c,"runtime",rt["selected_runtime"]=="right_padding" and rt["tau"]==1e-7,rt);add(c,"material",p["material"]["sha256"]==sha(CONTRACT/"material/frozen_inverse_lookup_cases.jsonl") and p["material"]["case_count"]==6912,p["material"]);add(c,"hidden_forbidden",p["hidden_states_read"] is False,p["hidden_states_read"]);add(c,"single_run",p["formal_run_budget"]==1,p["formal_run_budget"]);return c
def write(path,c,stage,auth):
 ok=all(x["passed"] for x in c);d={"phase":1301,"campaign":"C032","audit_stage":stage,"created_at_utc":datetime.now(timezone.utc).isoformat(),"auditor_imports_main":False,"checks":c,"passed_count":sum(x["passed"] for x in c),"total_count":len(c),"all_checks_passed":ok,"authorization":auth if ok else "none","protocol_digest":load(P)["protocol_digest"]};path.parent.mkdir(parents=True,exist_ok=True);path.write_text(json.dumps(d,ensure_ascii=False,indent=2)+"\n",encoding="utf-8");print(canonical({"stage":stage,"passed":d["passed_count"],"total":d["total_count"],"authorization":d["authorization"]}));
 if not ok:raise SystemExit(1)
def pre():
 p=load(P);c=base(p);add(c,"clear",not any(x.exists() for x in (RAW,GEN,S,F,C)),"clear");write(PRE,c,"pre_model","run_phase1301_once")
def rate(x):x=list(x);return float(np.mean(x)) if x else 0.0
def post():
 ...
 coverage=rate(x["covered"] for x in gen);exact=rate(x["exact_correct"] for x in gen);gg=defaultdict(list)
 for x in gen:gg[(x["partition"],x["profile_index"],x["attribute"],x["panel"],x["surface"])].append(x)
 pair=rate(len(v)==2 and all(x["exact_correct"] for x in v) for v in gg.values());gen_gates={"coverage":coverage>=th["generation_coverage_min"],"accuracy":exact>=th["generation_accuracy_min"],"pair_success":pair>=th["generation_pair_success_min"]};add(c,"generation_gates",summary["generation"]["gates"]==gen_gates and summary["generation"]["passed"]==all(gen_gates.values()),gen_gates);passed=all(candidate_gates.values()) and all(gen_gates.values());auth="phase1302_event_identity_hidden_only" if passed else "close_c032_without_hidden";add(c,"authorization",load(F)["authorization"]==auth and load(F)["hidden_states_read"] is False,load(F));qa=summary["model_audit"];add(c,"fp16",qa["has_fp16_parameters"] and not qa["has_quantized_modules"],qa);add(c,"complete",load(C)["formal_runs_consumed"]==1,load(C));write(POST,c,"post_model",auth)
