#!/usr/bin/env python3
"""Independent audit for the C102 exact-BF16 full-field archive."""
...
def main() -> None:
    producer = TESTS / "phase1583_c102_qwen_full_field_capture.py"
    ...
    checks = {
        ...
        "prediction": core.sha(OUT / "protocol/frozen_coordinate_barcode_predictions.json") == adapter["prediction_sha256"],
        ...
        "index": len(index) == 1152 and index[0]["token_start"] == 0 and index[-1]["token_end"] == field.shape[1],
        ...
        "authorization": report["authorization"] == "run_phase1584_c102_staged_barcode_analysis",
    }
    result = {"phase": 1583, "campaign": "C102", "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values())}
