#!/usr/bin/env python3
"""Audit Phase1087 protocol, arrays, predictions, and result integrity."""
...
ANALYSIS_DIGEST_KEYS = {
    "shared_relation_field_audit.json": "shared_relation_field_digest",
    ...
    "posthoc_surface_pair_diagnostic.json": "posthoc_diagnostic_digest",
}
...
def main() -> None:
    ...
    checks: dict[str, bool] = {
        ...
        "authorization_digest": digest_valid(
    ...
    for model_name in protocol.MODELS:
        ...
        with np.load(npz_path) as archive:
            ...
            key_check = set(archive.files) == required
            expected_shape = (
            ...
            shape_check = archive["direction_sum"].shape == expected_shape
            arrays_finite = all(
            ...
            content_index = protocol.SIGNED_FIELDS.index("content")
            pre_indices = [
                protocol.CAPTURE_ROLES.index(role)
                for role in protocol.PRE_QUERY_ROLES
            ]
            pre_direction = archive["direction_sum"][:, :, :, pre_indices, content_index]
            pre_relative = archive["relative_sum"][:, :, :, pre_indices, content_index]
            all_prequery_exact = (
                float(np.max(np.abs(pre_direction))) == 0.0
                and float(np.max(np.abs(pre_relative))) == 0.0
            )
        row = {
            "summary_digest_valid": digest_valid(summary, "summary_digest"),
            "projection_digest_valid": digest_valid(
            ...
            "all_registered_prequery_roles_exact": all_prequery_exact,
        ...
        for path in (summary_path, projection_path, npz_path):
    ...
    result = {
        "schema_version": "phase1087_result_integrity_audit.v1",
