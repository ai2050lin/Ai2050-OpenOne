#!/usr/bin/env python3
"""Independent recomputation audits for C119."""
...
import phase1643_c119_default_override_repair_common as c119
...
def save(name: str, phase: int, checks: dict, authorization: str) -> None:
    report = {"phase": phase, "campaign": "C119", "checks": checks, "passed": sum(checks.values()), "total": len(checks), "all_checks_passed": all(checks.values()), "producer_sha256": core.sha(Path(__file__)), "authorization": authorization}
...
def contract() -> None:
    ...
    checks = {
        ...
        "producer": protocol["producer_sha256"] == core.sha(TESTS / "phase1643_c119_default_override_repair_common.py"),
        ...
        "partitions": Counter(row["partition"] for row in units) == {name: 8 for name in c119.PARTITIONS},
        "factorial": cells == {(partition, *cell): 8 for partition in c119.PARTITIONS for cell in __import__("itertools").product((1, -1), repeat=5)},
        ...
        "roles": all(set(row["role_positions"]) == set(c119.ROLES) for row in compiled),
        "candidates": all(len(value) == 1 for row in compiled for value in row["candidate_ids"]),
        "object": protocol["object"] == "default inheritance and item-specific conflicting exception override with explicit applicability interface",
        "boundary": all(term in protocol["claim_boundary"] for term in ("no weights", "attention/MLP", "common module", "new mathematics")),
        "authorization": protocol["authorization"] == "execute_phase1644_c119_cuda_capture",
    }
    save("independent_contract_audit", 1643, checks, protocol["authorization"])
...
def capture() -> None:
    ...
    checks = {
        ...
        "authorization": report["authorization"] == ("run_phase1645_c119_discovery" if report["behavior_gate_passed"] else "close_hidden_state_route_and_continue_campaign_missingness_ledger"),
    }
    save("independent_capture_audit", 1644, checks, report["authorization"])
...
def discover() -> None:
    ...
    e, r, state = c119.EFFECTS.index("override"), c119.ROLES.index(nomination["role"]), int(nomination["state"])
    ...
    winner = sorted(eligible, key=lambda row: (-row["score"], -row["split_half_cosine"], row["state"], c119.ROLES.index(row["role"])))[0]
    checks = {
        ...
        "authorization": report["authorization"] == "execute_phase1646_c119_validation",
    }
    save("independent_discovery_audit", 1645, checks, report["authorization"])
...
def validate() -> None:
    ...
    checks = {
        ...
        "boundary": report["leave_c119_out_common_residual"]["definition"].startswith("leave-C119-out"),
        "authorization": report["authorization"] == "run_phase1647_c119_synthesis_visualization_and_closure",
    }
    save("independent_validation_audit", 1646, checks, report["authorization"])
...
def closure() -> None:
    ...
    effects = [row for row in payload["effect_rows"] if row.get("dataset") == "C119"]
    raw = [row for row in payload["raw_rows"] if row.get("dataset") == "C119"]
    checks = {
        "internal": internal["all_checks_passed"],
        ...
        "batch": payload["campaign"] == "C109-C119" and "c119_batch" in payload,
        ...
        "authorization": closure["next_authorization"].startswith("C120"),
    }
    save("independent_closure_audit", 1647, checks, "append_C119_memo_and_verify_client")
