#!/usr/bin/env python3
"""Run Phase1115 independent context-modulation behavior for one FP16 model."""
...
def run(model_name: str) -> None:
    ...
    audit = protocol.base.read_json(protocol.OUT_ROOT / "protocol" / "audit.json")
    ...
    if protocol.base.digest(rows) != preregistration["case_digests"][model_name]:
        raise RuntimeError("Phase1115 case digest mismatch")
    ...
    try:
        ...
        summary = {
            "schema_version": "phase1115_context_modulation_behavior_summary.v1",
            "phase": protocol.PHASE,
            "model": model_name,
        ...
        summary["summary_digest"] = protocol.base.digest(summary)
        output_root = protocol.OUT_ROOT / "behavior" / model_name
        protocol.base.write_jsonl(output_root / "candidate_detail.jsonl", detail)
        protocol.base.write_json(output_root / "summary.json", summary)
        print(
            json.dumps(
                {
                    "phase": protocol.PHASE,
