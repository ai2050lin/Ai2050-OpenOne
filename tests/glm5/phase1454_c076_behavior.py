#!/usr/bin/env python3
"""Phase1454: behavior qualification for C076 explicit relation discrimination."""
...
def balanced_accuracy(rows: list[dict]) -> float:
    positive, negative = [row for row in rows if row["truth"]], [row for row in rows if not row["truth"]]
    return (accuracy(positive) + accuracy(negative)) / 2.0


def main() -> None:
    if (OUT / "analysis/final.json").exists():
        raise RuntimeError("Phase1454 exists")
    ...
    if contract_final["authorization"] != "run_phase1454_c076_behavior" or not contract_audit["all_checks_passed"]:
        raise RuntimeError("Phase1453 did not authorize behavior")
    ...
    try:
        ...
        repeat_error = max(abs(a["scores"][i] - b["scores"][i]) for a, b in zip(first, repeat) for i in range(2))
            ...
            for relation_id in protocol["relations"]:
                ...
                for surface in protocol["surfaces"]:
                    ...
                    metrics = {
                        ...
                        "truth": {str(v).lower(): accuracy([row for row in rows if row["truth"] == v]) for v in (True, False)},
        ...
        qualified_relations = [relation_id for relation_id in protocol["relations"] if all(detail[family][relation_id][surface]["qualified"] for family in detail for surface in protocol["surfaces"])]
        ...
        checks = {
            ...
            "all_composition_sets": len(eligible) == len(composition),
        ...
        summary = {
            ...
            "surface_global": surface_global, "family_relation_surface": detail, "qualified_relations": qualified_relations,
            "eligible_count": len(eligible), "eligible_partition_counts": {name: sum(row["partition"] == name for row in eligible) for name in protocol["partitions"]},
            "checks": checks, "behavior_qualified": qualified, "numeric_repeat_max_abs_diff": repeat_error,
        ...
        authorization = "run_phase1455_c076_discovery_full_field_capture" if qualified else "close_c076_at_behavior_gate"
        core.save(OUT / "analysis/final.json", {"phase": PHASE, "campaign": CAMPAIGN, "behavior_qualified": qualified, "qualified_relations": qualified_relations, "authorization": authorization})
