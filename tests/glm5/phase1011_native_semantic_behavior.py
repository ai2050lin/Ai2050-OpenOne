#!/usr/bin/env python3
"""Qualify Phase1011 native first-token and free-rollout behavior."""
...
def behavior_batch(
    *,
    ...
    for index, case in enumerate(cases):
        ...
        rows.append({
            "schema_version": "phase1011_native_behavior_row.v1",
            "phase": PHASE,
            "model": model_name,
            "family": case["family"],
            ...
            "semantic_gate": int(panel_ids[index]) == expected,
            "strict_teacher_gate": int(full_ids[index]) == expected,
            ...
            "strict_rollout_gate": bool(
                decoded_exact and terminated
            ),
        })
    return rows
...
def pair_rows(
    ...
    for unit in units:
        for operation in PAIR_OPERATIONS:
            ...
            rows.append({
                "schema_version": "phase1011_native_pair_qualification.v1",
                "phase": PHASE,
                "model": unit["model"],
                "family": unit["family"],
...
def run_model(model_name: str) -> dict[str, Any]:
    ...
    if int(protocol["protocol_revision"]) != PROTOCOL_REVISION:
        raise RuntimeError("Phase1011 protocol revision drift")
    ...
    try:
        ...
        for key, group in sorted(grouped.items()):
            ...
            print(
                ...
                f"{'/'.join(str(value) for value in key[:4])} "
                f"n={len(panel)} "
                f"panel={np.mean([r['semantic_gate'] for r in panel]):.3f} "
                f"top1={np.mean([r['strict_teacher_gate'] for r in panel]):.3f} "
        ...
        panels = aggregate(rows, ("family", "output_mode"))
        modes = aggregate(rows, ("output_mode",))
        families = aggregate(rows, ("family",))
        summary = {
            "schema_version": "phase1011_native_behavior_summary.v1",
            "phase": PHASE,
            "model": model_name,
            ...
            "panel_rates": panels,
            "output_mode_rates": modes,
            "family_rates": families,
            ...
            "claim_limit": (
                "first-token rollout is natural model generation but does "
