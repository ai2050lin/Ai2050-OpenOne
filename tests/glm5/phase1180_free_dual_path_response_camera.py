#!/usr/bin/env python3
"""Phase1180 norm-safe free-training endpoint and prefix response cameras."""
...
PHASE = 1180
OUT_ROOT = ROOT / "tests/glm5/result/phase1180_free_dual_path_response_camera"
PROTOCOL_PATH = OUT_ROOT / "protocol/preregistration.json"
CAMERA_PATH = OUT_ROOT / "analysis/frozen_camera.json"
SCRIPT_PATH = Path(__file__).resolve()
LIBRARY_PATH = ROOT / "tests/glm5/phase1180_free_training_library.py"
BASE_LIBRARY_PATH = ROOT / "tests/glm5/phase1179_free_training_library.py"
AUDIT_PATH = ROOT / "tests/glm5/phase1180_free_dual_path_response_camera_audit.py"
PREFLIGHT_PATH = ROOT / "tests/glm5_temp/phase1180_full_modulus_preflight.json"
BLOCKS_PER_TASK = 8
...
PREFLIGHT_TASKS = (
    lib.TaskSpec("preflight_p11", 11, (1, 4, 2), 0),
    lib.TaskSpec("preflight_p13", 13, (2, 5, 3), 1),
    lib.TaskSpec("preflight_p17", 17, (3, 6, 4), 2),
    lib.TaskSpec("preflight_p19", 19, (4, 7, 5), 3),
)
...
def protocol_payload() -> dict[str, Any]:
    payload = {
        "phase": PHASE,
        "schema_version": "phase1180.protocol.v1",
        "created_at_utc": datetime.now(timezone.utc).isoformat(),
        "purpose": (
            "Norm-safe free-weight symmetric dual-path calibration: freeze an endpoint response-spectrum camera and "
        ...
        "scripts": {
            ...
            "base_library_sha256": sha256_file(BASE_LIBRARY_PATH),
        ...
        "training": {
            ...
            "norm_target_multiplier": lib.NORM_TARGET_MULTIPLIER,
        },
        ...
        "workflow": [
            "pass full-modulus engineering preflight on tasks disjoint from both formal splits",
            "preregister",
            "run discovery",
        ...
        "engineering_preflight_digest": read_json(PREFLIGHT_PATH)["preflight_digest"],
    }
...
def preregister(force: bool) -> dict[str, Any]:
    ...
    if not PREFLIGHT_PATH.exists():
        raise RuntimeError("run the full-modulus engineering preflight before preregistration")
    ...
    if not preflight["passed"] or preflight["script_hashes"] != current_hashes:
        raise RuntimeError("engineering preflight is missing, failed, or stale")
...
def require_cuda() -> torch.device:
    if not torch.cuda.is_available():
        raise RuntimeError("Phase1180 requires CUDA")
...
def freeze_camera() -> dict[str, Any]:
    ...
    payload = {
        "phase": PHASE,
        "schema_version": "phase1180.frozen_camera.v1",
...
def analyze() -> dict[str, Any]:
    ...
    payload = {
        "phase": PHASE,
        "schema_version": "phase1180.final.v1",
        "protocol_digest": read_json(PROTOCOL_PATH)["protocol_digest"],
        ...
        "next_gate": (
            ...
            "mode assignment or antithetic gate bias is supplied; do not retune Phase1180 on its tasks."
        ),
    }
...
def engineering_preflight() -> dict[str, Any]:
    device = require_cuda()
    rows = []
    failures = []
    for task_index, task in enumerate(PREFLIGHT_TASKS):
        for cohort_index, cohort in enumerate(COHORTS):
            for config_index in range(CONFIGS_PER_BLOCK):
                seed = 1_180_000 + task_index * 10_003 + cohort_index * 1_009 + config_index * 37
                try:
                    checkpoints, truth = lib.train_system(task, cohort, seed, config_index, device)
                    rows.append({
                        "task": task.name,
                        ...
                        "cohort": cohort,
                        "config_index": config_index,
                        "family": truth["response_family"],
                        "final_train_accuracy": checkpoints[-1]["train_accuracy"],
                        "final_parameter_l2": checkpoints[-1]["parameter_l2"],
                        "norm_target": lib.NORM_TARGET_MULTIPLIER * task.modulus,
                    })
                except Exception as exc:  # pragma: no cover - serialized failure path
                    failures.append({
                        "task": task.name,
                        "cohort": cohort,
                        "config_index": config_index,
    ...
    family_breadth = all(
        {row["family"] for row in rows if row["task"] == task.name and row["cohort"] == cohort}
        == set(lib.MODES)
        for task in PREFLIGHT_TASKS
        for cohort in COHORTS
    )
    norm_error = max(
        (abs(row["final_parameter_l2"] - row["norm_target"]) for row in rows),
        default=float("inf"),
    )
    script_hashes = {
        "main_sha256": sha256_file(SCRIPT_PATH),
    ...
    payload = {
        ...
        "task_digests": [lib.digest(task_payload(task)) for task in PREFLIGHT_TASKS],
        "system_count": len(rows),
        "expected_system_count": len(PREFLIGHT_TASKS) * len(COHORTS) * CONFIGS_PER_BLOCK,
        "failure_count": len(failures),
        "failures": failures,
        "family_breadth": family_breadth,
        "minimum_train_accuracy": min((row["final_train_accuracy"] for row in rows), default=0.0),
        "maximum_norm_target_error": norm_error,
        "script_hashes": script_hashes,
    }
    payload["passed"] = (
        payload["system_count"] == payload["expected_system_count"]
        and payload["failure_count"] == 0
        and family_breadth
        and payload["minimum_train_accuracy"] >= 0.99
        and norm_error <= 1.0e-4
    )
    payload["preflight_digest"] = lib.digest(payload)
...
def probe() -> dict[str, Any]:
    ...
    write_json(ROOT / "tests/glm5_temp/phase1180_formal_probe.json", payload)
    return payload
...
def main() -> None:
    ...
    parser.add_argument(
        "command",
        choices=("probe", "preflight", "preregister", "run", "freeze-camera", "analyze"),
    )
