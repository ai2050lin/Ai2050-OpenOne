#!/usr/bin/env python3
"""Audit Phase1079 artifacts and seal the automatic-next decision."""
...
def main() -> None:
    ...
    required = [
        ...
        protocol.OUT_ROOT / "analysis" / "exact_assignments.json",
        protocol.OUT_ROOT / "analysis" / "answer_alignment.json",
    ...
    for model in protocol.MODELS:
        required.extend([
            ...
            protocol.OUT_ROOT / "atlas" / model / "selected_mean_directions.fp16.npz",
        ])
    ...
    for model in protocol.MODELS:
        ...
        expected_split_direction_count = (
            len(protocol.CONDITIONINGS)
            * len(protocol.FAMILIES)
            * int(summary["event_count"])
            * len(protocol.CAPTURE_ROLES)
        ...
        checks = {
            ...
            "generation_count": (
                len(generations) == expected_generation_count
            ),
            "metric_count": len(metrics) == expected_metric_count,
            "split_direction_count": (
                len(split_rows) == expected_split_direction_count
            ),
            ...
            "model_protocol_audit_passed": (
                case_audit["all_checks_passed"]
            ),
            "fp16_present": summary["precision"][
                "has_fp16_parameters"
            ],
            "bf16_absent": not summary["precision"][
                "has_bf16_parameters"
            ],
            "quantization_absent": not summary["precision"][
                "has_quantized_modules"
            ],
            "pre_mode_causal_order_zero": (
                float(summary["pre_mode_operation_max_abs"])
                <= float(
            ...
            "identity_repeat_exact": (
                float(summary["identity_maximum"]) == 0.0
            ),
            "all_families_retained": set(
    ...
    expected_predictions = set(protocol.PROSPECTIVE_PREDICTIONS)
    global_checks = {
        ...
        "protocol_digest_matches": (
            final["protocol_digest"] == prereg["protocol_digest"]
        ),
        "model_order_frozen": tuple(final["models"]) == protocol.MODELS,
        "case_count_total": (
            int(final["case_count_total"])
            == int(prereg["case_count_per_model"])
            * len(protocol.MODELS)
        ),
        "unit_count_total": (
            int(final["unit_count_total"])
            == int(prereg["unit_count_per_model"])
            * len(protocol.MODELS)
        ),
        ...
        "all_families_have_evidence": set(
            final["family_evidence"]
        ) == set(protocol.FAMILIES),
        ...
        "prediction_keys_match": (
            set(prediction["predictions"]) == expected_predictions
        ),
        "exact_permutation_count": all(
            int(row["permutation_count"]) == math.factorial(
                len(protocol.BASE_FAMILIES)
            )
            for row in assignments["rows"]
        ),
        "all_finite_primary": (
            str(prereg["primary_population"]).startswith(
                "All preregistered finite states"
            )
        ),
        "new_mathematics_not_predeclared": (
            final["mathematical_status"][
    ...
    if not all_checks:
        ...
        automatic["reason"] = (
            "Integrity passed, but the frozen empirical continuation gate "
            "did not pass. Do not select components or neurons from peaks."
        )
    ...
    audit = {
        "schema_version": "phase1079_integrity_audit.v1",
