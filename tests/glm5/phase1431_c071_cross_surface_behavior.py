#!/usr/bin/env python3
"""Phase1431: Qwen3 cross-surface behavior qualification for C071."""
...
PHASE, CAMPAIGN = 1431, "C071"
CONTRACT = TESTS / "result/phase1430_c071_cross_surface_role_contract"
OUT = TESTS / "result/phase1431_c071_cross_surface_behavior"
BATCH = 24
...
def main() -> None:
    if (OUT / "analysis/final.json").exists():
        raise RuntimeError("Phase1431 exists")
    ...
    if contract_final["authorization"] != "run_phase1431_c071_cross_surface_behavior" or not contract_audit["all_checks_passed"]:
        raise RuntimeError("Phase1430 did not authorize behavior")
    ...
    set_keys = tuple(
        ...
        for kind in ("true_recipient", "false_recipient", "true_donor", "false_donor")
    )
    model = None
    try:
        ...
        for family in protocol["material"]["families"]:
            ...
            family_results[family] = {
                "surfaces": surface_results, "eligible_count": len(family_eligible),
                "eligible_partition_counts": partition_counts, "checks": family_checks, "qualified": qualified,
            }
            if qualified:
        ...
        surface_global = {
            surface: {"accuracy": accuracy([row for row in behavior if row["surface"] == surface]), "balanced_accuracy": balanced_accuracy([row for row in behavior if row["surface"] == surface])}
            for surface in protocol["material"]["surfaces"]
        }
        breadth = {
            "minimum_family_breadth": len(qualified_families) >= protocol["material"]["minimum_qualified_families"],
            "both_surfaces": all(value["balanced_accuracy"] >= protocol["zero_model_gate"]["required_model_balanced_accuracy_min"] for value in surface_global.values()),
        ...
        summary = {
            "phase": PHASE, "campaign": CAMPAIGN, "global_accuracy": accuracy(behavior),
            "global_balanced_accuracy": balanced_accuracy(behavior), "surface_global": surface_global,
            "family_results": family_results, "qualified_families": qualified_families,
            "selected_count": len(selected), "selected_partition_counts": {name: sum(row["partition"] == name for row in selected) for name in protocol["material"]["partitions"]},
            "breadth_checks": breadth, "behavior_qualified": behavior_qualified,
            "numeric_same_shape_max_abs_diff": numeric_repeat, "hidden_state_accessed": False,
        ...
        authorization = "run_phase1432_c071_role_map_camera" if behavior_qualified else "close_c071_at_behavior_gate"
        core.save(OUT / "analysis/final.json", {
            "phase": PHASE, "campaign": CAMPAIGN, "behavior_qualified": behavior_qualified,
            "qualified_families": qualified_families, "authorization": authorization,
        })
