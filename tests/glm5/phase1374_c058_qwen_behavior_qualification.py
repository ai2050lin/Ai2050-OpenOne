#!/usr/bin/env python3
"""Phase1374: behavior qualification for the independent C058 material."""
...
PHASE, CAMPAIGN = 1374, "C058"
CONTRACT = TESTS / "result/phase1373_c058_dose_distance_group_campaign_contract"
...
def parent() -> dict:
    ...
    if final.get("authorization") != "run_phase1374_c058_behavior_qualification" or not audit.get("all_checks_passed"):
        raise RuntimeError("Phase1373 did not authorize behavior")
...
def main() -> None:
    ...
    if (OUT / "analysis/final.json").exists():
        raise RuntimeError("Phase1374 already exists")
    ...
    try:
        ...
        numeric_max = max(abs(a["scores"][j] - b["scores"][j]) for a, b in zip(first, repeat) for j in range(2))
        ...
        active, active_rows = summarize(active_source, active_pred, ["partition", "surface", "target_family", "truth"])
        ...
        active_by, status_by = {r["case_id"]: r for r in active_rows}, {r["case_id"]: r for r in status_rows}
        eligible = [p for p in pairs if all(active_by[p[key]]["correct"]
        ...
        checks = {
            ...
            "partition_selected": all(sum(r["partition"] == p for r in selected) == 96
        ...
        core.save(OUT / "analysis/final.json", {
            "phase": PHASE, "campaign": CAMPAIGN, "behavior_qualified": summary["behavior_qualified"],
            "authorization": "run_phase1375_c058_instrument_calibration" if summary["behavior_qualified"]
