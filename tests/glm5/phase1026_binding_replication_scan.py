#!/usr/bin/env python3
"""Run the enlarged frozen-depth Phase1026 replication in FP16."""
...
def condition_metrics(
    ...
    for condition_index, condition in enumerate(protocol.CONDITIONS):
        accuracies = []
        margins = []
        for left_surface in range(protocol.SURFACE_COUNT):
            for right_surface in range(protocol.SURFACE_COUNT):
    ...
    result["bound_minus_controls"] = {
        condition: (
            bound
            - result[condition]["target_cross_surface_top1"]
        )
...
def alignment_metrics(
    ...
    for condition_index, condition in enumerate(protocol.CONDITIONS):
        ...
        accuracies = []
        margins = []
        for surface in range(protocol.SURFACE_COUNT):
            accuracy, margin = top1(
...
def main() -> None:
    ...
    frozen = sorted({
        0,
        int(prereg["primary_depth_frozen_from_phase1025"][args.model]),
        *map(
            int,
            prereg["auxiliary_depths_frozen_from_phase1025"][args.model],
        ),
    })
    started = time.time()
    ...
    try:
        ...
        for role_index, role in enumerate(ROLES):
            for depth_index, depth in enumerate(frozen):
                ...
                row = {
                    "schema_version": "phase1026_metric.v1",
                    "model": args.model,
                    "role": role,
                    "depth": depth,
        ...
        summary = {
            "schema_version": "phase1026_model_summary.v1",
            ...
            "primary_depth": int(
                prereg["primary_depth_frozen_from_phase1025"][args.model]
            ),
            "selection_source": "phase1025_only",
