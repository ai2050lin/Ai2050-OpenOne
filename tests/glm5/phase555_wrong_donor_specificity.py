#!/usr/bin/env python3
"""
Phase 555: wrong-donor specificity audit.

Phase554 showed that GLM4 vehicle_tool remove drops can be restored by
cross-object donor activations.  This phase asks the stricter question: does a
vehicle donor restore more than tool/furniture/clothing donors, or is the
restore just a nonspecific dynamic-state overwrite?
"""
...
OUT_ROOT = Path("results/glm5_phase555_wrong_donor_specificity")
...
DEFAULT_CONDITIONS = [
    ...
    "resid_donor_vehicle_base",
    "resid_donor_vehicle_add",
    "resid_donor_tool_base",
    "resid_donor_tool_add",
    "resid_donor_furniture_base",
    "resid_donor_furniture_add",
    "resid_donor_clothing_base",
    "resid_donor_clothing_add",
]
...
def condition_plan(condition: str) -> dict[str, Any]:
    ...
    if condition.startswith(prefix):
        ...
        donor_category, donor_state = tail.rsplit("_", 1)
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if len(windows) != 1:
            raise ValueError(f"Phase555 expects one window, got {windows}")
        ...
        log(f"{args.model}: phase555 pair={pair}, combos={combos}, routes={[r['name'] for r in routes]}")
            ...
            for route in routes:
                ...
                for condition in conditions:
                    ...
                    donor_rows = donor_rows_for(
                        pair, donor_scaffold, plan.get("donor_category"), args.test_n
                    )
                    all_records = []
                    seed_rows = []
                ...
                log(
                    ...
                    f"cloth={rows.get('resid_donor_clothing_add', base)['clean_non_object_rate']:.2f}"
                )
        ...
        return {
            ...
            "restore_note": "donor-category restore writes vehicle/tool/furniture/clothing donor activation at current last-token residual site after projection removal",
...
def write_tsv(result: dict[str, Any], out_dir: Path, model_name: str) -> None:
    fields = [
        ...
        "condition", "donor_category", "donor_state", "seed", "object", "donor_object", "quality",
    ...
    path = out_dir / f"phase555_{model_name}_readable_samples.tsv"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_path = out_dir / f"phase555_{args.model}_wrong_donor_specificity.json"
