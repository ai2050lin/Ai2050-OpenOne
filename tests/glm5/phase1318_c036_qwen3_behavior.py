#!/usr/bin/env python3
"""Phase1318: behavior-only qualification for frozen C036 forward lookup."""
...
def preregister(force: bool) -> None:
    if load(PARENT / "analysis/final.json").get("authorization") != "phase1318_qwen3_behavior_only":
        raise RuntimeError("Phase1317 did not authorize behavior")
    if not load(PARENT / "audit/independent_final_audit.json").get("all_checks_passed"):
        raise RuntimeError("Phase1317 audit failed")
    ...
    timeless = {
        "phase": PHASE, "campaign": CAMPAIGN, "schema_version": "phase1318.c036.behavior.v1",
        "model": "qwen3-4b-fp16-cuda-no-quantization", "formal_run_budget": 1,
        "runtime": {"compiler": "exact_length_batches", "candidate_batch": SCORE_BATCH, "generation_batch": GEN_BATCH},
        ...
        "candidate_scoring": "three frozen one-token value continuations at the assistant answer boundary",
        "generation": "active confirmation/holdout only; greedy, maximum eight tokens, exactly one candidate-value hit",
        "hidden_states_read": False,
        "success_authorization": "phase1319_embedding_field_only", "failure_authorization": "close_c036_without_hidden",
        "hard_stops": ["No hidden states", "No parser, material, split, model, or threshold change", "No second formal model run"],
...
def run() -> None:
    ...
    if pre.get("authorization") != "run_phase1318_once" or not pre.get("all_checks_passed"):
            ...
            for start in range(0, len(buckets[length]), SCORE_BATCH):
                ...
                for i, state in enumerate(batch):
                    ...
                    candidate_out.append({
                        "case_id": state["case_id"], "pair_key": pair["pair_key"], "partition": pair["partition"],
                        "profile_index": pair["profile_index"], "attribute": pair["attribute"], "surface": pair["surface"],
                        "panel": pair["panel"], "binding_state": state["binding_state"],
        ...
        for length in sorted(gen_buckets):
            for start in range(0, len(gen_buckets[length]), GEN_BATCH):
                ...
                for item, text in zip(batch, tokenizer.batch_decode(generated, skip_special_tokens=True)):
                    hits = [value for value in item["candidates"] if re.search(rf"\b{re.escape(value)}\b", text, re.I)]
                    ...
                    generation_out.append({**{k: item[k] for k in ("case_id", "pair_key", "partition", "profile_index",
                                                                    "attribute", "surface", "binding_state", "gold_value")},
                                           "generation": text, "candidate_hits": hits, "covered": len(hits) == 1,
                                           "prediction": prediction, "label_correct": prediction == item["gold_value"]})
        candidate_out.sort(key=lambda x: x["case_id"])
        generation_out.sort(key=lambda x: x["case_id"])
        write_rows(RAW, candidate_out); write_rows(GENERATIONS, generation_out)
        analysis = summarize(candidate_out, generation_out, protocol["thresholds"])
        authorization = "phase1319_embedding_field_only" if analysis["all_gates_passed"] else "close_c036_without_hidden"
...
if __name__ == "__main__":
    ...
    parser.add_argument("command", choices=("preregister", "run")); parser.add_argument("--force", action="store_true")
