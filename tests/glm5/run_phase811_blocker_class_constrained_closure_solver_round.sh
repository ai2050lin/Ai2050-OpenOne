...
for MODEL in qwen3 glm4 deepseek7b; do
  ...
  echo "[$(date +%H:%M:%S)] phase811 $ROUND: start $MODEL cases=$MODEL_MAX_CASES combo_size=$MAX_COMBO_SIZE pool=$MAX_COMBO_CANDIDATES combos=$MAX_COMBOS"
  python tests/glm5/phase811_blocker_class_constrained_closure_solver.py \
    --model "$MODEL" \
    "${COMMON_ARGS[@]}"
  echo "[$(date +%H:%M:%S)] phase811 $ROUND: done $MODEL"
done
