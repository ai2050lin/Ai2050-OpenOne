#!/usr/bin/env python3
"""Run Phase1101 behavior gates for one local FP16 model."""
...
def generation_selection(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for row in rows:
        if row["route_type"] != "semantic" or row["congruence"] != "conflict":
            continue
        key = (str(row["relation_pair"]), str(row["surface"]), str(row["split"]))
    ...
    try:
        ...
        with torch.inference_mode():
            for length in sorted(by_length):
                ...
                for start in range(0, len(panel), CANDIDATE_BATCH_SIZE[model_name]):
                    batch = panel[start:start + CANDIDATE_BATCH_SIZE[model_name]]
        ...
        with torch.inference_mode():
            for length in sorted(by_generation_length):
                ...
                for start in range(0, len(panel), GENERATION_BATCH_SIZE[model_name]):
                    batch = panel[start:start + GENERATION_BATCH_SIZE[model_name]]
                    input_ids = torch.tensor(
                        [row["input_ids"] for row in batch],
                        dtype=torch.long,
                        device=device,
        ...
        for key in sorted(generation_totals):
            ...
            per_generation_cell["|".join(key)] = {
                ...
                "target_before_distractor_accuracy": generation_hits[key] / total,
                "semantic_first_rate": generation_first[key] / total,
            }
        elapsed = time.time() - started
        summary = {
            "schema_version": "phase1101_behavior_summary.v1",
            "phase": protocol.PHASE,
            "model": model_name,
            ...
            "candidate_finite_fraction": sum(finite_counts.values()) / max(sum(totals.values()), 1),
            "candidate_accuracy": sum(hits.values()) / max(sum(finite_counts.values()), 1),
            "generation_count": sum(generation_totals.values()),
            "generation_target_before_distractor_accuracy": (
                sum(generation_hits.values()) / max(sum(generation_totals.values()), 1)
            ),
            "generation_semantic_first_rate": (
                sum(generation_first.values()) / max(sum(generation_totals.values()), 1)
            ),
            "per_cell": per_cell,
        ...
        protocol.write_jsonl(output_root / "generation_detail.jsonl", generation_detail)
