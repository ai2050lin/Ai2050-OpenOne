#!/usr/bin/env python3
"""Independent pre/post audit for Phase1321; never imports the executor."""
...
MAIN = T / "phase1321_c037_qwen3_behavior.py"
...
def base(protocol: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    parent_ok = load(PARENT / "analysis/final.json").get("authorization") == "phase1321_qwen3_behavior_only"
    parent_ok &= load(PARENT / "audit/independent_final_audit.json").get("all_checks_passed", False)
    add(checks, "parent_authorization", parent_ok, "Phase1320")
    ...
    add(checks, "generation_scope", Counter(x["partition"] for x in generation) == Counter({"confirmation": 96, "holdout": 96})
        and all(x["true_boundary"] == len(x["ids"]) - 1 for x in generation), dict(Counter(x["partition"] for x in generation)))
    add(checks, "behavior_only", protocol["hidden_states_read"] is False and protocol["formal_run_budget"] == 1
        and protocol["success_authorization"] == "phase1322_isomorphic_field_only", protocol["hard_stops"])
    return checks
...
def preaudit() -> None:
    ...
    write(PRE, checks, "pre_model", "run_phase1321_once")
...
def postaudit() -> None:
    ...
    add(checks, "raw_counts", len(candidate) == 1152 and len(generation) == 192,
        {"candidate": len(candidate), "generation": len(generation)})
    add(checks, "raw_balance", Counter(x["partition"] for x in candidate) == Counter({key: 384 for key in PARTITIONS})
    ...
    authorization = "phase1322_isomorphic_field_only" if result["all_gates_passed"] else "close_c037_without_hidden"
    final = load(F)
