#!/usr/bin/env python3
"""Freeze Phase1082 operation-by-lexical-world dual-Latin protocol.

The protocol crosses eight language operations with four disjoint lexical
worlds.  Every task returns one of eight shared, single-token code words in a
balanced schedule.  Internal measurements are forbidden until this file has
generated and audited every model-specific case plus the preregistration.
"""
...
CODE_WORDS = (
    ...
    "omega", "sigma", "theta", "amber",
)
CODE_PAIRS = tuple(
    (CODE_WORDS[index], CODE_WORDS[index + 1])
    for index in range(0, len(CODE_WORDS), 2)
)
STATES = tuple(
    f"t{template}_c{panel}_m{mapping}_q{query}_w{code_swap}"
    for template in (0, 1)
    for panel in PANELS
    for mapping in (0, 1)
    for query in (0, 1)
    for code_swap in (0, 1)
)
CAPTURE_ROLES = (
    "codebook_end",
    "record0_end",
...
GENERATION_UNITS_PER_FAMILY_SPLIT = 3
GENERATION_STEPS = 12
ASSISTANT_PREFILL = "Completion:"
OUT_ROOT = (
    ROOT / "tests" / "glm5" / "result"
    / "phase1082_dual_latin_operation_world_atlas"
)
SOURCE_PHASE1081 = (
    ROOT / "tests" / "glm5" / "result"
...
PROSPECTIVE_PREDICTIONS = {
    "P1": (
        "All operation-world cells, code-word balance, output-code mapping, "
    ...
    "P5": (
        "Content profiles under the two output-code assignments retrieve at "
    ...
    "P7": (
        "The pooled median maximum code/shell control-to-content ratio is at "
...
SHELLS = {
    0: (
        "Decision codes: {support_code} means supported; "
        "{contradict_code} means contradicted.\n"
        "Case A: {record0}\n"
        "Case B: {record1}\n"
        "Request: Return the decision code for case {selected}.\n"
        "Write only the code."
    ),
    1: (
        "Status key: {support_code} denotes supported; "
        "{contradict_code} denotes contradicted.\n"
        "Item A: {record0}\n"
        "Item B: {record1}\n"
        "Request: Supply the status code for item {selected}.\n"
...
def output_code_pair(
    ...
    return CODE_PAIRS[
        (operation_index + world_index + item_local_index) % len(CODE_PAIRS)
    ]
...
def build_case(
    ...
    template, panel, mapping, query, code_swap = state_factors(state)
    base_pair = output_code_pair(operation, world, item_local_index)
    semantic_codes = (
        base_pair if code_swap == 0 else (base_pair[1], base_pair[0])
    )
    ...
    codebook = (
        f"Decision codes: {semantic_codes[0]} means supported; "
        f"{semantic_codes[1]} means contradicted."
        if template == 0 else
        f"Status key: {semantic_codes[0]} denotes supported; "
        f"{semantic_codes[1]} denotes contradicted."
    )
    line0 = f"{'Case' if template == 0 else 'Item'} A: {record0}"
    line1 = f"{'Case' if template == 0 else 'Item'} B: {record1}"
    request = (
        f"Request: Return the decision code for case {selected}."
        if template == 0 else
        f"Request: Supply the status code for item {selected}."
    )
    raw_spans = {
    ...
    return {
        "schema_version": "phase1082_case.v1",
        "phase": PHASE,
        "model": model_name,
        ...
        "mapping": mapping,
        "query": query,
        "label_swap": code_swap,
        "code_swap": code_swap,
        "label_pair": list(base_pair),
...
def audit_model(
    ...
    code_swap_flips_output_only = True
        ...
        for template in (0, 1):
            for panel in PANELS:
                for mapping in (0, 1):
                    for code_swap in (0, 1):
                        left = lookup[
                            f"t{template}_c{panel}_m{mapping}_q0_w{code_swap}"
                        ]
                        right = lookup[
                            f"t{template}_c{panel}_m{mapping}_q1_w{code_swap}"
                        ]
    ...
    code_counts = Counter(
        (row["operation"], row["split"], code)
        for row in cases
        if row["state"] == "t0_cactive_m0_q0_w0"
        for code in row["label_pair"]
    )
    expected_code_units = (
        len(WORLDS) * ITEMS_PER_CELL_SPLIT * 2 // len(CODE_WORDS)
    )
    codes_balanced = all(
        code_counts[(operation, split, code)] == expected_code_units
        for operation in OPERATIONS
        for split in SPLITS
    ...
    checks = {
        ...
        "codes_balanced_per_operation_split": codes_balanced,
        ...
        "code_swap_changes_code_not_semantics": code_swap_flips_output_only,
    ...
    return {
        "schema_version": "phase1082_protocol_model_audit.v1",
        "phase": PHASE,
        "model": model_name,
        "case_count": len(cases),
        "unit_count": len(by_unit),
        "code_counts": {
            f"{operation}.{split}.{code}": code_counts[(operation, split, code)]
            for operation in OPERATIONS for split in SPLITS for code in CODE_WORDS
        },
        "checks": checks,
...
def main() -> None:
    ...
    prereg = {
        "schema_version": "phase1082_preregistration.v1",
        "phase": PHASE,
        ...
        "code_pairs": [list(value) for value in CODE_PAIRS],
        "states": list(STATES),
        "factor_definition": {
            ...
            "W": "shared output-code assignment swap",
        },
        ...
        "behavior_gate_definition": (
            ...
            "0.80 and the target code must occur before the paired code in at "
            "least 0.75 of 12-token generations. An operation passes a model "
            "only when at least three worlds pass both gates."
        ),
        "measurement_order": [
            "complete operation-world and code-token audits",
            ...
            "output-code assignment transfer",
        ...
        "evidence_levels": {
            ...
            "L2": "L1 plus held-out lexical-world and code-assignment transfer",
        ...
        "interpretation_limits": [
            "Shared output codes are experimental handles, not latent symbols.",
    ...
    global_audit = {
        "schema_version": "phase1082_protocol_audit.v1",
        "phase": PHASE,
