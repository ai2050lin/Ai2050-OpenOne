#!/usr/bin/env python3
"""Independent audit for Phase1254/C007."""
...
import phase1254_c007_free_transformer_edge_external_validity as main
...
def preaudit() -> dict[str, Any]:
    ...
    checks["component_counts"] = all(
        len(main.component_ids(config)) == config.layers * (2 * config.heads + 1)
        for config in main.ARCHITECTURES.values()
    )
    checks["selection_frozen"] = tuple(protocol["selection"]["candidate_prefix_sizes"]) == main.PREFIX_SIZES
