...
ROUND="${1:-smoke}"
EXTRA_ARGS=("${@:2}")
SCRIPT="tests/glm5/phase799_blocker_field_causal_suppressor_localization.py"
...
case "$ROUND" in
  smoke)
    COMMON_ARGS=(
      ...
      --max-baseline-blocker-classify 20000
      --log-every 1
    )
    ;;
  main)
    COMMON_ARGS=(
      ...
      --max-baseline-blocker-classify 30000
      --log-every 1
    )
    ;;
  confirm)
...
for MODEL in qwen3 glm4 deepseek7b; do
  echo "[$(date +%H:%M:%S)] phase799 ${ROUND}: start ${MODEL}"
  python "$SCRIPT" --model "$MODEL" "${COMMON_ARGS[@]}" "${EXTRA_ARGS[@]}"
  echo "[$(date +%H:%M:%S)] phase799 ${ROUND}: done ${MODEL}"
done
