#!/usr/bin/env python3
"""Post-hoc carrier diagnosis authorized by the frozen Phase1083 stop gate."""
...
def main() -> None:
    ...
    result = {
        "schema_version": "phase1083_posthoc_carrier_diagnostic.v1",
        ...
        "diagnosis": {
            "primary": (
                "The shared dossier removed the pre-query task-format confound, but "
                "attribute names and attribute-specific candidate vocabularies remain "
                "in active and carrier-only panels. The carrier-only route identifies "
                "attributes more strongly across worlds than the content difference."
            ),
            "secondary": (
                "Output-vocabulary and shell responses exceed the content field by "
                "roughly one order of magnitude. Direct normalized-depth/component "
                "alignment does not retrieve attributes across models."
            ),
            "required_next_protocol": (
                "Hold the attribute selector and candidate vocabulary fixed while "
                "changing only which dossier slot is causally relevant, or rotate "
                "field names and value vocabularies across slots with behavior-valid "
                "natural mappings. Require content identity to beat carrier-only "
                "identity before any component or neuron selection."
            ),
        },
        "limits": [
    ...
    protocol.write_json(root / "posthoc_carrier_diagnostic.json", result)
    print({
        "phase": protocol.PHASE,
