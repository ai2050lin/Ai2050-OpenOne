#!/usr/bin/env python3
"""
Phase 628: Prefix/Format Gate and Semantic Value Integration
前缀/格式门与语义值门整合

Phase 627 showed that semantic value patches can repair the first
discriminative token in natural greedy generation, while exact generation can
still fail because the shared prefix / format token is wrong. This phase tests
whether forcing the prefix gate lets semantic patches close exact generation.
"""
        ...
        with torch.inference_mode():
            for step in range(max_new_tokens):
                ...
                if forced_prefix_ids is not None and step < len(forced_prefix_ids):
                    tid = int(forced_prefix_ids[step])
                else:
...
def run_model(args) -> Dict:
    ...
    try:
        ...
        for si, case in enumerate(raw_cases):
            ...
            forced_token0 = correct_ids[:1]
            modes = {
                "base": {"prompt": case["base_prompt"], "layer": [], "final": None},
                "repair_prompt": {"prompt": case["repair_prompt"], "layer": [], "final": None},
                "prefix_forced_only": {"prompt": case["base_prompt"], "layer": [], "final": None, "forced": forced_token0},
                ...
                "prefix_forced_result_only": {
                    ...
                    "layer": result_patches,
                    "final": None,
                    "forced": forced_token0,
                },
                ...
                "prefix_forced_cumulative_layer_out": {
                    "prompt": case["base_prompt"],
                    "layer": cumulative_layer,
                    "final": None,
                    "forced": forced_token0,
                },
                "prefix_forced_cumulative_layer_out_random": {
                    "prompt": case["base_prompt"],
                    "layer": cumulative_layer_random,
                    "final": None,
                    "forced": forced_token0,
                },
                "final_output_all": {
            ...
            for mode, spec in modes.items():
                gen = greedy_generate_ids(
                    ...
                    forced_prefix_ids=spec.get("forced"),
                )
...
def main() -> None:
    ...
    out_path = out_dir / f"phase628_{args.model}_prefix_format_semantic_integration_{suffix}.json"
