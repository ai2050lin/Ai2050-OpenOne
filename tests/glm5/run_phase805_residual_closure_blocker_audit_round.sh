...
for MODEL in qwen3 glm4 deepseek7b; do
  ...
  echo "[$(date +%H:%M:%S)] phase805 $ROUND: start $MODEL cases=$MODEL_MAX_CASES"
  python tests/glm5/phase805_residual_closure_blocker_audit.py \
    --model "$MODEL" \
    "${COMMON_ARGS[@]}"
  echo "[$(date +%H:%M:%S)] phase805 $ROUND: done $MODEL"
done
