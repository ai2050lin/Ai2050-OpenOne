...
for MODEL in qwen3 glm4 deepseek7b; do
  ...
  COMMON_ARGS=(
    ...
    --max-emergence-rate-for-closer 0.15
    --min-local-suppression 0.25
  ...
  echo "[$(date +%H:%M:%S)] phase807 $ROUND: start $MODEL cases=$MODEL_MAX_CASES"
  python tests/glm5/phase807_readout_geometry_new_blocker_emergence_audit.py \
    --model "$MODEL" \
    "${COMMON_ARGS[@]}"
  echo "[$(date +%H:%M:%S)] phase807 $ROUND: done $MODEL"
done
