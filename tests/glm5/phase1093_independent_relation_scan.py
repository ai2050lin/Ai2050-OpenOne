#!/usr/bin/env python3
"""Collect Phase1093 signed relation fields, one FP16 model at a time."""
...
def run(model_name: str) -> None:
    ...
    if not authorization["hidden_scan_authorized"]:
        raise RuntimeError("Phase1093 hidden-state scan was not authorized")
    ...
    result["schema_version"] = "phase1093_model_signed_summary.v1"
    result["behavior_status"] = (
        "authorized"
        if model_name in authorization["authorized_models"]
        else "exploratory"
    )
