...
OUT_ROOT = Path("results/glm5_phase788_matched_source_unit_causal_fiber_validation")
RESULT_ROOT = Path("tests/result/phase788_matched_source_unit_causal_fiber_validation")
...
def median_float(values: list[Any]) -> float:
...
def robust_scale(values: list[Any]) -> float:
    vals = [float(v) for v in values if v is not None and math.isfinite(float(v))]
    if not vals:
        return 1.0
    center = median_float(vals)
    mad = median_float([abs(v - center) for v in vals])
    if mad > 1e-9:
        return mad
...
def add_source_activation_metrics(
    model,
    key: tuple[str, int],
    ...
    layer_idx = key[1]
    out = []
    if source_unit_kind == "attention_head_set":
        layer = get_layers(model)[layer_idx]
        n_heads = infer_num_heads(model, layer.self_attn)
        pre_base = base_state["attn_o_inputs"].get(key)
        pre_donor = donor_state["attn_o_inputs"].get(key)
        if not n_heads or pre_base is None or pre_donor is None:
            return scores
        if int(pre_base.numel()) % n_heads != 0:
            return scores
        head_dim = int(pre_base.numel()) // n_heads
        for row in scores:
            unit_id = int(row["unit_id"])
            start = unit_id * head_dim
            end = start + head_dim
            base_slice = pre_base[start:end].float()
            donor_slice = pre_donor[start:end].float()
            item = dict(row)
            item["source_base_norm"] = float(base_slice.norm().item())
            item["source_donor_norm"] = float(donor_slice.norm().item())
            item["source_delta_norm"] = float((donor_slice - base_slice).norm().item())
            out.append(item)
        return out
    if source_unit_kind == "mlp_channel_set":
        pre_base = base_state["mlp_down_inputs"].get(key)
        pre_donor = donor_state["mlp_down_inputs"].get(key)
        if pre_base is None or pre_donor is None:
            return scores
        for row in scores:
            unit_id = int(row["unit_id"])
            base_val = float(pre_base[unit_id].float().item())
            donor_val = float(pre_donor[unit_id].float().item())
            item = dict(row)
            item["source_base_norm"] = abs(base_val)
            item["source_donor_norm"] = abs(donor_val)
            item["source_delta_norm"] = abs(donor_val - base_val)
            out.append(item)
        return out
    return scores


def select_matched_unit_set(
    scores: list[dict[str, Any]],
    top_units: list[dict[str, Any]],
    size: int,
) -> list[dict[str, Any]]:
    if not scores or not top_units:
        return []
    size = min(int(size), len(scores))
    top_ids = {int(r["unit_id"]) for r in top_units}
    pool = [r for r in scores if int(r["unit_id"]) not in top_ids]
    if len(pool) < size:
        pool = [r for r in scores if int(r["unit_id"]) not in top_ids] or list(scores)
    target_abs = median_float([r.get("source_abs_score") for r in top_units])
    target_delta = median_float([r.get("source_delta_norm") for r in top_units])
    target_base = median_float([r.get("source_base_norm") for r in top_units])
    abs_scale = robust_scale([r.get("source_abs_score") for r in pool])
    delta_scale = robust_scale([r.get("source_delta_norm") for r in pool])
    base_scale = robust_scale([r.get("source_base_norm") for r in pool])
    matched = []
    for row in pool:
        distance = (
            abs(float(row.get("source_abs_score") or 0.0) - target_abs) / abs_scale
            + abs(float(row.get("source_delta_norm") or 0.0) - target_delta) / delta_scale
            + 0.5 * abs(float(row.get("source_base_norm") or 0.0) - target_base) / base_scale
        )
        item = dict(row)
        item["matched_distance"] = float(distance)
        matched.append(item)
    matched.sort(key=lambda r: (r.get("matched_distance") or 0.0, -float(r.get("source_abs_score") or 0.0)))
    return matched[:size]
...
def source_set_meta(
    ...
    return {
        ...
        "source_set_mean_abs_score": safe_mean([u.get("source_abs_score") for u in units]),
        "source_set_mean_delta_norm": safe_mean([u.get("source_delta_norm") for u in units]),
        "source_set_mean_base_norm": safe_mean([u.get("source_base_norm") for u in units]),
        "source_set_mean_donor_norm": safe_mean([u.get("source_donor_norm") for u in units]),
        "source_set_mean_matched_distance": safe_mean([u.get("matched_distance") for u in units]),
        **component_scores,
    }
...
def make_source_row(
    ...
    row.update(
        {
            "row_kind": "phase788_matched_source_unit_causal_fiber_validation",
...
def route_source_candidates(
    ...
    for key, dims in selected.items():
        ...
        if not scores:
            continue
        scores = add_source_activation_metrics(model, key, source_kind, scores, base_state, donor_state)
        top_units = select_unit_set(scores, size, "top", (*seed_parts, key[0], key[1], mode))
        unit_sets = [
            ("top", top_units),
            ("matched", select_matched_unit_set(scores, top_units, size)),
            ("random", select_unit_set(scores, size, "random", (*seed_parts, key[0], key[1], mode))),
        ]
        for selection_kind, units in unit_sets:
            if not units:
                continue
            meta = {
...
def audit_case_route(
    ...
    for mode, budget_label, budget in specs:
        ...
        for meta in candidates:
            ...
            if meta["source_selection_kind"] in {"top", "matched"}:
        ...
        payload.update(
            {
                ...
                "mean_source_set_mean_abs_score": safe_mean([r.get("source_set_mean_abs_score") for r in items]),
                "mean_source_set_mean_delta_norm": safe_mean([r.get("source_set_mean_delta_norm") for r in items]),
                "mean_source_set_mean_base_norm": safe_mean([r.get("source_set_mean_base_norm") for r in items]),
                "mean_source_set_mean_donor_norm": safe_mean([r.get("source_set_mean_donor_norm") for r in items]),
                "mean_source_set_mean_matched_distance": safe_mean([r.get("source_set_mean_matched_distance") for r in items]),
        ...
        payload.update(
            {
                ...
                "random_n": random_row.get("n") if random_row else None,
                "top_strict_gain": top.get("strict_gain_rate_vs_reference"),
                "matched_strict_gain": matched.get("strict_gain_rate_vs_reference"),
                "random_strict_gain": random_row.get("strict_gain_rate_vs_reference") if random_row else None,
                "top_strict_loss": top.get("strict_loss_rate_vs_reference"),
                "matched_strict_loss": matched.get("strict_loss_rate_vs_reference"),
                "top_delta_margin": top.get("mean_delta_margin_vs_reference"),
                "matched_delta_margin": matched.get("mean_delta_margin_vs_reference"),
                "random_delta_margin": random_row.get("mean_delta_margin_vs_reference") if random_row else None,
                "top_minus_matched_delta_margin": (top.get("mean_delta_margin_vs_reference") or 0.0)
                - (matched.get("mean_delta_margin_vs_reference") or 0.0),
                "top_minus_random_delta_margin": (top.get("mean_delta_margin_vs_reference") or 0.0)
                - ((random_row or {}).get("mean_delta_margin_vs_reference") or 0.0),
                "top_minus_matched_strict_gain": (top.get("strict_gain_rate_vs_reference") or 0.0)
                - (matched.get("strict_gain_rate_vs_reference") or 0.0),
                "top_minus_matched_strict_loss": (top.get("strict_loss_rate_vs_reference") or 0.0)
                - (matched.get("strict_loss_rate_vs_reference") or 0.0),
                "top_source_abs": top.get("mean_source_set_abs_score"),
                "matched_source_abs": matched.get("mean_source_set_abs_score"),
                "top_source_delta_norm": top.get("mean_source_set_mean_delta_norm"),
                "matched_source_delta_norm": matched.get("mean_source_set_mean_delta_norm"),
                "matched_distance": matched.get("mean_source_set_mean_matched_distance"),
            }
        )
        payload["matched_specificity_score"] = (
            max(payload["top_minus_matched_delta_margin"], 0.0)
            * (1.0 + max(payload["top_minus_matched_strict_gain"], 0.0))
            if payload.get("intervention_kind") == "patch_baseline_from_donor_source_units"
            else max(-payload["top_minus_matched_delta_margin"], 0.0)
            * (1.0 + max(payload["top_minus_matched_strict_loss"], 0.0))
        )
        out.append(payload)
    out.sort(key=lambda r: (r.get("matched_specificity_score") or 0.0, abs(r.get("top_minus_matched_delta_margin") or 0.0)), reverse=True)
    return out
...
def summarize_rows(rows: list[dict[str, Any]], args: argparse.Namespace, attn_impl: str, routes: list[dict[str, Any]], specs: list[tuple[str, str, int | None]]) -> dict[str, Any]:
    ...
    matched = matched_comparisons(by_intervention)
    return {
        "phase": 788,
        "title": "Matched Source Unit Causal Fiber Validation",
        "model": args.model,
        ...
        "method_note": (
            "Patch top, matched, or random source units from donor into baseline at answer-site source tensors. "
        ...
        "top_matched_specificity": matched[:40],
        ...
        "strict_interpretation": (
            "This phase upgrades random controls to matched source-unit controls. "
            "It validates answer-site source-unit specificity only; it still does not prove full Q/K/V or cross-position semantic fibers."
        ),
    }
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        for ci, source_row in enumerate(selected, 1):
            ...
            if ci % args.log_every == 0 or ci == len(selected):
                log(f"{args.model}: matched source-unit causal fiber validation {ci}/{len(selected)} cases; rows={len(rows)}")
    finally:
        release_model(model)
        del tokenizer
        gc.collect()
    ...
    for root in (out_dir, result_dir):
        write_jsonl(root / f"phase788_{args.model}_rows.jsonl", rows)
        write_json(root / f"phase788_{args.model}_summary.json", summary)
    print(
        json.dumps(
            {
                "model": args.model,
...
def write_markdown(path: Path, payload: dict[str, Any]) -> None:
    lines = [
        f"# Phase 788 Matched Source Unit Causal Fiber Validation ({payload['round']})",
        "",
    ...
    for model_name in MODELS:
        ...
        if not data:
            continue
        for row in (data.get("top_matched_specificity") or [])[:20]:
            lines.append(
                f"| {model_name} | `{row['source_unit_kind']}` | `{row['subspace_mode']}` | `{row['intervention_kind']}` | "
                f"{row['source_set_size']} | {fmt(row['top_strict_gain'])} | {fmt(row['matched_strict_gain'])} | "
                f"{fmt(row['top_delta_margin'])} | {fmt(row['matched_delta_margin'])} | "
                f"{fmt(row['top_minus_matched_delta_margin'])} | {fmt(row['matched_distance'])} |"
            )
    lines += [
        "",
...
def build_atlas_graph(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    add_node(
        {
            "id": "phase788:task:matched_source_unit_causal_fiber",
            "type": "task",
            "model": "cross_model",
            "label": "Phase 788 matched source-unit causal fiber",
            "role": "matched_control_validation",
            "evidence_level": "causal_intervention_with_matched_control",
            "phase": 788,
            "position": [0, 0, -1],
        }
    )
    for model_name in MODELS:
        ...
        add_node({"id": model_id, "type": "model", "model": model_name, "label": model_name, "phase": 788, "position": [-4, 0, lane]})
        edges.append(
            {
                "source": model_id,
                "target": "phase788:task:matched_source_unit_causal_fiber",
                "relation": "tested_by",
                "phase": 788,
                "evidence": "matched_control_intervention",
                "weight": 1.0,
            }
        )
        for row in (data.get("top_matched_specificity") or [])[:24]:
            kind = row.get("source_unit_kind")
            mode = row.get("subspace_mode")
            intervention = row.get("intervention_kind")
            set_size = row.get("source_set_size")
            node_id = f"{model_name}:source_set:{kind}:{mode}:{intervention}:k{set_size}"
            y = 30 if kind == "attention_head_set" else 36
            x = (set_size or 0) / 8.0
            add_node(
                {
                    "id": node_id,
                    "type": "cluster",
                    "model": model_name,
                    "label": f"{kind}:{mode}:top",
                    "role": "matched_source_unit_set",
                    "evidence_level": "causal_intervention_with_matched_control",
                    "phase": 788,
                    "source_unit_kind": kind,
                    "subspace_mode": mode,
                    "intervention_kind": intervention,
                    "source_set_size": set_size,
                    "score": row.get("matched_specificity_score"),
                    "top_minus_matched_delta_margin": row.get("top_minus_matched_delta_margin"),
                    "top_minus_matched_strict_gain": row.get("top_minus_matched_strict_gain"),
                    "matched_distance": row.get("matched_distance"),
                    "position": [x, y, lane],
                }
            )
            readout_id = f"{model_name}:readout:token0_margin"
            add_node(
                {
                    "id": readout_id,
                    "type": "intervention",
            ...
            edges.append(
                {
                    ...
                    if (row.get("top_minus_matched_delta_margin") or 0.0) >= 0
                    else "negative_effect",
                    "weight": abs(row.get("top_minus_matched_delta_margin") or 0.0),
                    "phase": 788,
                    "evidence": "top_vs_matched_intervention",
                    "top_delta_margin": row.get("top_delta_margin"),
                    "matched_delta_margin": row.get("matched_delta_margin"),
                    "top_minus_matched_delta_margin": row.get("top_minus_matched_delta_margin"),
                }
            )
    graph = {
        "schema_version": "atlas_graph_v1",
        "title": f"Phase 788 Matched Source Unit Causal Fiber Validation ({payload['round']})",
        "model_info": {
            "model": "cross_model",
            "models": payload.get("models", []),
            "phase": 788,
            "timestamp": payload.get("timestamp"),
            "evidence_type": "answer-site source-unit patch with matched controls",
        },
        "layout": {
            "x": "source set size / component offset",
            "y": "approximate layer band by source kind",
            "z": "model lane",
        },
        ...
        "source_files": [
            f"results/glm5_phase788_matched_source_unit_causal_fiber_validation/{payload['round']}/phase788_cross_model_summary.json"
        ],
    }
    return graph
...
def write_cross_summary(round_name: str) -> dict[str, Any]:
    by_model = {}
    for model_name in MODELS:
        path = OUT_ROOT / round_name / f"phase788_{model_name}_summary.json"
        if path.exists():
    ...
    for root in (OUT_ROOT, RESULT_ROOT):
        ...
        write_json(out_dir / "phase788_cross_model_summary.json", payload)
        write_markdown(out_dir / "phase788_cross_model_summary.md", payload)
        write_json(out_dir / "phase788_atlas_graph.json", build_atlas_graph(payload))
