#!/usr/bin/env python3
"""
Phase 553: scaffold route split and activation restore for the clean
paraphrase gate.

Phase552 showed that GLM4 vehicle_tool has native residual necessity evidence.
This phase asks whether the drop caused by removing the residual_perp component
can be restored by cached baseline/add activations at residual, attention, or
MLP sites, and whether natural_qa uses a different route from
...
OUT_ROOT = Path("results/glm5_phase553_scaffold_route_restore")
...
def condition_plan(condition: str) -> dict[str, Any]:
    if condition == "baseline":
        return {"site": "none", "component": None, "remove": False, "add": False, "restore": None}
    if condition == "add_perp":
        return {"site": "resid", "component": "residual_perp", "remove": False, "add": True, "restore": None}
    if condition == "resid_remove_perp":
        return {"site": "resid", "component": "residual_perp", "remove": True, "add": False, "restore": None}
    if condition == "resid_restore_base":
        return {"site": "resid", "component": "residual_perp", "remove": True, "add": False, "restore": "baseline"}
    if condition == "resid_restore_add":
        return {"site": "resid", "component": "residual_perp", "remove": True, "add": False, "restore": "add_perp"}
    if condition == "resid_remove_full":
        return {"site": "resid", "component": "residual_full", "remove": True, "add": False, "restore": None}
    if condition == "resid_remove_random_perp":
        return {"site": "resid", "component": "random_perp", "remove": True, "add": False, "restore": None}
    if condition == "attn_remove_perp":
        return {"site": "attn", "component": "residual_perp", "remove": True, "add": False, "restore": None}
    if condition == "attn_restore_base":
        return {"site": "attn", "component": "residual_perp", "remove": True, "add": False, "restore": "baseline"}
    if condition == "attn_restore_add":
        return {"site": "attn", "component": "residual_perp", "remove": True, "add": False, "restore": "add_perp"}
    if condition == "mlp_remove_perp":
        return {"site": "mlp", "component": "residual_perp", "remove": True, "add": False, "restore": None}
    if condition == "mlp_restore_base":
        return {"site": "mlp", "component": "residual_perp", "remove": True, "add": False, "restore": "baseline"}
    if condition == "mlp_restore_add":
        return {"site": "mlp", "component": "residual_perp", "remove": True, "add": False, "restore": "add_perp"}
    if condition == "attn_remove_full":
        return {"site": "attn", "component": "residual_full", "remove": True, "add": False, "restore": None}
    if condition == "mlp_remove_full":
        return {"site": "mlp", "component": "residual_full", "remove": True, "add": False, "restore": None}
    ...
    for start in range(0, len(texts), batch_size):
        ...
        if plan["site"] != "none":
            if plan["restore"] is not None:
                restore_cache = collect_restore_cache(
                    model, layers, batch, pos, components_by_layer, layer_ids,
...
def compact_metrics(
    ...
    if condition.endswith("_restore_base") or condition.endswith("_restore_add"):
...
def run_model(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        if len(windows) != 1:
            raise ValueError(f"Phase553 expects one window, got {windows}")
        ...
        log(f"{args.model}: phase553 pair={pair}, combos={combos}, scaffold_modes={scaffold_modes}")
        ...
        for combo_name, layer_ids in combos.items():
            ...
            for scaffold, mode in scaffold_modes:
                ...
                rr = rows.get("resid_restore_base", base)
                ar = rows.get("attn_restore_base", base)
                mr = rows.get("mlp_restore_base", base)
                log(
                    ...
                    f"restore={rr['clean_non_object_rate']:.2f}/{ar['clean_non_object_rate']:.2f}/{mr['clean_non_object_rate']:.2f}"
                )
        ...
        return {
            ...
            "restore_note": "restore_base/add writes cached donor activation at current last-token site after projection removal",
...
def write_tsv(result: dict[str, Any], out_dir: Path, model_name: str) -> None:
    ...
    path = out_dir / f"phase553_{model_name}_readable_samples.tsv"
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    out_path = out_dir / f"phase553_{args.model}_scaffold_route_restore.json"
