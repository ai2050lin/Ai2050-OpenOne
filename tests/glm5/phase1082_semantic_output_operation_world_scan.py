#!/usr/bin/env python3
"""Run one frozen Phase1082 revision-2 operation-world atlas in FP16."""
...
# frozen Phase1082 protocol rather than the source Phase1079 module.
...
def generation_selection(rows: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    for family in protocol.FAMILIES:
        for split in protocol.SPLITS:
            ...
            for unit_id, state in zip(units, balanced_states):
                row = next(
                    row for row in rows
...
def run(model_name: str) -> None:
    ...
    if not audit["all_checks_passed"]:
        raise RuntimeError("Phase1082 revision-2 protocol audit failed")
    ...
    try:
                ...
                for unit, offset in zip(batch_units, offsets):
                    ...
                    for local, state in enumerate(state_order):
                        ...
                        behavior_records.append({
                            "schema_version": "phase1082_candidate_behavior.v2",
        ...
        for row in generation_rows:
            ...
            generation_records.append({
                "schema_version": "phase1082_natural_generation.v2",
                "phase": protocol.PHASE,
                "model": model_name,
                ...
                for split_name, split in split_index.items():
                    for event_index, event in enumerate(events):
                        for role_name, role in role_index.items():
                            ...
                            record: dict[str, Any] = {
                                "schema_version": "phase1082_response_metric.v2",
                                "phase": protocol.PHASE,
                                "model": model_name,
        ...
        for conditioning_name, conditioning in conditioning_index.items():
            for family_name, family in family_index.items():
                for event_index, event in enumerate(events):
                    for role_name, role in role_index.items():
                        record = {
                            "schema_version": "phase1082_split_direction_repeat.v2",
                            "phase": protocol.PHASE,
                            "model": model_name,
                            "conditioning": conditioning_name,
                            "family": family_name,
        ...
        summary = {
            "schema_version": "phase1082_model_summary.v2",
            "phase": protocol.PHASE,
            "model": model_name,
