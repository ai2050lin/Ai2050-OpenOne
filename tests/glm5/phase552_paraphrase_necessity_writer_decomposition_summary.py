#!/usr/bin/env python3
"""Summary for Phase552 paraphrase necessity and writer decomposition."""
...
DEFAULT_ROOT = Path("results/glm5_phase552_paraphrase_necessity_writer_decomposition")
MODELS = ["qwen3", "glm4", "deepseek7b"]


def load(root: Path, model: str) -> dict[str, Any] | None:
    path = root / f"phase552_{model}_paraphrase_necessity_writer_decomposition.json"
    if not path.exists():
        return None
...
def fmt(model: str, row: dict[str, Any]) -> str:
    return (
        ...
        f"{row['drop_vs_random']:+.2f} | {row['class']} |"
    )
...
def main() -> None:
    ...
    lines = ["# Phase552 Paraphrase Necessity and Writer Decomposition Summary", ""]
    all_rows = []
    for model, d in data.items():
        ...
        lines.append("")
        lines.append("| model | combo | layers | scaffold | mode | condition | base clean-no | clean-no | clean delta | base label | label | label delta | object echo | prompt echo | score delta | random delta | drop-vs-random | class |")
        lines.append("|---|---|---|---|---|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|")
        for row in sorted(rows, key=lambda r: (r["class"] == "necessity_drop", -r["clean_delta"], -r["score_delta"]), reverse=True):
            lines.append(fmt(model, row))
        lines.append("")

        lines.append("### Best drops by condition")
        lines.append("")
        lines.append("| model | condition | best row | clean delta | score delta | random delta | class |")
        lines.append("|---|---|---|---:|---:|---:|---|")
        for cond in d["conditions"]:
            ...
            lines.append(f"| {model} | {cond} | {desc} | {best['clean_delta']:+.2f} | {best['score_delta']:+.2f} | {best['random_delta']:+.2f} | {best['class']} |")
        lines.append("")
    ...
    out = root / "phase552_cross_model_summary.md"
    out.write_text("\n".join(lines) + "\n", encoding="utf-8")
    print(f"Wrote {out}")
