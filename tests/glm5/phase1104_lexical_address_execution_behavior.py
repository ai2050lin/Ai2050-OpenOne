#!/usr/bin/env python3
"""Run Phase1104 candidate-logit behavior for one local FP16 model."""
...
BATCH_SIZE = {"qwen3": 16, "glm4": 16, "deepseek7b": 16}
    ...
    try:
            ...
            for length in sorted(by_length):
                ...
                for start in range(0, len(panel), BATCH_SIZE[model_name]):
                    ...
                    for slot, row in enumerate(batch):
                        ...
                        finite = all(math.isfinite(v) for v in scores.values()) and math.isfinite(margin)
                        hit = finite and margin > 0.0
                        key = (
                            str(row["relation_pair"]), str(row["surface"]),
                            str(row["split"]), str(row["label_regime"]),
        ...
        for key in sorted(totals):
            ...
            per_cell["|".join(key)] = {
                ...
                "median_expected_margin": median_margin,
            }
        elapsed = time.time() - started
        total = sum(totals.values())
        finite_total = sum(finite_counts.values())
        summary = {
            "schema_version": "phase1104_behavior_summary.v1",
            "phase": protocol.PHASE,
            "model": model_name,
            ...
            "generation_is_not_an_authorization_gate": True,
        }
