...
const output = path.resolve('tests/gpt5/result/phase393_attribute_content_holdout/screenshots');
...
async function inspectViewport(browser, name, viewport) {
  ...
  const result = await page.evaluate(async () => {
    ...
    const atlas = await fetch('/vis_data/pattern_family_atlas/v2/phase393_joint_formation_stage_summary.json', { cache: 'no-store' }).then((response) => response.json());
    ...
    return {
      ...
      footerHasAttributeTransport: latestText.includes('属性搬运 71/72'),
      footerHasStrictBoundary: latestText.includes('错误深度 72/72') && latestText.includes('完整路径 0') && latestText.includes('深度特异 0/3') && latestText.includes('神经元路径 0'),
      stageMatches: atlas?.results?.phase393_attribute_answer_switch_count === 71 && atlas?.results?.complete_language_path_count === 0,
      neuronBoundaryMatches: neuron?.phase === 393 && neuron?.phase393_audit?.new_neuron_path_nodes_promoted === 0,
    };
  });
...
(async () => {
  ...
  try {
    const results = [];
    results.push(await inspectViewport(browser, 'phase393_desktop_1440x900', { width: 1440, height: 900 }));
    results.push(await inspectViewport(browser, 'phase393_mobile_390x844', { width: 390, height: 844 }));
    const valid = results.every((row) => (
      ...
      row.footerHasAttributeTransport && row.footerHasStrictBoundary &&
      row.stageMatches && row.neuronBoundaryMatches && row.errors.length === 0 &&
      row.overflowingLabels.length === 0 && row.canvasPixelStats.nonblank
    ));
    const payload = { phase_id: 'Phase393', created_at: new Date().toISOString(), valid, results };
