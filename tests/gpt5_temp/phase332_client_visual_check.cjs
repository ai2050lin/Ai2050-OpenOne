...
const output = path.resolve('tests/gpt5/result/phase332_interface_branch_atlas/interface_branch_atlas/screenshots');
...
async function inspectViewport(browser, name, viewport) {
  ...
  await controls.getByRole('button', { name: '接口脉络' }).click();
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      footerText,
      phase332MetricsVisible: footerText.includes('接口路径成员') && footerText.includes('稳定共享成员') && footerText.includes('接口分支成员'),
      interfacePathFocusActive: Array.from(controlsNode?.querySelectorAll('button') || []).some((node) => (
        node.textContent?.trim() === '接口脉络' && node.classList.contains('is-active')
      )),
    };
  });
...
(async () => {
  ...
  try {
    const results = [];
    results.push(await inspectViewport(browser, 'phase332_desktop_1440x900', { width: 1440, height: 900 }));
    results.push(await inspectViewport(browser, 'phase332_mobile_390x844', { width: 390, height: 844 }));
    const valid = results.every((row) => (
      ...
      row.phase332MetricsVisible && row.interfacePathFocusActive && row.errors.length === 0 &&
      row.overflowingLabels.length === 0
    ));
    const payload = { phase_id: 'Phase332', created_at: new Date().toISOString(), valid, results };
