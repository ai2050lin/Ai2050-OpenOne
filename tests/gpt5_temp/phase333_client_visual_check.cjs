...
const output = path.resolve('tests/gpt5/result/phase333_dynamic_path_atlas/dynamic_path_atlas/screenshots');
...
async function inspectViewport(browser, name, viewport) {
  ...
  await controls.getByRole('button', { name: '动态时序' }).click();
  ...
  const result = await page.evaluate(() => {
    ...
    return {
      ...
      footerText,
      phase333MetricsVisible: footerText.includes('动态事件锚点') && footerText.includes('补偿候选') && footerText.includes('动态路径完整门'),
      dynamicPathFocusActive: Array.from(controlsNode?.querySelectorAll('button') || []).some((node) => (
        node.textContent?.trim() === '动态时序' && node.classList.contains('is-active')
      )),
    };
  });
...
(async () => {
  ...
  try {
    const results = [];
    results.push(await inspectViewport(browser, 'phase333_desktop_1440x900', { width: 1440, height: 900 }));
    results.push(await inspectViewport(browser, 'phase333_mobile_390x844', { width: 390, height: 844 }));
    const valid = results.every((row) => (
      ...
      row.phase333MetricsVisible && row.dynamicPathFocusActive && row.errors.length === 0 &&
      row.overflowingLabels.length === 0
    ));
    const payload = { phase_id: 'Phase333', created_at: new Date().toISOString(), valid, results };
