...
const OUT = path.join(ROOT, 'tests/gpt5/result/phase406_conditioned_sequence_state/screenshots');
...
async function inspectViewport(browser, name, viewport) {
  ...
  const canvasPath = path.join(OUT, `phase406_atlas_${name}_canvas.png`);
  ...
  const latestValue = page.getByText('3,512/5,760 sequences / 0/3 cross-model', { exact: true });
  ...
  const requiredTexts = [
    'Phase406-ConditionedSequenceStage：条件化短序列响应表与接口迁移审计',
    'Phase406 正式发现案例：5760/5760。',
    'H12 短序列语义正确案例：3512/5760。',
    '正式组通过数：5/72。',
    '跨模型条件状态族：0/3。',
    '新增单神经元因果路径：0/72。',
  ];
  ...
  const screenshot = path.join(OUT, `phase406_atlas_${name}.png`);
...
async function main() {
  ...
  try {
    ...
    const result = {
      phase_id: 'Phase406-ClientVisualCheck',
      url: URL,
      checks,
      pass: checks.every((item) => item.allAssertionsPass && !item.horizontalDocumentOverflow),
    };
    const output = path.join(OUT, 'phase406_client_visual_check.json');
