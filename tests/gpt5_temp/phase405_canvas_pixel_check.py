#!/usr/bin/env python3
"""Check that Phase405 desktop and mobile WebGL captures are nonblank."""
...
SCREENSHOTS = ROOT / "tests/gpt5/result/phase405_natural_future_state/screenshots"
...
def main() -> None:
    ...
    payload = {
        "phase_id": "Phase405-CanvasPixelCheck",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "valid": len(rows) == 2 and all(row["nonblank"] for row in rows),
        "screenshots": rows,
    }
    (SCREENSHOTS / "phase405_canvas_pixel_check.json").write_text(
