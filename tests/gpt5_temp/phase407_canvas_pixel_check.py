#!/usr/bin/env python3
"""Check that Phase407 desktop and mobile WebGL captures are nonblank."""
...
SCREENSHOTS = ROOT / "tests/gpt5/result/phase407_event_horizon_kernel/screenshots"
...
def main() -> None:
    ...
    payload = {
        "phase_id": "Phase407-CanvasPixelCheck",
        "created_at": datetime.now(timezone.utc).isoformat(),
        "valid": len(rows) == 2 and all(row["nonblank"] for row in rows),
        "screenshots": rows,
    }
    (SCREENSHOTS / "phase407_canvas_pixel_check.json").write_text(
