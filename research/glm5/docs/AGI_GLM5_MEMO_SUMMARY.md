...
完整分析已追加到 [AGI_GLM5_MEMO.md](/D:/AI2050/Ai2050-OpenOne/research/glm5/docs/AGI_GLM5_MEMO.md:30099)，记录为 Phase 983。

**总体推进逻辑**

Phase 941–982 并不是沿着某个既定公式逐步完成闭合，而是经历了：
...
核心进步是不断发现并排除混杂因素，研究对象也从“某个神经元是否重要”逐渐转为“协议、语义、模式、时间和终止如何共同形成轨迹”。

## 1. Phase 941–944：语义方向落到物理组件

核心问题：语义方向是不是抽象统计向量，还是能落到真实残差坐标和 MLP 通道。
...
=
\operatorname{Vote}
\bigl(TopK(d_1),\ldots,TopK(d_n)\bigr)
$$
...
=
a_c(x)
\left\langle W_{down,:,c},d\right\rangle
$$
...
- Qwen3 部分颜色和功能通道具有正向因果信号。
- GLM4 候选通道和随机通道都能强烈移动边界，存在通用敏感性污染。
- DS7B 有坐标集中，但没有稳定边界因果效应。

所以这里只完成了“语义坐标→局部组件”的部分桥接，没有自然门控和生成闭合。

## 2. Phase 945–950：三条路线竞争

三条候选路线是：

- 路线 A：语义方向→通道→输出边界。
- 路线 B：否定、量化、模态等语言算子代数。
- 路线 C：协议 token、候选竞争和停止边界。
...
Qwen3、GLM4、DS7B 分别约为 `0.873、0.394、0.769`。这说明双否定并不是稳定的跨模型向量代数；算子组合也不是简单向量加法。

因此 Phase 950 选择路线 C 为主线，路线 A 保留，简单算子代数降级。

## 3. Phase 951–957：协议场与非线性混杂
...
z_{token}(x)
\approx
\beta_0+\sum_c\beta_ca_c(x)
$$

训练内一度得到接近 1 的 $R^2$，但交叉验证失败，说明是“通道多、样本少”造成的过拟合。

Phase 955 引入最重要的纠错公式：

$$
Odd(u)
=
\frac{\Delta z(+u)-\Delta z(-u)}{2}
$$
...
- `Odd`：真正与方向符号相关的一阶效应。
- `Even`：正负注入都出现的曲率、范数和全路径非线性效应。

三模型×五类 token 的 15 个条件全部以 `Even` 为主。这意味着此前大量“语义方向改变协议 token”的结果，其实主要来自非线性扰动。

## 4. Phase 958–963：组件能改变生成，但没有停止闭合
...
GLM4 `L39_H21` 被发现具有强模式控制效果，但增强它同时压低 EOS。按权重余弦寻找 EOS 促进组件也失败了：

$$
s_c
=
\left\langle W_{component,c},W_{U,EOS}\right\rangle
$$

原因是静态权重方向不能预测经过残差、归一化和后续层后的真实前向结果。到 Phase 963，所有自然组件组合的 strict-clean 仍为 0。

## 5. Phase 964–970：外部 EOS 控制与小样本假象

真正阻碍停止的不是 top1-top2 margin，而是：

$$
g=z_{top1}-z_{EOS}
$$

实测约为 `12–25`，远大于普通 margin 的约 1。
...
b(x)
=
\left\lfloor g_{after\_ablate}(x)+2\right\rfloor
$$
...
\Delta g
=
\Delta z_{top1}-\Delta z_{EOS}
$$

这说明既可以提高 EOS，也可以降低继续 token。

小样本一度得到 GLM4 `9/10=90%` clean；但 Phase 970 扩大到 65 条后降为：

- Clean：`42/65=64.6%`
- EOS：`61/65=93.8%`
- Expected：`49/65=75.4%`

而且外部 $b$ 减半后 clean 和 EOS 全部降为 0。自然 heads 只解释约 `4%–7%` 的 gap。因此这只是“外部控制上界”，不是自然机制闭合。

## 6. Phase 971–976：原生协议是关键条件

Phase 971 的 DLA 曾把 L38 解释为 protocol builder、L39 解释为 reducer，但真实前向干预没有稳定复现。解析贡献不能替代：

$$
\Delta g(x;I)=g_I(x)-g(x)
$$

Phase 973 修正合法 EOS 集合上的竞争：
...
- GLM4 从 plain-text 的 `0/160 EOS` 升到 native-chat 的 `144/160`。
- Qwen3 从 `0/160` 升到 `157/160`。

这证明此前大量“模型缺乏自然停止”的现象来自协议失配。

Phase 975–976 又发现 Qwen3 的 no-think mode block 具有跨新数据的因果必要性：

$$
N_s
=
g^*_{corrupt}(s)-g^*_{clean}(s)
$$

L34 完整 residual patch 能恢复当前 teacher-forced 状态，但尚未定位模式最初写入层和跨 token 维持机制。

## 7. Phase 977–982：从机制搜索转向严格轨迹验证

生成被拆成三道边界：

$$
T_C=\text{thinking 合法关闭位置}
$$

$$
T_A=\text{final answer 首次形成位置}
$$
...
若 $T_E>B$，只能说在预算 $B$ 下被右删失，不能直接判定永久失败。

Phase 978 中 hard-thinking 在 1536 token 时仍有：

$$
13/64=20.31\%
$$

达到预算上限，超过预注册的 10%，因此 development 必须判 `NO-GO`。
...
Phase 981 在全新 256 项、三条 seed stream 上，thinking 配置使有效停止分别增加 `94、102、98`，但也新增 `22、14、18` 条未闭合轨迹，全部超过事前上限 12，所以正式结论仍是 `NO-GO`。
...
C_{1536}
\rightarrow
(V_{2048},C_{2048},I_{sem,2048})
=
(11,54,14)
$$

更长预算既会让部分轨迹正确结束，也会让另一部分错误结束，并不单调趋向正确。

**最终结论**

Phase 941–982 没有得到语言编码的闭合数学公式，但完成了非常重要的研究升级：

1. 排除了最高激活、简单算子加法、静态权重余弦等过度简化方案。
2. 证明部分语义子空间和组件具有局部因果作用。
3. 证明协议、角色和输出模式会重构整个生成动力学。
4. 建立了合法模式、语义答案、实际 EOS、预算删失和失败通道的严格测量框架。
5. 明确区分了“外部可以强制控制”与“模型内部自然机制”。

当前最应该做的不是继续寻找最高神经元或拟合宏大公式，而是先用全新、角色反转、选项互换、任务平衡的数据，稳定一个可重复的自然轨迹目标；外部目标在第二份数据通过后，再打开 token×layer×component×time 的内部因果脉络提取。只有反复出现的真实路径稳定下来，数学结构才有资格从数据中自然浮现。