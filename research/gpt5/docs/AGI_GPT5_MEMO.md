...
## Phase 327: 恢复原 DNN 形状并叠加语言模式族物理证据 [2026-07-09 21:15]

### 任务背景

Phase325/326 为了突出语言模式族物理候选，客户端在模式族开关开启时使用互斥渲染分支：

```text
模式族视图开启 -> 卸载原 AppleNeuronSceneContent -> 显示独立竖直脉络；
模式族视图关闭 -> 恢复原 DNN 层栈和节点场景。
```

该实现虽然聚焦证据，但改变了三维空间中的深度神经网络基础形状、相机位置和观察习惯，不符合“在固定 DNN 模型上持续叠加研究结果”的工程目标。

本阶段没有修改 Phase326 的研究数据或证据等级，只修改场景组合方式。
...
```text
基础模型层：负责原 DNN 形状、层平面、模型标题、层内展开、前向动画和旧交互；
研究证据层：负责语言模式族、物理候选节点、证据颜色、筛选和详情；
研究证据层不得改变基础模型的坐标、相机或形状。
```

原 DNN 层平面的深度坐标为：

$$
z_l = \left(l-\frac{L-1}{2}\right)\times 0.92
$$

模式族物理候选继续使用确定性角度和半径，但投影到同一个层平面：

$$
\theta_u = 2\pi\operatorname{frac}(0.618033988749895u + 0.037l + b_{kind})
$$

$$
p_u = \left(r_u\cos\theta_u,\ r_u\sin\theta_u,\ z_l\right)
$$

其中 $b_{kind}$ 只用于避免注意力头、多层感知机组和单元索引发生完全重叠；该坐标是显示布局，不是模型内部真实欧氏几何。
...
```text
删除模式族视图对原 DNN 场景的互斥替换；
AppleNeuronSceneContent 始终渲染；
RealUnitTraceRenderer 和原逆向分析叠层始终按旧规则渲染；
PatternFamilyNeuronAtlasRenderer 只在开关开启时附加；
恢复原 DNN 相机位置、视场角、观察中心和最小缩放距离；
模式族开关现在只控制研究叠层显隐。
```

`PatternFamilyNeuronAtlasRenderer.jsx`：

```text
新增 overlay 模式；
overlay 模式不生成独立来源节点、读出节点、竖直主轴、标题和图例；
候选按真实层号投影到原 DNN 的 Z 层平面；
每个候选只连接到所在层的中心锚点；
候选筛选、颜色、拾取、点击详情和证据边界保持不变；
未映射模式族在 overlay 模式不生成占位几何，基础 DNN 继续显示。
```
...
格式文档增加强制场景集成规则：模式族图谱只能作为基础 DNN 的叠层，不能再次替换或重塑基础模型。
...
```text
PatternFamilyNeuronAtlasRenderer.jsx：通过；
PatternFamilyAtlasControls.jsx：通过；
App.jsx 全文件仍有既有未使用变量等历史 ESLint 错误，本阶段没有扩大处理范围；
git diff --check：通过。
```
...
```text
内容知识/推理约束切换和扩大确认筛选：通过；
390x844 面板边界和页面无横向溢出：通过；
关闭模式族叠层后 Canvas 仍可见且保持非空：通过；
开启和关闭叠层的 Canvas 图像不同，说明开关只改变附加证据；
3 tests passed。
```
...
```text
桌面开启叠层：原 36 层平面、Qwen3-4B 标题、层号、层内展开结构均保留；
桌面关闭叠层：彩色候选消失，原 DNN 层栈和相机视角不变；
390x844：原层栈可见，候选附着在对应层面，底部叠层控制可用；
未出现原竖直脉络与旧 DNN 同时叠加的双骨架问题。
```
...
```text
1. 本阶段是客户端坐标和组合方式修复，没有增加新的模型证据；
2. 原 DNN 形状本身是工程显示坐标，不是模型参数空间的真实几何；
3. 模式族候选投影到层面只表示层地址，不表示候选之间的真实距离；
4. 候选到层中心的连线仍然 causal=false；
5. 叠层最多显示当前筛选后的关键候选，不代表该层只有这些神经元；
6. Phase326 的 72 个扩大确认集合成员、288 个集合成员和 833 个单元候选数量均未改变；
7. 单神经元因果、自然门控和 L5 闭合仍然为 0。
```

### 进度与下一步

本阶段只提升可视化一致性，不上调语言编码机制研究进度：
...
```text
新研究结果 -> 固定证据分区 -> 投影到原 DNN 层坐标 -> 可开关叠加；
基础 DNN 形状不再随研究阶段更换；
只有真实数据新增或证据等级变化时才修改候选叠层；
Phase327 原计划的自然知识检索因果链研究顺延为下一研究阶段。
```

### 阶段结论

```text
原深度神经网络层栈、相机和交互形状已经恢复；
语言模式族和物理候选现在是旧模型上的独立证据叠层；
打开叠层可以研究模式族关键脉络，关闭叠层可以无损查看旧模型；
研究数据、证据边界和因果计数没有因视觉改造而改变。
```
