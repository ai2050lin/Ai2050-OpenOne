...
## Phase 317: 图谱到主 3D 真实单元映射审计与证据系统接入 [2026-07-09 22:23]

### 任务

分析 Phase316 最新图谱能否加载到主 3D 神经网络空间，并在前向传播动画中以真实神经元地址定位显示；同时实现当前已有真实单元数据的最小完整接线。

### 数据层审计

Phase311-316 的三类主要记录具有不同物理精度：

```text
phase314_graph_nodes / graph_edges:
  family、model、position_role、attention/MLP/residual 组件事件和 peak depth；
  没有 layer + unit_kind + unit_index 的完整真实单元地址。

phase311_core_language_component_rows:
  具有 model、layer_index、component、token_position 和组件 margin；
  可以定位层、组件和 token 位置，不能定位单神经元。

Phase286/287 Evidence Kernel:
  具有 model_revision、layer、component、unit_kind、unit_index、token_position、source_artifact；
  可以定位真实 MLP product unit、attention head channel、residual dimension 和 W_U token。
```

因此结论必须分成两层：

```text
Phase311-316 全局语言模式图谱：可以加载到主 3D，显示模式族、层、组件和路径事件；
Phase286/287 颜色真实 Trace：可以在播放中定位真实物理单元；
Phase311-316 全图谱：目前不能整体宣称为单神经元图谱，因为缺少 unit address。
```
...
显示坐标只是地址的确定性布局，不是声称模型内部天然存在三维几何：
...
(x,y)=(r_{component}\cos\theta,r_{component}\sin\theta)
$$

其中 $l$ 是真实层号，$u$ 是对应组件内的真实 index，$U$ 是该组件的真实宽度。不同 `unit_kind` 使用不同半径，避免把 attention channel、MLP neuron、residual dimension 和 vocabulary token 淵称为同一种神经元。
...
frontend/src/neural_vis/renderers/RealUnitTraceRenderer.jsx
frontend/src/App.jsx
frontend/src/blueprint/EvidenceKernelDashboard.jsx
server/research_orchestrator/*
frontend/src/AIRnD/AIRnDOrchestratorTab.jsx
```
...
```text
选择模型
-> 读取 research_kernel manifest
-> 读取对应 Phase287 real component trace
-> 读取 Phase286 稳定真实单元候选
-> 当前 layer + 当前 subphase 查找真实 event
-> 从 event.top_units 取得物理地址
-> RealUnitTraceRenderer 定位
-> hover 显示 evidence level、causal scope 和 source artifact
```

前向动画已从“每个计时周期直接跳到下一层”改为：

```text
input -> norm1 -> q/k/v -> attention -> residual1
-> norm2 -> gate/up -> product -> down -> residual2 -> next layer
```

每个子阶段按真实 Trace 事件切换当前高亮单元。Phase287 没有直接捕获 pre-softmax attention score 和 softmax probability，因此这两个阶段明确显示“暂无直接组件观测”，没有用 K/V 投影冒充注意力权重。

### 当前真实 Trace 数据

三模型已存在完整 embedding-to-W_U 单提示 Trace：

```text
Qwen3:      36 layers, 470 events, 469 full vectors, next token = " red"
GLM4:       40 layers, 482 events, 481 full vectors, next token = " red"
DeepSeek7B: 28 layers, 366 events, 365 full vectors, next token = " determined"
```

DeepSeek7B 的局部颜色候选中 red 排名第一，但全词表下一 token 为 `determined`。这再次证明：

$$
LocalCandidateFieldOpen \not\Rightarrow GlobalOutputClosed
$$

当前动画显示的是“这一计算事件中激活较大的真实地址”和“Phase286 稳定颜色候选”，不能把 Top-K 激活直接解释为单神经元因果机制。

### 验证

```text
frontend npm run build: passed, 3664 modules transformed
research/unit/trace/orchestrator/Phase288 design tests: 14 passed
Phase288 1440-case design dry-run: passed
```

本阶段没有运行新的 Phase288 CUDA 单神经元干预。Phase288 只完成了正式脚本、1440-case 分母、匹配激活同层对照和 dry-run；不得记录为新的因果结果。

### 仍缺少的关键连接

要让 Phase311-316 全图谱都能在动画中定位真实神经元，必须增加统一连接表：
...
```json
{
  "atlas_case_id": "phase311:content_knowledge:category_binding:00:qwen3",
  "trace_run_id": "...",
  "event_type": "mlp_product",
  "layer": 22,
  "token_position": 3,
  "real_unit_refs": [
    {
      ...
      "unit_index": 6212,
      "evidence_level": "L2",
      "source_artifact": "trace_events.jsonl#row=..."
    }
  ]
}
```

没有 `real_unit_refs` 的图谱事件只能显示在组件层，客户端不得用伪随机 index 补成神经元。

### 严格结论

```text
可以加载图谱并在主 3D 中播放；
当前已能对 Phase286/287 颜色 Trace 定位真实单元；
Phase311-316 只能精确到层、组件和 token 位置；
若要完成全图谱单神经元动画，下一步不是继续修改 3D 外观，
而是为每个 atlas case 采集真实 component trace 并建立 atlas-to-unit join index。
```

这一区分很重要：3D 渲染能力已经具备，真正的瓶颈是全局图谱尚未生成足够的真实单元级数据，而不是客户端不会画。
