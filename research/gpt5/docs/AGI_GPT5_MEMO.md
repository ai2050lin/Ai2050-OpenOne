...
## Phase 403: Phase401 超 100MB 结果文件忽略与远端重提交 [2026-07-13 01:57]
...
该提交中包含 3 个超过 100MB 的 Phase401 局部边图结果文件：

```text
tests/gpt5/result/phase401_local_edge_graph/local_edges/discovery/private/glm4/pair_rows.jsonl
tests/gpt5/result/phase401_local_edge_graph/local_edges/discovery/private/qwen3/pair_rows.jsonl
tests/gpt5/result/phase401_local_edge_graph/local_edges/discovery/private/deepseek7b/pair_rows.jsonl
```

它们会触发 GitHub 普通 Git 仓库的 100MB 单文件限制。
...
```text
1. 不删除本地大结果文件；
2. 从待推送提交中移除超过 100MB 的 blob；
3. 将三条超限路径加入 .gitignore；
4. 重新提交其余脚本、摘要、图谱小文件和文档；
5. 不运行 CUDA 模型测试。
```
...
```text
backup/pre-100mb-ignore-20260713-015703
```
...
加入 `.gitignore` 后重新暂存。Git 状态显示三条大文件已经被忽略：
...
第一，本阶段没有改变 Phase401 的实验结论，只是把原始大轨迹从 Git 普通历史中排除。

第二，局部边图的完整 `pair_rows.jsonl` 仍在本地硬盘上；若需要跨机器共享，应转入 Git LFS、对象存储或分片归档。

第三，未来脚本应在写出结果后自动检查文件大小，超过 100MB 时只提交摘要、校验和与索引。

### 5. 结论

本阶段完成 Phase401 三个超 100MB 文件的 Git 忽略准备：
...
Phase401 已经证明真实来源 K/V 替换能产生可重复的直接注意力响应，但不能超过全部错误来源、错误结构、错误位置和错误时间控制。Phase402 不再继续调整同一个单来源补丁，而是检验多个真实计算图父分区能否联合形成更特异的直接子状态。

对“模型能正确输出，因此内部必然存在高效、类似大脑的语言结构”这一判断需要分层：

1. 模型能够稳定输出正确内容，说明内部必然存在实现这些行为的某种计算，这是必要事实。
2. 该计算是否紧凑、唯一、可读、与大脑语言结构同构，当前没有证据。
3. 静态状态是否可以商掉表面差异形成因果状态商，当前没有证据。
4. 是否存在跨任务稳定算子代数，当前没有证据。
5. 神经元、头、通道和层级单元没有单独闭合，不等于它们不可能作为联合机制的组成部分。

因此本阶段先检验可执行的多父直接子状态，不预设因果状态商、算子代数或大脑结构还原。

### 二、四类可执行父分区

原先的内容、结构、查询、历史只是语义标签，不能直接干预。Phase402 将接收者位置之前的真实注意力 K/V 位置冻结为四个互斥分区：

```text
source_content：来源实体和值词元；
source_structure：来源区域内其余结构词元；
query_local：查询实体、查询结构词元与接收者自身；
remaining_prefix：尚未归入前三类的提示前缀。
```

四类分区互斥，且并集覆盖接收者之前的全部位置。`remaining_prefix` 不是生成历史，因为首个答案词元之前还没有已生成词元。
...
枚举 `16` 个子集，空集作为重放控制。对供体 (d)、接收者 (r)、层 (l)、查询位置 (q)：
...
直接子节点只定义为查询位置注意力输出。没有在发现阶段测量最终自然生成，也没有把同层 MLP 或后续层状态登记成额外直接边。

### 三、行为分母

注册六个任务面：实体属性绑定、角色填槽、指代解析、两步组合、条件存在、数量一致。

每面 `24` 组、每组 `16` 个析因条件、三个模型：

\[
6\times24\times16\times3=6912.
\]

候选组在生成前冻结为发现、校准、物理留出 `12/6/6`。任务面必须在三个模型上拥有至少 `6/3/3` 个完整组，禁止跨分割补组。

模型行为结果：

```text
Qwen3：2125/2304 语义正确；
GLM4：1712/2304 语义正确；
DS7B：1748/2304 语义正确；
合计：5585/6912。
```

GLM4 的格式完全匹配为 `0/2304`，主要是正确目标后的续写，不能改写为所有语义答案错误。

三模型完整组：

```text
实体属性绑定：19，发现/校准/物理为 11/2/6，不合格；
角色填槽：22，11/6/5，合格；
指代解析：9，3/3/3，不合格；
两步组合：0，不合格；
条件存在：18，9/5/4，合格；
...
因此只有角色填槽和条件存在进入内部追踪，冻结两个任务面各 `6/3/3` 个组，共 `1152` 个模型案例；其中发现集 `576`、校准保留 `288`、物理留出保留 `288`。

当前结果不能代表知识、推理和语法均已覆盖。两步组合与数量一致在本轮没有合格内部分母。
...
```text
Qwen3：1152/1152 行通过，最大空子集误差 0.002610453；
GLM4：1280/1280 行通过，最大空子集误差 0.003598911；
DS7B：896/896 行通过，最大空子集误差 0.008925387；
合计：3328/3328。
```

最初 GLM4 有 `13/1280` 行失败。原因是重放代码在 QK 矩阵乘法前提升到 float32，与模型原生 dtype 路径不同。代码修正为先按原生 dtype 完成 QK、再以 float32 softmax，随后按照 Qwen3、GLM4、DS7B 的顺序重新运行全部账本。阈值 `0.01` 没有放宽，失败运行未进入正式结果。
...
令自然供体、接收者和反事实直接子状态为 (v_d,v_r,v_{cf})：
...
配对门要求自然差异足够、(D\ge0.6)、(R\ge0.2)。联合组层子集还要同时满足：
...
控制包括同目标错误顺序、错误接收者、错误语义时刻、错误深度、内容角色置换、同内容错误结构、随机自然供体和符号置换。符号置换仅为测量控制，不是合法计算图父补丁；不适用控制在采集前声明并从对应门排除。

### 六、三模型 CUDA 发现结果

按 Qwen3、GLM4、DS7B 顺序完成：

```text
Qwen3：192 案例，36 层，995328 配对指标，62208 聚合指标；
GLM4：192 案例，40 层，1105920 配对指标，69120 聚合指标；
DS7B：192 案例，28 层，774144 配对指标，48384 聚合指标；
合计：576 案例，2875392 配对指标，179712 聚合指标。
```
...
```text
S0110 = source_structure + query_local；
depth_zone = early。
```
...
它们说明少量早层案例存在联合局部响应迹象，但没有在同一模型的足够独立组中稳定，也没有三模型复现。
...
\[
\boxed{
\text{四个宽 K/V 父分区在同层查询注意力直接子节点上形成可跨模型父状态}
}
\]
...
完整语言路径仍为 `0/72`，单神经元因果路径仍为 `0/72`。

### 八、协议硬伤

1. 发现组率冻结为 `0.666666667`。六组分母下 `4/6` 小于该近似值，所以实际要求 `5/6`。本轮按冻结合同执行，未事后放宽。它不改变跨模型为零的结论，但下一阶段必须直接使用整数门。
2. `source_structure` 与 `query_local` 仍是宽位置集合，不能直接解释为抽象结构变量和查询变量。
3. 本轮只测同层直接注意力子状态，没有终端语义和自然生成效果。
4. `2875392` 条指标不是等量独立样本，科学独立单位首先是预注册平行组。
5. 六个任务面只有两个通过行为门，不能外推到九个语言模式族。
6. 小模型可能粗糙，但不能用模型规模事后消解本轮冻结合同上的负结果。

### 九、图谱与客户端同步

Phase402 已同步到 Pattern Family Atlas 与神经元图谱镜像。客户端显示：
...
公开数据只包含阶段摘要、八个局部单元和证据边界。`2875392` 条私有配对指标、完整组层表和原始状态不发布到客户端。3D 只能用聚合迹象显示 `S0110` 的早层局部提示，不能画成具体头、通道、神经元或完整语言路径。
...
单一全局科学进度百分比仍无效。若继续保留约 `23%` 的项目管理估计，本阶段不能因记录量巨大而上调；科学硬分母仍是完整语言路径 `0/72`、单神经元路径 `0/72`。

### 十一、下一阶段

下一阶段应改为动态跨层多组件父链，但当前只授权协议设计，不授权自动启动模型运行。原因是新的实验改变因果对象与数据合同，不能复用 Phase402 校准/物理组，也不能放宽当前门槛继续挖掘。
...
所有离散门使用整数计数；
提示前缀与生成历史分离；
把宽父分区改为来源读取、MLP 写入、查询整合、跨层传播、终端读出事件；
直接子门、传播门、终端语义门分别冻结；
建立新的知识、两步推理和语法行为分母；
继续按 Qwen3、GLM4、DS7B 顺序运行；
只在三模型发现候选后打开校准；
只在校准通过后打开物理留出和终端验证；
只在自然答案因果成立后定位头、通道和神经元。
```
...
结果目录：`tests/gpt5/result/phase402_multiparent_graph/`。

详细报告：`research/MainAnalysis/20260713_03_多父条件状态与直接子状态审计.md`。
...
Vite 生产构建：通过，2817 个模块；
AtlasControlDashboard 与 EvidenceKernelDashboard 定向 ESLint：通过。
```

Vite 仍有主分块超过 500 kB 的既有体积警告，不影响本阶段证据同步。

通俗说明：这轮把四类上下文真的换进网络并让注意力重新计算。少数 DS7B 和一个 GLM4 早层案例显示，“来源结构和查询上下文一起换”比只换一个更像供体状态；但 Qwen3 没有，同一模型也不够稳定。因此它只能作为局部提示，不能画成语言线路，更不能放大到神经元。下一步需要按读取、写入、整合、传播和读出的真实顺序寻找事件链，而不是继续扩大同一种静态补丁。

补充客户端验证：桌面 `1440x900` 与移动 `390x844` 的五项 Phase402 文本断言全部可见，两个视口均无横向页面溢出。视觉检查记录一条可选后端连接被拒绝的既有控制台信息，但静态图谱数据、Phase402 摘要和证据分母均加载成功。
