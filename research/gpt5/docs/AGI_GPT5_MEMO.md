...
## Phase 379: GitHub 大文件拒绝推送的本地历史清理 [2026-07-12 01:05]
...
被 GitHub 拒绝。直接原因是本地未推送提交历史中包含多个超过 GitHub 100MB 限制的结果文件，并有若干超过 50MB 推荐上限的文件。

关键超限文件包括：

```text
phase354_semantic_time_rows.jsonl：330MB-553MB；
phase352_generated_time_rows.jsonl：115MB-162MB；
phase365 directed_flow_descriptors：211MB-300MB；
phase371c_blind_route_contrasts.jsonl：337MB；
phase375_blind_subgraph_inventory.jsonl：55MB。
```

### 2. 处理原则

本阶段只解决 Git 推送限制，不删除本地科研结果。

采用原则：

```text
1. 大结果文件保留在本地硬盘；
2. 从待推送的 Git 历史中移除超限 blob；
3. 将明确超限或接近超限的结果文件写入 .gitignore；
4. 重新生成不含大文件的新本地提交；
5. 不运行 CUDA 模型测试，不改变实验结论。
```
...
```text
backup/pre-large-file-cleanup-20260712-010350
```

然后将本地未推送提交拆回工作区，加入 `.gitignore` 大文件规则，再重新暂存可提交内容。
...
```text
git diff --cached --name-only -z | xargs -0 ls -l
```
...
第一，本阶段没有压缩或抽样这些大结果文件，只是让它们不进入 GitHub 普通 Git 历史。

第二，若后续需要共享完整大结果，应使用 Git LFS、外部对象存储或分片压缩索引，而不是普通 Git blob。

第三，结果文件不入 Git 不等于结果无效。它只说明 GitHub 普通仓库不适合承载数百 MB 的逐行轨迹。

第四，本地历史被重写为新的未推送提交；已有备份分支可回退。

### 5. 理论和工程启示

语言机制研究正在进入“证据文件远大于代码文件”的阶段。此时 Git 的角色应从保存全部原始轨迹，转为保存：
...
完整轨迹应进入专门的大对象存储。否则版本控制系统会成为理论推进的瓶颈。
...
```text
1. 超过 50MB 的结果默认不进入 Git；
2. 每个大结果生成摘要、行数、sha256 和本地路径索引；
3. public 图谱只发布摘要和可视化所需小文件；
4. 原始大轨迹转存到 LFS 或外部归档；
5. 测试脚本输出时自动检测文件大小并给出提交建议。
```

### 7. 结论

本阶段完成 GitHub 大文件推送失败的本地历史清理准备：
