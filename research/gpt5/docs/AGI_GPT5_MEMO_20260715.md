...
Qwen3：4 个物理转移对象，4 个赢家翻转对象；
GLM4：4 个物理转移对象，4 个赢家翻转对象；
DS7B：4 个物理转移对象，2 个赢家翻转对象；

异构物理确认对象：4；
三模型物理确认对象：2。
```

关系绑定晚段当前位置残差在三模型、发现、校准和物理三个分母均复现双向答案词元转移。

实体新近性在 Qwen3+GLM4 物理复现；DS7B 的转移增益通过，但赢家翻转只有 1/4 组，未达到 3 组门。

质量异常：DS7B 有 1/32 个物理自然决策重放词元不一致，未删除案例。
...
同时，机制条件 A/C 和直接路线控制 B/D 都产生同量级的大幅内容转移。

因此四个原始模板对象必须合并为两个最小证据对象：
...
$$
\boxed{\text{晚段当前位置残差能够因果搬运下一答案词元内容}}
$$
...
它没有回答内容在哪里形成，也没有证明来源和查询路径对赢家转移是必要的。

### 四、图谱同步

语言模式图谱和神经元图谱已同步 Phase378：
...
客户端可以显示两个经过物理确认的末端载体，以及“上游形成未知”节点；不会把它们显示成语言机制或神经元路径。

### 五、算法和智能理论更新

本阶段最重要的算法进展不是找到一个输出端点，而是确认了正确的研究坐标：

```text
固定词元编号不是跨模型语义时间；
答案决策事件才是可比较时间锚点；
跨时向量几何相似不是首要因果证据；
预注册的自然边界干预可以直接检验内容转移；
输出端载体与上游计算机制必须分层登记。
```

模型能够生成正确答案而旧算法无法闭合，并不是因为“网络没有规则”，而是因为旧算法同时犯了两个错误：

1. 在不同模型中比较了不同语义阶段。
2. 要求一个局部向量在原坐标中线性预测经过非线性层和新词元后的未来向量。

修复这两个错误后，三模型立即出现稳定的物理内容转移正结果。这说明算法读出确实是此前瓶颈之一。

但末端残差已经靠近词表输出，它更像“已经算好的答案载体”，不是“答案如何算出来”的规则。智能理论仍未闭合。

### 六、下一阶段 Phase379

阶段目标：定位末端当前位置内容状态在更早层中如何形成。

必须停止继续交换末端残差，改为以末端残差和供体词元优势为双重读出，追踪更早的自然事件：
...
$$
\Delta h_{terminal}^{(c,l,r)}
=
h_{terminal}(\operatorname{swap}(c,l,r))-h_{terminal}(baseline)
$$
...
\Delta M^{(c,l,r)}
=
M(\operatorname{swap}(c,l,r))-M(baseline)
$$
...
```text
Phase375-378 + Phase369/371 回归测试：27/27 通过；
Phase365 动态仪器测试：17/17 通过；
总计：44/44 通过；
前端生产构建：通过；
语言图谱与客户端镜像：一致；
神经元图谱新增路径：0；
已知 Vite 大 chunk 警告：仍存在，不影响数据一致性。
```

### 八、通俗总结

以前三个模型看起来都在正确答题，但内部路线总对不上。现在找到一个很具体的原因：我们拿“生成后的第一个、第二个、第三个词”当共同时间点，可 DS7B 经常到第十五个词才真正开始给答案。三个模型其实没有在同一时刻被比较。

改成“就在目标答案词即将生成之前”以后，把一个案例晚层当前位置的内部状态换给另一个案例，三个模型都会把下一个答案词明显推向供体内容。这个结果经过发现集、独立校准和物理留出三轮验证。

不过这仍像把已经写好的答案纸条递给输出端，并没有解释纸条是怎样写出来的。下一轮真正要追的是：哪些更早的注意力、MLP 和残差事件共同把这张“答案纸条”形成出来。
